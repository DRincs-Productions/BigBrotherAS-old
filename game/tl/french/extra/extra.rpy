# TODO: Translation updated at 2022-10-30 23:58

translate french strings:

    # game/extra/extra.rpy:60
    old "Новый купальник Лизы"
    new "Le nouveau maillot de bain de Lisa"

    # game/extra/extra.rpy:60
    old "Извинительная пижамка для Алисы"
    new "Pyjama du pardon pour Alice"

    # game/extra/extra.rpy:60
    old "Тёмное кружево"
    new "Dentelle sombre"

    # game/extra/extra.rpy:60
    old "Влажные фантазии"
    new "Fantaisies humides"

    # game/extra/extra.rpy:60
    old "Давай я нанесу крем"
    new "Laisses-moi appliquer la crème"

    # game/extra/extra.rpy:60
    old "Ножкам приятно"
    new "Tes jambes sont belles"

    # game/extra/extra.rpy:60
    old "Помассирую не только ножки"
    new "Je vais masser plus que tes jambes"

    # game/extra/extra.rpy:60
    old "Могу не только руками"
    new "Je peux faire plus avec mes mains"

    # game/extra/extra.rpy:60
    old "Ответная благодарность"
    new "Merci en retour"

    # game/extra/extra.rpy:60
    old "Первый массаж ног"
    new "Premier massage des jambes"

    # game/extra/extra.rpy:60
    old "Внимание к пальчикам"
    new "Attention aux doigts"

    # game/extra/extra.rpy:60
    old "Разомнём и плечики"
    new "Massage des épaules"

    # game/extra/extra.rpy:60
    old "Массаж для любимой тёти"
    new "Massage pour votre tante bien-aimée"

    # game/extra/extra.rpy:60
    old "Совсем другой массаж"
    new "Un massage complètement différent"

    # game/extra/extra.rpy:60
    old "Я спасу их от солнца"
    new "Je vais les sauver du soleil"

    # game/extra/extra.rpy:60
    old "Ночные страхи"
    new "Terreurs nocturnes"

    # game/extra/extra.rpy:60
    old "Кто это там ползёт"
    new "Qui est-ce qui rampe là-bas"

    # game/extra/extra.rpy:60
    old "Монстр в ванной комнате"
    new "Monstre dans la salle de bain"

    # game/extra/extra.rpy:60
    old "Держи и не отпускай"
    new "Persévérer et ne pas lâcher prise"

    # game/extra/extra.rpy:60
    old "После клуба"
    new "Après le club"

    # game/extra/extra.rpy:60
    old "Я была плохой девочкой"
    new "J'ai été une mauvaise fille"

    # game/extra/extra.rpy:60
    old "Как тебе такое?"
    new "Comment est-ce possible pour toi?"

    # game/extra/extra.rpy:60
    old "Я принес тебе полотенце!"
    new "Je t’ai apporté une serviette!"

    # game/extra/extra.rpy:60
    old "Первые снимки для блога Алисы"
    new "Premières photos pour le blog d’Alice"

    # game/extra/extra.rpy:60
    old "Я обошёл Эрика с подарком для Алисы"
    new "Je suis allé voir Eric avec un cadeau pour Alice"

    # game/extra/extra.rpy:60
    old "Попка, которую я теперь могу отшлёпать"
    new "Je peux maintenant donner une fessée"

    # game/extra/extra.rpy:60
    old "Меня нужно наказать именно так!"
    new "Je dois être puni comme ça!"

    # game/extra/extra.rpy:60
    old "Смотрим порно вместе с тётей"
    new "Regarder du porno avec ma tante"

    # game/extra/extra.rpy:60
    old "Первый урок поцелуев"
    new "La première leçon de baisers"

    # game/extra/extra.rpy:60
    old "А это уже совсем не массаж!"
    new "Ce n'est pas du tout un massage !"

    # game/extra/extra.rpy:60
    old "Второй урок поцелуев"
    new "La deuxième leçon de baisers"

    # game/extra/extra.rpy:60
    old "Третий урок поцелуев"
    new "La troisième leçon de baisers"

    # game/extra/extra.rpy:60
    old "Горячее, чем порно"
    new "Plus chaud que le porno"

    # game/extra/extra.rpy:60
    old "Небольшое приключение перед сном"
    new "Une petite aventure au coucher"

    # game/extra/extra.rpy:60
    old "Не зря купил сорочку"
    new "Pas étonnant que j’ai acheté une chemise"

    # game/extra/extra.rpy:60
    old "С меня приятный должок"
    new "J'ai une belle dette de gratitude"

    # game/extra/extra.rpy:60
    old "Стриптиз после клуба"
    new "Striptease après le club"

    # game/extra/extra.rpy:60
    old "Порно-портфолио для Киры"
    new "Porte folio pour Kira"

    # game/extra/extra.rpy:60
    old "Немного БДСМ от Киры"
    new "Un peu de BDSM avec Kira"

    # game/extra/extra.rpy:60
    old "Хватит мять сиськи"
    new "Arrêtes de serrer les seins"

    # game/extra/extra.rpy:60
    old "И помылись, и порезвились"
    new "Nous avons pris le bain, nous nous sommes amusés"

    # game/extra/extra.rpy:60
    old "Кто нас фотографирует?"
    new "Qui nous photographie ?"

    # game/extra/extra.rpy:60
    old "Вместо ночного плаванья"
    new "Au lieu de nager la nuit"

    # game/extra/extra.rpy:60
    old "Вкусные уроки с сестрёнкой"
    new "Délicieuses leçons avec ma sœur"

    # game/extra/extra.rpy:60
    old "Кажется, мы что-то забыли"
    new "J'ai oublié quelque chose, non?"

    # game/extra/extra.rpy:60
    old "Всё ниже и ниже"
    new "De plus en plus bas"

    # game/extra/extra.rpy:60
    old "Больше, чем помощь с домашним заданием"
    new "Plus que de l’aide pour les devoirs"

    # game/extra/extra.rpy:60
    old "Ужастики в обнимку с Лизой"
    new "Horreur, avec un câlin, avec Lisa"

    # game/extra/extra.rpy:60
    old "Без майки куда интереснее"
    new "Sans T-shirt c'est beaucoup plus intéressant"

    # game/extra/extra.rpy:60
    old "Долой смущение"
    new "A bas l'embarras"

    # game/extra/extra.rpy:60
    old "Это точно триллер-детектив?"
    new "Est-ce un détective professionnel?"

    # game/extra/extra.rpy:60
    old "Полотенце снова сползает..."
    new "La serviette glisse à nouveau..."

    # game/extra/extra.rpy:60
    old "Йога с мамой"
    new "Yoga avec maman"

    # game/extra/extra.rpy:60
    old "Ещё больше массажа у ТВ"
    new "Encore plus de massage à la télé"

    # game/extra/extra.rpy:60
    old "Не надо печалиться"
    new "Ne sois pas triste"

    # game/extra/extra.rpy:60
    old "В магазин с Алисой"
    new ""

    # game/extra/extra.rpy:60
    old "С добрым утром"
    new ""

    # game/extra/extra.rpy:60
    old "Афера века"
    new ""

    # game/extra/extra.rpy:60
    old "Сама напросилась"
    new ""

    # game/extra/extra.rpy:60
    old "Так себе охранник"
    new ""

    # game/extra/extra.rpy:60
    old "Хоть на что-то сгодился"
    new ""

    # game/extra/extra.rpy:60
    old "Пока паук где-то рядом..."
    new ""

    # game/extra/extra.rpy:60
    old "Лучшее пробуждение с Алисой"
    new ""

    # game/extra/extra.rpy:60
    old "Лучше делать то, что она хочет..."
    new ""

    # game/extra/extra.rpy:60
    old "Покорность будет вознаграждаться..."
    new ""

    # game/extra/extra.rpy:60
    old "Выбери своё наказание"
    new ""

    # game/extra/extra.rpy:60
    old "Вот и посмотрели ТВ"
    new ""

    # game/extra/extra.rpy:60
    old "В ванную комнату за минетом!"
    new ""

    # game/extra/extra.rpy:60
    old "Меня ждёт кое-что особенное..."
    new ""

    # game/extra/extra.rpy:60
    old "Самый хитрый охранник"
    new ""

    # game/extra/extra.rpy:60
    old "Урок по минету от мамы и Эрика"
    new "Leçon de maman et Eric"

    # game/extra/extra.rpy:60
    old "Так близко к маминой попке"
    new "Si près du cul de maman"

    # game/extra/extra.rpy:60
    old "Глубокий минет в мамином исполнении"
    new "Une fellation profonde réalisée par maman"

    # game/extra/extra.rpy:60
    old "Секс-шоу с первых рядов"
    new "Sexe-show des premiers rangs"

    # game/extra/extra.rpy:60
    old "Её первые познания..."
    new "Ses premières connaissances..."

    # game/extra/extra.rpy:60
    old "Как возбудить ещё больше?"
    new "Comment être encore plus exciter?"

    # game/extra/extra.rpy:60
    old "Нежно и аккуратно!"
    new "Doux et soigné!"

    # game/extra/extra.rpy:60
    old "Как долго это нужно делать?"
    new "Combien de temps cela prend-il ?"

    # game/extra/extra.rpy:60
    old "Стесняется трогать..."
    new "Trop timide pour toucher..."

    # game/extra/extra.rpy:60
    old "Ещё стесняется, но трогает..."
    new "Encore timide, mais oser toucher..."

    # game/extra/extra.rpy:60
    old "Кружевное боди для Алисы от Эрика"
    new "Body en dentelle pour Alice d'Eric"

    # game/extra/extra.rpy:60
    old "1-ый интимный урок с Анной"
    new "La 1ère leçon intime avec Anna"

    # game/extra/extra.rpy:60
    old "2-ый интимный урок с Анной"
    new "2ème leçon intime avec Anna"

    # game/extra/extra.rpy:60
    old "3-ый интимный урок с Анной"
    new "La 3e leçon intime avec Anna"

    # game/extra/extra.rpy:60
    old "4-ый интимный урок с Анной"
    new "La 4e leçon intime avec Anna"

    # game/extra/extra.rpy:192
    old "ВОСПОМИНАНИЯ"
    new "ENREGISTREMENTS"

    # game/extra/extra.rpy:195
    old "ФОТОСНИМКИ"
    new "PHOTOS"

    # game/extra/extra.rpy:248
    old "Воспоминание недоступно"
    new "Les enregistrements ne sont pas disponible"

    # game/extra/extra.rpy:254
    old "Воспоминание ещё не открыто"
    new "La mémoire n'est pas encore ouverte"

    # game/extra/extra.rpy:260
    old "В вашей коллекции ещё нет фотоснимков."
    new "Il n'y a pas encore de photos dans votre collection."

    # game/extra/extra.rpy:329
    old "Предыдущий снимок"
    new "Photo précédente"

    # game/extra/extra.rpy:335
    old "Вернуться в коллекцию"
    new "Retour à la collection"

    # game/extra/extra.rpy:340
    old "Следующий снимок"
    new "Photo suivante"

