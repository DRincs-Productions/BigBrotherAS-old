# TODO: Translation updated at 2022-07-09 12:49

# game/events/max.rpy:4
translate french StartDialog_e88dfdc8:

    # Max_10 "{m}Я чувствую себя слишком уставшим для этого. Было бы неплохо сначала вздремнуть и набраться сил...{/m}"
    Max_10 "{m}Je me sens trop fatigué pour ça. Ce serait bien de faire une sieste d'abord et de prendre de la force...{/m}"

# game/events/max.rpy:32
translate french Sleep_52abcf74:

    # Max_00 "{m}Пожалуй, пора ложиться спать...{/m}" nointeract
    Max_00 "{m}Peut-être qu'il est temps d'aller au lit...{/m}" nointeract

# game/events/max.rpy:35
translate french Sleep_9878a7fd:

    # Max_00 "{m}В каком часу мне будет лучше проснуться?{/m}" nointeract
    Max_00 "{m}À quelle heure ferais-je mieux de me réveiller? {/m}" nointeract

# game/events/max.rpy:47
translate french Sleep_06b9714c:

    # Max_19 "{m}Как же в этом доме хорошо...{/m}"
    Max_19 "{m}Comment cette maison est bonne...{/m}"

# game/events/max.rpy:65
translate french Wearied_d8557c8f:

    # Max_10 "{m}Моя голова уже совсем не соображает, нужно ложиться спать...{/m}" nointeract
    Max_10 "{m}Je n'ai pas les idées claires, je dois aller me coucher...{/m}" nointeract

# game/events/max.rpy:67
translate french Wearied_06b9714c:

    # Max_19 "{m}Как же в этом доме хорошо...{/m}"
    Max_19 "{m}Comment cette maison est bonne...{/m}"

# game/events/max.rpy:85
translate french LittleEnergy_d97dcb62:

    # Max_10 "{m}Я слишком вымотался, нужно хоть немного вздремнуть...{/m}" nointeract
    Max_10 "{m}je suis trop épuisé, j’ai besoin de faire une petite sieste... {/m}" nointeract

# game/events/max.rpy:96
translate french LittleEnergy_06b9714c:

    # Max_19 "{m}Как же в этом доме хорошо...{/m}"
    Max_19 "{m}Comment cette maison est bonne...{/m}"

# game/events/max.rpy:116
translate french Nap_9f963ea2:

    # Max_00 "{m}Я сейчас не очень хочу спать, но немного вздремнуть лишним не будет...{/m}" nointeract
    Max_00 "{m}je ne veux pas vraiment dormir maintenant, mais une petite sieste ne sera pas superflue...{/m}" nointeract

# game/events/max.rpy:118
translate french Nap_5e123eb6:

    # Max_00 "{m}Ох и вымотался же я сегодня, надо немного вздремнуть...{/m}" nointeract
    Max_00 "{m}Oh et je suis épuisé aujourd'hui, il faut faire une petite sieste...{/m}" nointeract

# game/events/max.rpy:135
translate french Nap_06b9714c:

    # Max_19 "{m}Как же в этом доме хорошо...{/m}"
    Max_19 "{m}Comment cette maison est bonne...{/m}"

# game/events/max.rpy:144
translate french Alarm_9878a7fd:

    # Max_00 "{m}В каком часу мне будет лучше проснуться?{/m}" nointeract
    Max_00 "{m}À quelle heure ferais-je mieux de me réveiller? {/m}" nointeract

# game/events/max.rpy:157
translate french Alarm_06b9714c:

    # Max_19 "{m}Как же в этом доме хорошо...{/m}"
    Max_19 "{m}Comment cette maison est bonne...{/m}"

# game/events/max.rpy:172
translate french Shower_691d4581:

    # Max_19 "{m}Всё-таки чистым быть намного лучше. Хотя не всегда хочется...{/m}" nointeract
    Max_19 "{m}toujours être propre est beaucoup mieux. Bien que pas toujours envie...{/m}" nointeract

# game/events/max.rpy:178
translate french Shower_8b383cf3:

    # Max_03 "{m}Ага! Я только сейчас обратил внимание на то, что здесь есть ещё заднее окно! И, как мне кажется, через него на ванну откроется просто шикарный вид... Конечно же дело не в самой ванне, а в том, кто будет её принимать.{/m}"
    Max_03 "{m}Aha! Je viens juste de remarquer qu'il y a encore une fenêtre arrière ici! Et, comme il me semble, à travers lui sur la baignoire s'ouvrira tout simplement un look chic... Bien sûr, ce n'est pas dans le bain lui-même, mais dans qui va le prendre.{/m}"

# game/events/max.rpy:179
translate french Shower_d716938a:

    # Max_09 "{m}Только вот расположено оно высоковато... Нужно достать что-то, с чего будет удобно подглядывать и что не вызовет, в случае чего, подозрений...{/m}"
    Max_09 "{m}Seulement ici il est situé haut... Vous devez obtenir quelque chose qui sera confortable à regarder et qui ne causera pas, dans ce cas, des soupçons...{/m}"

# game/events/max.rpy:180
translate french Shower_38c5acb7:

    # Max_01 "{m}Возможно подойдёт лестница, а ещё лучше стремянка. О да, пожалуй, это будет то, что нужно!{/m}"
    Max_01 "{m}peut-être une échelle, ou encore mieux un escabeau. Oh oui, peut-être que ce sera ce qu'il faut!{/m}"

# game/events/max.rpy:195
translate french Bath_691d4581:

    # Max_19 "{m}Всё-таки чистым быть намного лучше. Хотя не всегда хочется...{/m}" nointeract
    Max_19 "{m}toujours être propre est beaucoup mieux. Bien que pas toujours envie...{/m}" nointeract

# game/events/max.rpy:201
translate french Bath_8b383cf3:

    # Max_03 "{m}Ага! Я только сейчас обратил внимание на то, что здесь есть ещё заднее окно! И, как мне кажется, через него на ванну откроется просто шикарный вид... Конечно же дело не в самой ванне, а в том, кто будет её принимать.{/m}"
    Max_03 "{m}Aha! Je viens juste de remarquer qu'il y a encore une fenêtre arrière ici! Et, comme il me semble, à travers lui sur la baignoire s'ouvrira tout simplement un look chic... Bien sûr, ce n'est pas dans le bain lui-même, mais dans qui va le prendre.{/m}"

# game/events/max.rpy:202
translate french Bath_d716938a:

    # Max_09 "{m}Только вот расположено оно высоковато... Нужно достать что-то, с чего будет удобно подглядывать и что не вызовет, в случае чего, подозрений...{/m}"
    Max_09 "{m}Seulement ici il est situé haut... Vous devez obtenir quelque chose qui sera confortable à regarder et qui ne causera pas, dans ce cas, des soupçons...{/m}"

# game/events/max.rpy:203
translate french Bath_38c5acb7:

    # Max_01 "{m}Возможно подойдёт лестница, а ещё лучше стремянка. О да, пожалуй, это будет то, что нужно!{/m}"
    Max_01 "{m}peut-être une échelle, ou encore mieux un escabeau. Oh oui, peut-être que ce sera ce qu'il faut!{/m}"

# game/events/max.rpy:216
translate french Box_97aab2e1:

    # Max_08 "{m}Так, мама попросила разобрать коробки. Сейчас глянем, что тут у нас...{/m}"
    Max_08 "{m}donc, ma mère m'a demandé de démonter les boîtes. Etudions maintenant que nous avons...{/m}"

# game/events/max.rpy:218
translate french Box_53491d2a:

    # Max_09 "{m}Жаль, но все коробки пустые... Но что это такое? Какая-то камера?{/m}"
    Max_09 "{m}Désolé, mais toutes les boîtes sont vides... Mais qu'est-ce? Une caméra?{/m}"

# game/events/max.rpy:220
translate french Box_333c951b:

    # Max_01 "{m}Тут внутри какая-то инструкция, описание... Да это скрытая камера! Любопытно, зачем она понадобилась отцу?{/m}"
    Max_01 "{m}il y a une instruction, une Description à l'intérieur... C'est une caméra cachée! Je me demandais pourquoi mon père en avait besoin.{/m}"

# game/events/max.rpy:224
translate french Box_911e2e27:

    # Max_10 "{m}Может быть, она установлена где-то в доме и за нами кто-то наблюдает?! Нужно будет осмотреть дом...{/m}" nointeract
    Max_10 "{m}peut-être qu'elle est installée quelque part dans la maison et que quelqu'un nous surveille?! Il faudra inspecter la maison...{/m}" nointeract

# game/events/max.rpy:250
translate french Notebook_299609c3:

    # Max_00 "{m}Итак, чем интересным я займусь?{/m}"
    Max_00 "{m}alors, qu'est-ce qui m'intéresse?{/m}"

# game/events/max.rpy:311
translate french nothing_search_841c64c4:

    # Max_00 "{m}Сейчас мне нечего искать...{/m}"
    Max_00 "{m}maintenant, je n'ai rien à chercher...{/m}"

# game/events/max.rpy:319
translate french buyfood_14a9f290:

    # Max_04 "{m}Так... Посмотрим список продуктов... Ага. Сейчас всё закажем...{/m}"
    Max_04 "{m}Ainsi... Voyons la liste des produits... Ouais. On va tout commander...{/m}"

# game/events/max.rpy:321
translate french buyfood_da7a3eda:

    # Max_04 "{m}Готово. Да это же самая лёгкая задача!{/m}"
    Max_04 "{m}Terminer. Oui, c'est la tâche la plus facile!{/m}"

# game/events/max.rpy:324
translate french buyfood_31efbd83:

    # Max_01 "{m}Готово. То, что я делаю это без маминой финансовой помощи точно пойдёт мне только в плюс.{/m}"
    Max_01 "{m}Terminer. Le fait que je le fasse sans l'aide financière de ma mère ne me fera que gagner.{/m}"

# game/events/max.rpy:358
translate french create_site_c3e958a9:

    # Max_00 "{m}Итак, пришло время заняться своим сайтом. Для начала нужно купить домен, хостинг, шаблон дизайна и оплатить услуги стримингового сервиса. На всё в сумме нужно $100{/m}" nointeract
    Max_00 "{m}donc, il est temps de s'attaquer à votre site. Pour commencer, vous devez acheter un domaine, un hébergement, un modèle de conception et payer pour les services de streaming. Pour tout le montant nécessaire $ 100{/m}" nointeract

# game/events/max.rpy:365
translate french create_site_4c292724:

    # Max_04 "{m}Отлично! Теперь у меня есть свой сайт и домен! Осталось только соединить поток данных от камеры со стриминговым сервисом...{/m}" nointeract
    Max_04 "{m}Bien! Maintenant, j'ai mon site et mon domaine! Il ne reste plus qu'à connecter le flux de données de la caméra au service de streaming...{/m}" nointeract

# game/events/max.rpy:369
translate french create_site_2b30be60:

    # Max_04 "{m}Да! Всё работает! Теперь люди смогут заходить на мой сайт и смотреть шоу. Конечно, если они каким-то образом узнают про мой сайт... Ладно, подумаю ещё что можно сделать...{/m}"
    Max_04 "{m}Oui! Tout fonctionne! Maintenant, les gens pourront aller sur mon site et regarder l'émission. Bien sûr, s'ils découvrent mon site... Ok, vous pensez encore que vous pouvez faire...{/m}"

# game/events/max.rpy:404
translate french about_cam_42f3e467:

    # Max_09 "{m}Так, любопытно... Эти камеры можно настроить так, чтобы они транслировали изображение в интернет!{/m}"
    Max_09 "{m}donc, curieux... Ces caméras peuvent être configurées pour diffuser l'image sur Internet!{/m}"

# game/events/max.rpy:405
translate french about_cam_c6823e2f:

    # Max_07 "{m}Но что ещё интереснее, некоторые люди готовы платить за доступ к таким камерам...{/m}"
    Max_07 "{m}mais plus intéressant encore, certaines personnes sont prêtes à payer pour avoir accès à de telles caméras...{/m}"

# game/events/max.rpy:406
translate french about_cam_8ca532b0:

    # Max_09 "{m}Может быть, мне сделать свой сайт и пусть люди мне платят за просмотр видео? Но я не умею ничего толком...{/m}"
    Max_09 "{m}peut-être que je fais mon site et laisser les gens me payer pour regarder des vidéos? Mais je ne sais rien faire...{/m}"

# game/events/max.rpy:419
translate french about_blog_ea375606:

    # Max_00 "{m}Итак, попробуем что-то найти о блогах. С чего начать?{/m}" nointeract
    Max_00 "{m}alors, essayons de trouver quelque chose sur les blogs. Par où commencer?{/m}" nointeract

# game/events/max.rpy:422
translate french about_blog_c3072649:

    # Max_10 "{m}Ох... Сколько цифр. Неужели, во всём этом можно разобраться?{/m}" nointeract
    Max_10 "{m}Oh... Combien de chiffres. Est-ce que tout cela peut être compris?{/m}" nointeract

# game/events/max.rpy:431
translate french about_blog_eaab3fbf:

    # Max_04 "{m}Прикольно... Ага. Котики. Не, устарели. Киски. Ну, смотря какие...{/m}" nointeract
    Max_04 "{m}Cool... Ouais. Minets. Non, obsolète. Chattes. Ça dépend de quoi...{/m}" nointeract

# game/events/max.rpy:440
translate french about_blog_f7287104:

    # Max_14 "{m}Ух. Сколько гадости в комментах... Ладно, попробуем найти в этом крупицу смысла...{/m}" nointeract
    Max_14 "{m}Oreille. Quelle méchanceté dans les commentaires... Essayons de trouver un peu de sens...{/m}" nointeract

# game/events/max.rpy:449
translate french about_blog_2d9761c1:

    # Max_09 "{m}[_text!t]{/m}" nointeract
    Max_09 "{m}[_text!t]{/m}" nointeract

# game/events/max.rpy:451
translate french about_blog_116ee643:

    # Max_01 "{m}Это что же получается. Неужели, всё так просто? Главное - сиськи! Не важно о чём блог, важно что на экране. И если там сиськи, всё в порядке с популярностью! Но я и так об этом догадывался...{/m}"
    Max_01 "{m}C'est ce qui se passe. Est-ce que, tout simplement? La chose principale est les seins! Peu importe ce que le blog, ce qui est important à l'écran. Et s'il y a des seins là-bas, tout va bien avec la popularité! Mais je, et, comme le pense...{/m}"

# game/events/max.rpy:452
translate french about_blog_b55fbee5:

    # Max_00 "{m}И зачем я только что-то изучал...{/m}"
    Max_00 "{m}et pourquoi je viens d'apprendre quelque chose...{/m}"

# game/events/max.rpy:465
translate french about_secretbook_d428da7a:

    # Max_00 "{m}Так... Сейчас погуглим. Как там она называлась? \"Sugar Daddies\"?... Любовный роман? И что в нём такого может быть?{/m}" nointeract
    Max_00 "{m}Ainsi... On va chercher sur Google. Comment s'appelait-elle? \"Sugar Daddies\"?... Une histoire d'amour? Et qu'est-ce qui peut y arriver?{/m}" nointeract

# game/events/max.rpy:467
translate french about_secretbook_331cf11d:

    # Max_06 "{m}Ого! Да это не простой любовный роман... Это же эротика. Да ещё какая! Теперь понятно, почему Алиса не хотела рассказывать, что читает...{/m}"
    Max_06 "{m}Wow! Oui ce n'est pas un simple roman d'amour... C'est érotique. Oui encore quelle! Maintenant, il est clair pourquoi Alice ne voulait pas dire ce qu'elle lisait...{/m}"

# game/events/max.rpy:481
translate french about_spider_c9502343:

    # Max_00 "{m}Так... Пауки. Ох, сколько видов... Какие же тут водятся...{/m}" nointeract
    Max_00 "{m}Ainsi... Araignées. Oh, combien d'espèces... Quelles sont les mêmes ici se trouvent les...{/m}" nointeract

# game/events/max.rpy:483
translate french about_spider_1c3432cf:

    # Max_00 "{m}Ага, отлично. Выбираем наш регион, сортируем по видам пауков...{/m}"
    Max_00 "{m}ouais, bien. Nous choisissons notre région, trions par type d'araignées...{/m}"

# game/events/max.rpy:485
translate french about_spider_d718f75f:

    # Max_00 "{m}Так, пауки. Смотрим какие водятся в этом климате...{/m}"
    Max_00 "{m}Ok, araignées. Nous regardons ce qui se trouve dans ce climat...{/m}"

# game/events/max.rpy:486
translate french about_spider_06795649:

    # Max_00 "{m}Так, и что у нас получается?{/m}"
    Max_00 "{m}alors, qu'est-ce qu'on fait?{/m}"

# game/events/max.rpy:488
translate french about_spider_825e59bc:

    # Max_01 "{m}Ну вот, подходит. Самый популярный паук в наших краях. Ага! Теперь узнаем как его поймать...{/m}" nointeract
    Max_01 "{m}eh Bien, parfait. L'araignée la plus populaire de notre région. Ah!ah! Maintenant, nous allons savoir comment l'attraper...{/m}" nointeract

# game/events/max.rpy:491
translate french about_spider_43ff8442:

    # Max_09 "{m}Так, питается комарами и мошками. Как это обычно... нет, это ничего не даёт...{/m}" nointeract
    Max_09 "{m}donc, se nourrit de moustiques et de moucherons. Comme c'est habituellement... non, ça ne donne rien...{/m}" nointeract

# game/events/max.rpy:496
translate french about_spider_313f4e4c:

    # Max_04 "{m}Вот, отлично! Ночью отлично маскируются, значит, не подходит, а вот как только солнце начинает прогревать землю, выползают из травы проверить добычу. А это у нас часов 10-11?{/m}"
    Max_04 "{m}Voici, excellent! La nuit, ils se déguisent parfaitement, ce qui signifie qu'ils ne conviennent pas, mais dès que le soleil commence à réchauffer la terre, ils rampent hors de l'herbe. Est-ce qu'on a 10 ou 11 heures?{/m}"

# game/events/max.rpy:497
translate french about_spider_1b3b8906:

    # Max_01 "{m}Будем искать!{/m}"
    Max_01 "{m}Allons cherchons!{/m}"

# game/events/max.rpy:513
translate french SearchCam_29b2ce2f:

    # Max_04 "{m}Ого! Вот же она! Кто-то её так хорошо запрятал в стену, что найти камеру можно только точно зная, что ищешь...{/m}"
    Max_04 "{m}Wow! Voici la même il est! Quelqu'un l'a si bien cachée dans le mur que vous ne pouvez trouver la caméra que si vous savez exactement ce que vous cherchez...{/m}"

# game/events/max.rpy:514
translate french SearchCam_6eab0c74:

    # Max_09 "{m}Так... Но она ни к чему не подключена сейчас. Видимо, отец так следил за ходом строительства и ремонта, а сейчас уже некому следить и не за чем...{/m}"
    Max_09 "{m}Ainsi... Mais elle n'est connectée à rien maintenant. Apparemment, mon père a suivi le progrès de la construction et de la réparation, et maintenant il n'y a personne à surveiller et rien...{/m}"

# game/events/max.rpy:515
translate french SearchCam_7c5a1bd7:

    # Max_04 "{m}Но если её подключить, то можно подглядывать и за кое-чем другим. Вот только нужно во всём как следует разобраться!{/m}"
    Max_04 "{m}Mais si vous le branchez, vous pouvez regarder autre chose. C'est juste que vous devez comprendre tout correctement!{/m}"

# game/events/max.rpy:523
translate french SearchCam_73e63b9a:

    # Max_14 "{m}Кажется, здесь нет никаких камер... Нужно поискать в самом доме!{/m}"
    Max_14 "{m}il ne Semble pas y avoir de caméras ici... Nous devons chercher dans la maison elle-même!{/m}"

# game/events/max.rpy:525
translate french SearchCam_74aa4ac8:

    # Max_14 "{m}Кажется, здесь нет никаких камер... Может быть, стоит поискать в другой комнате?{/m}"
    Max_14 "{m}il ne Semble pas y avoir de caméras ici... Peut-être que vous devriez chercher dans une autre pièce?{/m}"

# game/events/max.rpy:537
translate french ClearPool_f95b5748:

    # Max_11 "{m}Эх... Не лёгкая это работа, но нужно отработать те деньги, что мама уже заплатила...{/m}"
    Max_11 "{m}Eh... Ce n'est pas un travail facile, mais vous devez travailler l'argent que votre mère a déjà payé...{/m}"

# game/events/max.rpy:540
translate french ClearPool_26cb581b:

    # Max_01 "{m}Эх... Работа нудная, но важно, чтобы мои девочки плескались в чистой водичке. И теперь, я слежу за этим сам.{/m}"
    Max_01 "{m}Eh... Le travail est fastidieux, mais il est important que mes filles éclaboussent dans de l'eau propre. Et maintenant, je le suis moi-même.{/m}"

# game/events/max.rpy:560
translate french DishesWashed_62001d7c:

    # Max_00 "{m}Эх... столько посуды. И почему в этом огромном доме нет маленькой посудомоечной машины?{/m}" nointeract
    Max_00 "{m}Eh... beaucoup de vaisselle. Et pourquoi n'y a-t-il pas un petit lave-vaisselle dans cette immense maison?{/m}" nointeract

# game/events/max.rpy:586
translate french delivery1_f0fc4226:

    # Max_07 "{m}Звонок в ворота! Похоже, к нам кто-то приехал...{/m}"
    Max_07 "{m}appel à la porte! On dirait que quelqu'un est venu...{/m}"

# game/events/max.rpy:589
translate french delivery1_80685efe:

    # Sam_00 "Здравствуйте! По этому адресу на сегодня назначена доставка. Распишитесь?"
    Sam_00 "Bonjour ! Une livraison est prévue pour aujourd'hui à cette adresse. Vous signez ?"

# game/events/max.rpy:590
translate french delivery1_509c6af4:

    # Max_00 "Конечно! А что тут?"
    Max_00 "Bien sûr ! Qu'en est-il ?"

# game/events/max.rpy:592
translate french delivery1_0ba1be34:

    # Max_00 "Да, то что нужно. Спасибо!"
    Max_00 "Oui, c'est ce dont j'avais besoin. Merci !"

# game/events/max.rpy:619
translate french delivery2_f0fc4226:

    # Max_07 "{m}Звонок в ворота! Похоже, к нам кто-то приехал...{/m}"
    Max_07 "{m}appel à la porte! On dirait que quelqu'un est venu...{/m}"

# game/events/max.rpy:623
translate french delivery2_60d5c126:

    # Christine_00 "Здравствуйте! По этому адресу на сегодня назначена доставка. Распишитесь?"
    Christine_00 "Bonjour ! Une livraison est prévue pour aujourd'hui à cette adresse. Vous signez ?"

# game/events/max.rpy:624
translate french delivery2_509c6af4:

    # Max_00 "Конечно! А что тут?"
    Max_00 "Bien sûr ! Qu'en est-il ?"

# game/events/max.rpy:626
translate french delivery2_0ba1be34:

    # Max_00 "Да, то что нужно. Спасибо!"
    Max_00 "Oui, c'est ce dont j'avais besoin. Merci !"

# game/events/max.rpy:636
translate french delivery2_885c0cfc:

    # Max_02 "{m}Боди у меня! Теперь, нужно подарить его Алисе и больше всего мне может повезти, когда она занимается своим блогом. Она и так в это время в нижнем белье, а с учётом того, что она получит боди раньше времени, то вполне может переодеться и при мне...{/m}"
    Max_02 "{m}J'ai le body! Maintenant, je dois le donner à Alice et le mieux, c'est quand elle fait son blog. Elle est déjà en sous-vêtements à cette heure là, et comme elle a un body, autant qu'elle se change devant moi...{/m}"

# game/events/max.rpy:638
translate french delivery2_1f5e97ce:

    # Max_10 "{m}Боди у меня! Вот только поздно... Эрик уже купил Алисе то, что она хотела. Остаётся лишь выставить на ebay, так хотя бы половину стоимости верну.{/m}"
    Max_10 "{m}J'ai le body! Sauf que c'est trop tard... Eric a déjà acheté à Alice ce qu'elle voulait. La seule chose qu'il reste à faire est de le mettre sur ebay, ce qui permettra de récupérer au moins la moitié du coût.{/m}"

# game/events/max.rpy:649
translate french BookRead_9562ffab:

    # Max_00 "Пришло время почитать что-то..." nointeract
    Max_00 "Il est temps de lire quelque chose..." nointeract

# game/events/max.rpy:658
translate french BookRead_manual_41be558e:

    # Max_00 "Хм... куча непонятных слов. Кажется, нужно будет заново перечитать первые главы...\n\n{color=[orange]}{i}(Книга изучена на 20%%){/i}{/color}"
    Max_00 "Hmm... beaucoup de mots incompréhensibles. Il va falloir relire les premiers chapitres...\n\n{color=[orange]}{i}(Livre étudié à 20%%){/i}{/color}"

# game/events/max.rpy:660
translate french BookRead_manual_0826f3fe:

    # Max_00 "Так, ну с этим я уже разобрался, хорошо... А это что такое? Не ясно. Нужно будет всё осмыслить...\n\n{color=[orange]}{i}(Книга изучена на 40%%){/i}{/color}"
    Max_00 "Bon, je l'ai déjà compris, d'accord... C'est quoi, ça? Pas clair. Il faudra tout comprendre...\n\n{color =[orange]}{i}(Livre étudié à 40%%) {/i} {/color}"

# game/events/max.rpy:662
translate french BookRead_manual_3605e1d8:

    # Max_00 "Ого, вот это здорово! Уже можно делать сайт? А, нет... Ещё не всё понятно... Ну, разберусь в другой раз.\n\n{color=[orange]}{i}(Книга изучена на 60%%){/i}{/color}"
    Max_00 "Wow, c'est génial! Est-il déjà possible de faire un site Web? Oh, non... Tout n'est pas encore clair... Je m'en occuperai une autre fois.\n\n{color =[orange]}{i}(Livre étudié à 60%%) {/i} {/color}"

# game/events/max.rpy:664
translate french BookRead_manual_1c08c83d:

    # Max_00 "Так, ну теперь картина вырисовывается. Осталось разобраться только с мелочами... Или это не мелочи?\n\n{color=[orange]}{i}(Книга изучена на 80%%){/i}{/color}"
    Max_00 "Eh bien, maintenant l'image se dessine. Il ne reste plus qu'à gérer les petites choses... Ou ce n'est pas des petites choses?\n\n{color =[orange]}{i}(Livre étudié à 80%%) {/i} {/color}"

# game/events/max.rpy:666
translate french BookRead_manual_bd8ad388:

    # Max_00 "Всё, вот теперь точно всё понятно! Я уже могу сделать свой сайт и транслировать на него изображение! Но как получать за это деньги?"
    Max_00 "C'est ça, maintenant tout est parfaitement clair! Je peux déjà créer mon propre site web et y diffuser une image! Mais comment être payé pour ça?"

# game/events/max.rpy:678
translate french BookRead_sex_ed_e2e2e4d9:

    # Max_01 "Ага. У каждого есть свои особенности, а то я не знал! Вот, строение половых органов девочки-подростка, то что надо... Будем читать и разглядывать.\n\n{color=[orange]}{i}(Книга изучена на 25%%){/i}{/color}"
    Max_01 ". Tout le monde a ses propres caractéristiques, sinon je ne savais pas! Voici, la structure des organes génitaux d'une adolescente, ce qui est nécessaire... On va lire et regarder.\n\n{color =[orange]}{i}(Livre étudié à 25%%) {/i} {/color}"

# game/events/max.rpy:680
translate french BookRead_sex_ed_5dc0d1da:

    # Max_03 "Так, это не особо интересно... А вот сексуальное поведение подростков - это как раз про меня! Ещё про мои утренние стояки написали бы, было бы вообще супер...\n\n{color=[orange]}{i}(Книга изучена на 50%%){/i}{/color}"
    Max_03 "Donc, ce n'est pas particulièrement intéressant... Mais le comportement sexuel des adolescents est juste à propos de moi! Encore à propos de mes érections du matin écriraient, ce serait généralement super...\n\n{color =[orange]}{i}(Livre étudié à 50%%) {/i} {/color}"

# game/events/max.rpy:682
translate french BookRead_sex_ed_cdf53c7a:

    # Max_07 "Ого, здесь даже есть краткий исторический очерк о сексуальном воспитании детей и подростков... Как только голову не дурили за всё это время!\n\n{color=[orange]}{i}(Книга изучена на 75%%){/i}{/color}"
    Max_07 "Wow, il y a même un bref essai historique sur l'éducation sexuelle des enfants et des adolescents... Une fois que la tête n'a pas été dupé pendant tout ce temps!\n\n{color =[orange]}{i}(Livre étudié à 75%%) {/i} {/color}"

# game/events/max.rpy:687
translate french BookRead_sex_ed_58528ae0:

    # Max_04 "Вот и последние главы... Всё-таки прикосновения очень важны! Да я и на практике уже это понял... Эх, надо было раньше эту книжку купить! Но лучше поздно, чем никогда. Материал усвоен и теперь можно дарить её Лизе."
    Max_04 "Voici les derniers chapitres... Le toucher est très important après tout! J'ai appris cela dans la pratique, aussi... Eh, j'aurais dû acheter ce livre plus tôt! Mais mieux vaut tard que jamais. La matière est apprise et maintenant tu peux pratiquer avec Lisa."

# game/events/max.rpy:689
translate french BookRead_sex_ed_37326ac9:

    # Max_04 "Вот и последние главы... Кажется, я понял о чём говорила Лиза. Эх, надо было раньше эту книжку прочитать, сейчас бы не тратил на это время! Но лучше поздно, чем никогда. Материал усвоен и теперь можно снова приставать к Лизе."
    Max_04 "Voici les derniers chapitres... Je crois avoir compris de quoi parlait Lisa. Eh, j'aurais dû lire ce livre avant, maintenant je n'aurais pas perdu de temps! Mais mieux vaut tard que jamais. Le matériel est appris et maintenant vous pouvez à nouveau harceler Lisa."

# game/events/max.rpy:703
translate french SearchSecretBook_4d6b282e:

    # Max_10 "{m}Так... И с чего начать поиск? И нужно поспешить: если Алиса меня поймает, то сначала убьёт, а только потом поздоровается...{/m}" nointeract
    Max_10 "{m}Ainsi... Et par où commencer la recherche? Et il faut se dépêcher: si Alice m'attrape, elle me tue d'abord, puis elle dit bonjour...{/m}" nointeract

# game/events/max.rpy:718
translate french SearchSecretBook_pillow_9214580d:

    # Max_14 "{m}Нет, здесь её нет... Ну где же эта чёртова книга? Шаги? Нет, показалось...{/m}" nointeract
    Max_14 "{m}Non, elle n'est pas là... Où est ce putain de livre? Pas? Non, il a semblé...{/m}" nointeract

# game/events/max.rpy:731
translate french SearchSecretBook_bed_fdbef300:

    # Max_14 "{m}Нет, тут её точно нет. Ну где же она? Кажется, я слышу шум...{/m}" nointeract
    Max_14 "{m}Non, elle n'est pas là. Eh bien, où est-elle? Je crois que j'entends un bruit...{/m}" nointeract

# game/events/max.rpy:745
translate french SearchSecretBook_table_1c89acaf:

    # Max_14 "{m}Может быть, её здесь нет? Или попытаться ещё поискать, на свой страх и риск?{/m}" nointeract
    Max_14 "{m}Peut-être qu'elle n'est pas là? Ou essayer de chercher encore, à vos risques et périls?{/m}" nointeract

# game/events/max.rpy:761
translate french SearchSecretBook_wardrobe_b73b38d5:

    # Max_04 "{m}Вот же она! И зачем её так прятать? Любопытная обложка. Запомню-ка я название. Интересно, о чём эта книга? Может быть, погуглить? Так, всё, надо уходить...{/m}"
    Max_04 "{m}la Voilà! Pourquoi la cacher comme ça? Curieuse couverture. Je me souviendrai du nom. Je me demande de quoi parle ce livre? Peut-être googler? Ok, tout le monde, il faut partir...{/m}"

# game/events/max.rpy:772
translate french InstallCam_90e45062:

    # Max_00 "{m}Здесь уже есть камера. Пожалуй, стоит установить её в другом месте.{/m}"
    Max_00 "{m}il y a déjà une caméra ici. Peut-être que vous devriez l'installer ailleurs.{/m}"

# game/events/max.rpy:780
translate french InstallCam_44699dcc:

    # Max_04 "{m}В этой комнате столько всего может происходить... Думаю, зрителям понравится! Главное - спрятать все провода, чтобы Лиза не заметила новую микро-камеру...{/m}" nointeract
    Max_04 "{m}Tant de choses peuvent se passer dans cette pièce... Je pense que le public va adorer ! L'essentiel est de cacher tous les fils pour que Lisa ne remarque pas la nouvelle micro-caméra....{/m}" nointeract

# game/events/max.rpy:786
translate french InstallCam_b9324e31:

    # Max_04 "{m}Пусть зрители посмотрят, чем Алиса занимается в своей комнате, когда её не видят... Я бы и сам был бы рад посмотреть, но пока такой функции у меня нет...{/m}" nointeract
    Max_04 "{m}Laissez le public voir ce qu'Alice fait dans sa chambre quand on ne la voit pas... J'aimerais le voir moi-même, mais je n'ai pas encore cette fonction...{/m}" nointeract

# game/events/max.rpy:792
translate french InstallCam_9d11ed16:

    # Max_04 "{m}Конечно, здесь редко происходят события. Зато, когда они происходят, то здесь такое... Думаю, зрители будут рады таким моментам...{/m}" nointeract
    Max_04 "{m}Bien sûr, les événements se produisent rarement ici. Mais quand ils le font, c'est... Je pense que les téléspectateurs apprécieront ces moments...{/m}" nointeract

# game/events/max.rpy:799
translate french InstallCam_bc72cb94:

    # Max_03 "{m}Теперь через обе камеры можно увидеть всё самое интересное! Девочки любят покрасоваться у зеркала, а моя мама и Эрик, как я заметил, стараются не упустить возможность потрахаться перед этим же зеркалом... Моим зрителям это явно понравится!{/m}" nointeract
    Max_03 "{m}Maintenant, je peux voir tout ce qui est intéressant à travers les deux caméras! Les filles aiment s'exhiber devant le miroir, et ma mère et Eric, je l'ai remarqué, ne manquent pas l'occasion de baiser devant le même miroir... Mes téléspectateurs vont clairement adorer !{/m}" nointeract

# game/events/max.rpy:805
translate french InstallCam_3a6027ee:

    # Max_04 "{m}Конечно, с точки зрения морали ставить камеру в ванной сомнительно. Однако, тут и так окно во всю стену. Так что, формально я лишь приоткрыл это окно...{/m}" nointeract
    Max_04 "{m}Bien sûr, du point de vue de la moralité, mettre une caméra dans la salle de bain est douteux. Cependant, il y a déjà une fenêtre sur tout le mur. Donc, officiellement, j'ai juste ouvert une autre fenêtre...{/m}" nointeract

# game/events/max.rpy:811
translate french InstallCam_918d8b5b:

    # Max_04 "{m}Уж не знаю, будет ли какой-то толк от этой камеры... Тут так редко что-то происходит... Ну пусть будет. Раз уж взялся всё подключать...{/m}" nointeract
    Max_04 "{m}Je ne sais pas si cette caméra va faire quelque chose... Il se passe rarement quelque chose ici... Eh bien, laissons-la. J'ai tout branché....{/m}" nointeract

# game/events/max.rpy:818
translate french InstallCam_3a93939c:

    # Max_04 "{m}Вот теперь зрители смогут насладится всеми мокрыми и блестящими красотами, происходящими во дворе...{/m}" nointeract
    Max_04 "{m}Maintenant, le public pourra profiter de toutes les beautés humides et brillantes qui se prélassent dans la cour...{/m}" nointeract

# game/events/max.rpy:824
translate french InstallCam_3426c2fd:

    # Max_04 "{m}Двор... Тут почти всё время кто-то есть и что-то делает, пока светит солнце. Думаю, тут зрители будут зависать постоянно в надежде увидеть кого-то с голыми сиськами...{/m}" nointeract
    Max_04 "{m}Cour... Ici, presque tout le temps, quelqu'un est là et fait quelque chose pendant que le soleil brille. Je pense que les spectateurs vont traîner dans l'espoir de voir quelqu'un avec des seins nus...{/m}" nointeract

# game/events/max.rpy:826
translate french InstallCam_7711d18e:

    # Max_09 "{m}Пожалуй, из-за большой площади мне стоило бы установить здесь несколько камер, чтобы зрители смогли лучше разглядеть каждую попку, которая тут бывает...{/m}"
    Max_09 "{m}Peut-être, en raison de la grande surface, je devrais installer quelques caméras ici pour que le public puisse mieux voir chaque cul qui se passe ici...{/m}"

# game/events/max.rpy:860
translate french SearchSpider_d37d0a80:

    # Max_00 "{m}Так, нужно хорошенько рассмотреть траву...{/m}" nointeract
    Max_00 "{m}donc, vous devez examiner attentivement l'herbe...{/m}" nointeract

# game/events/max.rpy:865
translate french SearchSpider_1b27d19f:

    # Max_04 "Ага! Попался! Отлично..."
    Max_04 "oui! C’est cool..."

# game/events/max.rpy:869
translate french SearchSpider_f2d13038:

    # Max_00 "{m}Нет, ничего похожего на большого страшного паука тут нет... Может быть, я всех переловил и стоит подождать денёк-другой?{/m}"
    Max_00 "{m}Non, il n'y a rien de tel qu'une grande araignée effrayante... Peut-être que j'ai surpassé tout le monde et que je devrais attendre un jour ou deux?{/m}"

# game/events/max.rpy:882
translate french HideSpider_a2694b69:

    # Max_00 "{m}Я могу не успеть как следует припрятать паука, прежде чем Алиса вернется из ванной.{/m}"
    Max_00 "{m}je n'aurai peut-être pas le temps de cacher l'araignée avant qu'Alice ne revienne de la salle de bain.{/m}"

# game/events/max.rpy:887
translate french HideSpider_ebac98bc:

    # Max_00 "{m}Интересно, что будет, если Алиса заметит паука ночью? Она прибежит за помощью? Вот только этот монстр может сбежать... Так что, чем позже я его спрячу, тем больше шансов на успех...{/m}" nointeract
    Max_00 "{m}je me Demande ce qui se passera si Alice remarque une araignée la nuit? Elle va chercher de l'aide? Seul ce monstre peut s'échapper... Donc, plus je le cache tard, plus j'ai de chances de réussir...{/m}" nointeract

# game/events/max.rpy:890
translate french HideSpider_9502351b:

    # Max_00 "{m}Что ж, будем надеяться, что паук не сбежит до того, как Алиса ляжет спать...{/m}"
    Max_00 "{m}espérons que l'araignée ne s'échappera pas avant qu'Alice ne se couche...{/m}"

# game/events/max.rpy:933
translate french ViewLesson_f9eae862:

    # Max_00 "{m}Хорошая штука эти онлайн-курсы - можно научиться всему, не входя из дома! Вот только и стоит это немало...{/m}"
    Max_00 "{m}Bonne chose ces cours en ligne - je peux tout apprendre sans quitter la maison! Cela en vaut la peine... {/m}"

# game/events/max.rpy:940
translate french SearchCigarettes_7d1de5a5:

    # Max_09 "{m}Так... Где же Алиса спрятала сигареты сегодня?{/m}"
    Max_09 "{m}Ainsi... Où Alice a-t-elle caché ses cigarettes aujourd'hui?{/m}"

# game/events/max.rpy:959
translate french SearchCigarettes_no_025a648e:

    # Max_10 "{m}Кажется, здесь их нет... Пора уходить, а то если кто-то заметит меня...{/m}" nointeract
    Max_10 "{m}il Ne semble pas y en avoir ici... Il est temps de partir, ou si quelqu'un me remarque...{/m}" nointeract

# game/events/max.rpy:967
translate french SearchCigarettes_yes_f44440e9:

    # Max_04 "{m}Ага, нашёл! Так... Теперь их нужно положить таким образом, чтобы мама их заметила, если заглянет в комнату...{/m}" nointeract
    Max_04 "{m}oui, je l'ai trouvé! Donc... Maintenant, ils doivent être placés de manière à ce que ma mère les remarque s'il regarde dans la pièce...{/m}" nointeract

# game/events/max.rpy:985
translate french need_money_7d60b08e:

    # Max_10 "{m}Сегодня уже четверг. Последний день когда я могу заказать подарки девчонкам, чтобы опередить Эрика.{/m}"
    Max_10 "{m}aujourd'hui est jeudi. Le dernier jour où je peux commander des cadeaux aux filles pour devancer Eric.{/m}"

# game/events/max.rpy:992
translate french need_money_d5c0c6aa:

    # Max_11 "{m}Нужно скорее купить платье Алисе и купальник для Лизы, а денег у меня хватает лишь на что-то одно...{/m}"
    Max_11 "{m}vous devez plutôt acheter une robe Alice et un maillot de bain pour Lisa, et je n'ai assez d'argent que pour quelque chose...{/m}"

# game/events/max.rpy:994
translate french need_money_b39c15a3:

    # Max_11 "{m}Нужно скорее купить платье Алисе и купальник для Лизы, а денег мне не хватит даже на что-то одно...{/m}"
    Max_11 "{m}vous devez plutôt acheter une robe Alice et un maillot de bain pour Lisa, et je n'ai même pas assez d'argent pour quelque chose...{/m}"

# game/events/max.rpy:1000
translate french need_money_adfa0cdd:

    # Max_11 "{m}Я уже купил платье Алисе, но на купальник для Лизы мне не хватит денег...{/m}"
    Max_11 "{m}j'ai déjà acheté une robe à Alice, mais je n'ai pas assez d'argent pour un maillot de bain pour Lisa...{/m}"

# game/events/max.rpy:1006
translate french need_money_ae118bc4:

    # Max_11 "{m}Я уже купил купальник для Лизы, но на платье Алисе мне не хватит денег...{/m}"
    Max_11 "{m}j'ai déjà acheté un maillot de bain pour Lisa, mais je n'ai pas assez d'argent pour la robe d'Alice...{/m}"

# game/events/max.rpy:1011
translate french need_money_aa38a4e7:

    # Max_08 "{m}Сайт у меня есть и уже приносит какую-то прибыль, но нужно время, чтобы раскрутить его.{/m}"
    Max_08 "{m}le Site que j'ai et apporte déjà un certain profit, mais il faut du temps pour le promouvoir.{/m}"

# game/events/max.rpy:1013
translate french need_money_6eaa0c79:

    # Max_08 "{m}К сожалению, у меня нет источника доходов, все мои деньги я получаю только от мамы. Ну и от Алисы ещё могу...{/m}"
    Max_08 "{m}malheureusement, je n'ai pas de source de revenus, tout mon argent je ne reçois que de ma mère. Eh bien, je peux encore d'Alice...{/m}"

# game/events/max.rpy:1014
translate french need_money_14f2a995:

    # Max_09 "{m}Нужно поискать какую-нибудь информацию в интернете, может есть возможность получить кредит.{/m}"
    Max_09 "{m}vous Devez rechercher des informations sur Internet, il peut y avoir une possibilité d'obtenir un prêt.{/m}"

# game/events/max.rpy:1034
translate french cheat_money_46383d7e:

    # "На данном этапе игры у Макса не может быть такой суммы. Взлом игры может привести к непредсказуемым последствиям, в частности, к потере некоторых возможностей и функционала игры, а так же к возникновению критических ошибок, которые не позволят вам продолжить игру."
    "A ce stade du jeu, Max ne peut pas avoir ce montant. Le piratage peut entraîner des conséquences imprévisibles, telles que la perte de certaines caractéristiques et fonctionnalités, ainsi que des erreurs critiques qui vous empêcheront de continuer à jouer au jeu."

# game/events/max.rpy:1038
translate french cheat_money_strateg_16b9bde6:

    # "Вы либо аккуратный взломщик, либо хороший стратег. В любом случае вы не нуждаетесь в дополнительных методах получения денег. Учтите, взлом игры может привести к непредсказуемым последствиям, в частности, к потере некоторых возможностей и функционала игры, а так же к возникновению критических ошибок, которые не позволят вам продолжить игру."
    "Vous êtes soit un hacker prudent, soit un bon stratège. Dans les deux cas, vous n'avez pas besoin de méthodes supplémentaires pour obtenir de l'argent. Sachez que le piratage peut avoir des conséquences imprévisibles, telles que la perte de certaines caractéristiques et fonctionnalités du jeu, ainsi que des erreurs critiques qui vous empêcheront de poursuivre le jeu."

# game/events/max.rpy:1046
translate french about_credit_3ac552dc:

    # Max_00 "{m}Итак, поищем, где можно простому парню разжиться деньгами...{/m}"
    Max_00 "{m}donc, nous allons chercher où un gars simple peut obtenir de l'argent...{/m}"

# game/events/max.rpy:1047
translate french about_credit_aa91eb63:

    # Max_09 "{m}Это не подходит... Здесь нужно иметь официальное трудоустройство на работе...{/m}"
    Max_09 "{m}Ce n'est pas approprié... Ici, vous devez avoir un emploi officiel au travail...{/m}"

# game/events/max.rpy:1048
translate french about_credit_2eacd86e:

    # Max_07 "{m}Ага, а вот это может и подойти - краткосрочные займы начинающим интернет-предпринимателям. О, да, это про меня...{/m}"
    Max_07 "{m}oui, mais cela peut convenir-prêts à court terme pour les entrepreneurs Internet en herbe. Oh, oui, c'est à propos de moi...{/m}"

# game/events/max.rpy:1049
translate french about_credit_90bb447c:

    # Max_01 "Бла, бла, бла... ... если у Вас есть работающий проект в интернете, мы предоставляем займы на раскрутку Вашего бизнеса..."
    Max_01 "Bla, bla, bla... ...si vous avez un projet Internet qui fonctionne, nous fournissons des prêts pour promouvoir votre entreprise..."

# game/events/max.rpy:1051
translate french about_credit_73692d65:

    # Max_04 "{m}Подытожим условия: \n{b}В течение месяца нужно вернуть всю сумму займа + 10%% \nВ случае не погашения в срок, сумма долга утраивается каждые 30 дней, а с моего сайта будут ежедневно изымать половину прибыли. И занять ещё раз уже не получится...{/b} \n\nЛучше, конечно же, до такого не доводить.{/m}"
    Max_04 "{m}, pour conclure conditions: \n{b}Dans le délai d'un mois vous devez rembourser la totalité du montant du prêt + 10%% \nEn cas de non-remboursement dans les délais, le montant de la dette triples toutes les 30 jours, et avec mon site tous les jours de retirer la moitié des bénéfices. Et prendre une fois de plus ne fonctionnera pas...{/b} \n\nMieux, bien sûr, de ne pas apporter à cela.{/m}"

# game/events/max.rpy:1052
translate french about_credit_5011fb20:

    # Max_05 "{m}Регистрируюсь... Указываю свои реквизиты... Свой сайт в качестве источника дохода... Готово!{/m}"
    Max_05 "{m}m'inscris... Je précise mes coordonnées... Votre site comme une source de revenus... C'est prêt!{/m}"

# game/events/max.rpy:1053
translate french about_credit_ec914392:

    # Max_02 "{m}Теперь я могу взять кредит, если срочно понадобятся деньги. Главное вовремя его погасить, чтобы проблем не было...{/m}"
    Max_02 "{m}Maintenant, je peux prendre un prêt si vous avez un besoin urgent d'argent. L'essentiel est de le rembourser à temps pour qu'il n'y ait pas de problèmes...{/m}"

# game/events/max.rpy:1056
translate french about_credit_dc6f567b:

    # Max_16 "{m}Вот чёрт, а у меня нет никакого проекта! Получается, что денег мне никто не даст.{/m}"
    Max_16 "{m}merde, je n'ai pas de projet! Il s'avère que personne ne me donnera de l'argent.{/m}"

# game/events/max.rpy:1067
translate french getting_load_f43833f1:

    # Max_00 "{m}Сколько мне сейчас нужно занять?{/m}" nointeract
    Max_00 "{m}Combien dois-je emprunter maintenant?{/m}" nointeract

# game/events/max.rpy:1087
translate french return_part_loan_67582e30:

    # Max_00 "{m}Сколько я верну сейчас?{/m}" nointeract
    Max_00 "{m}combien vais-je récupérer maintenant?{/m}" nointeract

# game/events/max.rpy:1112
translate french bedtime_thoughts_a9917f30:

    # Max_07 "{m}Как же напакостить Эрику, чтобы он хотя бы на время перестал лезть к моим сёстрам... и Кире... да и к маме тоже... Хм... У меня была бы отличная возможность что-нибудь подмешать в его еду за ужином, если помочь Алисе накрыть на стол! Стоит посмотреть в интернет-магазине, можно ли что-то такое купить...{/m}"
    Max_07 "{m}Comment faire pour qu'Eric arrête d'embêter mes sœurs, au moins pour un moment... et Kira... et ma mère aussi... Hmmm... J'aurais une belle occasion de glisser quelque chose dans sa nourriture au dîner si j'aidais Alice à mettre la table ! Cela vaut la peine de regarder la boutique en ligne pour voir si je peux acheter quelque chose comme ça....{/m}"

# game/events/max.rpy:1147
translate french SearchPhone_no_9a51253a:

    # Max_10 "{m}Здесь телефона Лизы нет.{/m}"
    Max_10 "{m}le téléphone de Lisa n'Est pas là.{/m}"

# game/events/max.rpy:1155
translate french SearchPhone_yes_48a26421:

    # Max_07 "{m}А вот и телефон Лизы! Надеюсь, диктофон на нём есть, иначе я влип...{/m}"
    Max_07 "{m}voici le téléphone de Lisa! J'espère qu'il y a un enregistreur sur lui, sinon je suis coincé...{/m}"

# game/events/max.rpy:1157
translate french SearchPhone_yes_075e9758:

    # Max_09 "{m}Да, диктофон есть. Теперь, пока Лиза не вернулась, нужно идти к Эрику и попробовать вывести его на чистую воду...{/m}" nointeract
    Max_09 "{m}Oui, il y a un enregistreur. Maintenant, avant que Lisa ne revienne, il faut aller voir Eric et essayer de le mettre à l'eau claire...{/m}" nointeract

translate french strings:

    # game/events/max.rpy:31
    old "{i}установить будильник{/i}"
    new "{i}mettre une alarme{/i}"

    # game/events/max.rpy:34
    old "{i}в 6 утра{/i}"
    new "{i}à 6 heures du matin{/i}"

    # game/events/max.rpy:34
    old "{i}в 7 утра{/i}"
    new "{i}à 7 heures du matin{/i}"

    # game/events/max.rpy:84
    old "{i}вздремнуть{/i}"
    new "{i}faire une sieste{/i}"

    # game/events/max.rpy:120
    old "{i}подремать пару часов{/i}"
    new "{i}faire une sieste pendant quelques heures{/i}"

    # game/events/max.rpy:120
    old "{i}подремать 3 часа{/i}"
    new "{i}faire une sieste pendant 3 heures{/i}"

    # game/events/max.rpy:120
    old "{i}подремать 4 часа{/i}"
    new "{i}faire une sieste pendant 4 heures{/i}"

    # game/events/max.rpy:120
    old "{i}подремать 5 часов{/i}"
    new "{i}faire une sieste pendant 5 heures{/i}"

    # game/events/max.rpy:120
    old "{i}не-а, может позже...{/i}"
    new "{i}non, peut-être plus tard... {/i}"

    # game/events/max.rpy:278
    old "{i}почитать о камерах{/i}"
    new "{i}Regarder les caméras{/i}"

    # game/events/max.rpy:280
    old "{i}читать о блогах{/i}"
    new "{i}En apprendre plus sur les blogs{/i}"

    # game/events/max.rpy:282
    old "{i}узнать о книге Алисы{/i}"
    new "{i}Découvrir le livre d'Alice{/i}"

    # game/events/max.rpy:284
    old "{i}читать о пауках{/i}"
    new "{i}En apprendre plus sur les araignées{/i}"

    # game/events/max.rpy:286
    old "{i}искать информацию по кредитам{/i}"
    new "{i}recherche d’informations sur les prêts{/i}"

    # game/events/max.rpy:357
    old "Оплатить всё ($100)"
    new "Payer pour tout (100 $)"

    # game/events/max.rpy:364
    old "Настроить работу сайта"
    new "Configurer votre site"

    # game/events/max.rpy:418
    old "Собрать статистику"
    new "Recueillir des statistiques"

    # game/events/max.rpy:418
    old "Просмотреть популярные блоги"
    new "Voir les blogs populaires"

    # game/events/max.rpy:418
    old "Почитать комменты на блогах"
    new "Lire les commentaires sur les blogs"

    # game/events/max.rpy:421
    old "Проанализировать результаты"
    new "Analyser les résultats"

    # game/events/max.rpy:421
    old "Построить таблицу"
    new "Mettre la table"

    # game/events/max.rpy:421
    old "Нарисовать график"
    new "Dessiner un graphique"

    # game/events/max.rpy:424
    old "Хм... Так... Ага. Это сюда запишем, это сюда..."
    new "Hum... Donc... Ouais. On va écrire ça ici..."

    # game/events/max.rpy:426
    old "Так. Из этой таблицы мы делаем вывод. Ага. Вот это значит, что... Нет, не так. Вот. Вроде получилось..."
    new "Donc. De ce tableau, j'en conclus. Ouais. Voici ce qui signifie que... Non, pas vrai. Voici. Voilà..."

    # game/events/max.rpy:428
    old "Рисователь графиков из меня ещё тот. Но попробуем. Так, это шкала популярности, это... Ага. Кажется, всё сходится..."
    new "Je suis le dessinateur. Mais essayons. Donc, c'est l'échelle de popularité, c'est... Ouais. Tout semble converger..."

    # game/events/max.rpy:430
    old "Составить список"
    new "Faire une liste"

    # game/events/max.rpy:430
    old "Отсортировать..."
    new "Trier..."

    # game/events/max.rpy:430
    old "Сравнить количество лайков..."
    new "Comparer le nombre de likes..."

    # game/events/max.rpy:433
    old "Так, вычёркиваем из списка этих, вот этих и тех. Что тут у нас остаётся?"
    new "On raye de la liste ceux-là, ceux-là et ceux-là?"

    # game/events/max.rpy:435
    old "Так, сравниваем аудиторию. Время публикации... Исключаем сомнительный контент..."
    new "Donc, nous comparons le public. Temps de publication... Exclure le contenu douteux..."

    # game/events/max.rpy:437
    old "Сортируем по количеству лайков. Убираем тех, кто с дизлайками больше этого процента..."
    new "Trier par le nombre de likes. Nous enlevons ceux qui ont des aversions plus que ce pourcentage..."

    # game/events/max.rpy:439
    old "Воспользоваться поиском"
    new "Utiliser la recherche"

    # game/events/max.rpy:439
    old "Читать всё подряд"
    new "Tout lire"

    # game/events/max.rpy:439
    old "Выбрать лучшие комменты"
    new "Sélectionner les meilleurs commentaires"

    # game/events/max.rpy:442
    old "Так, в поиске мы видим любопытные результаты. Так, выберем то что нам подходит..."
    new "Ainsi, dans la recherche, je vois des résultats curieux. Je choisirais ce qui me convient..."

    # game/events/max.rpy:444
    old "Ох. Всё оказалось ещё хуже. Кажется, тут нет ничего полезного... Хотя. Думаю, можно сделать даже какой-то вывод..."
    new "Oh. Tout est encore pire. Il n'y a rien d'utile ici... Quoique. Je pense que je peux même tirer une conclusion..."

    # game/events/max.rpy:446
    old "Так, топовые комменты все сводятся к одному. Не может быть?"
    new "Donc, les principaux commentaires se résument tous à un. Pas possible?"

    # game/events/max.rpy:448
    old "Сделать вывод..."
    new "Déduire..."

    # game/events/max.rpy:464
    old "{i}читать о книге{/i}"
    new "{i}En apprendre plus sur le livre{/i}"

    # game/events/max.rpy:480
    old "Искать виды насекомых по регионам"
    new "Rechercher des espèces d'insectes par région"

    # game/events/max.rpy:480
    old "Читать где водятся какие пауки"
    new "Lire où sont les araignées"

    # game/events/max.rpy:487
    old "Выяснить, чем питается..."
    new "Découvrir ce qu’elle mange..."

    # game/events/max.rpy:487
    old "Почитать о повадках..."
    new "En savoir plus sur les habitudes..."

    # game/events/max.rpy:648
    old "{i}читать \"WEB STANDARDS\"{/i}"
    new "{i}lire les 'WEB STANDARDS'{/i}"

    # game/events/max.rpy:648
    old "{i}читать \"СЕКС-ОБРАЗОВАНИЕ\"{/i}"
    new "{i}lire 'ÉDUCATION SEXUELLE'{/i}"

    # game/events/max.rpy:702
    old "{i}искать под подушкой{/i}"
    new "{i}regarder sous l’oreiller{/i}"

    # game/events/max.rpy:702
    old "{i}искать в шкафу{/i}"
    new "{i}regarder dans le placard{/i}"

    # game/events/max.rpy:702
    old "{i}прекратить поиски{/i}"
    new "{i}arrêter la recherche{/i}"

    # game/events/max.rpy:859
    old "{i}искать...{/i}"
    new "{i}chercher...{/i}"

    # game/events/max.rpy:886
    old "{i}Подложить сейчас{/i}"
    new "{i}Mettre maintenant{/i}"

    # game/events/max.rpy:932
    old "Вы просматриваете видеоурок и повышаете свои навыки."
    new "Regarder des leçons vidéo pour améliorer mes compétences."

    # game/events/max.rpy:966
    old "{i}подставить Алису{/i}"
    new "{i}piéger Alice{/i}"

    # game/events/max.rpy:966
    old "{i}не подставлять Алису{/i}"
    new "{i}ne pas piéger Alice{/i}"

    # game/events/max.rpy:1066
    old "$500"
    new "500 $"

    # game/events/max.rpy:1066
    old "$1000"
    new "1000 $"

    # game/events/max.rpy:1066
    old "$2000"
    new "2000 $"

    # game/events/max.rpy:1066
    old "$5000"
    new "5000 $"

    # game/events/max.rpy:1086
    old "$50"
    new "50 $"

    # game/events/max.rpy:1086
    old "$100"
    new "100 $"

    # game/events/max.rpy:1086
    old "$200"
    new "200 $"

    # game/events/max.rpy:1156
    old "{i}идти к Эрику с включённым диктофоном{/i}"
    new "{i}aller voir Eric avec l'enregistreur activé {/i}"

