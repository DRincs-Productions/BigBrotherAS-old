# TODO: Translation updated at 2022-07-09 12:49

# game/events/other.rpy:11
translate french shoping_034007bc:

    # Ann_05 "Макс! Мы ушли на шоппинг. Не скучай тут без нас, хорошо? Вернёмся часа через 3..." nointeract
    Ann_05 "Max! Nous sommes allés faire du shopping. Ne te manque pas ici sans nous, d'accord? On revient dans 3 heures..." nointeract

# game/events/other.rpy:34
translate french back_shoping_bd2b389d:

    # Lisa_02 "Привет, Макс! Мы вернулись..."
    Lisa_02 "Bonjour, Max ! Nous sommes de retour..."

# game/events/other.rpy:35
translate french back_shoping_cd0b2b2b:

    # Max_04 "Ну как, удачно сходили?"
    Max_04 "Eh bien, ça s’est bien passé?"

# game/events/other.rpy:36
translate french back_shoping_3945eada:

    # Alice_13 "Ну, так... Платье мне, конечно, не купили. Даже то очень длинное, которое чуть выше колен..."
    Alice_13 "Alors... Je n'ai pas eu de robe courte, bien sûr. Même la très longue, qui est juste au-dessus des genoux..."

# game/events/other.rpy:37
translate french back_shoping_d40343f5:

    # Max_07 "А почему не купили?"
    Max_07 "Pourquoi ne l'avez-vous pas achetée ?"

# game/events/other.rpy:38
translate french back_shoping_8ac1e4ec:

    # Alice_01 "А вот у мамы спроси..."
    Alice_01 "Mais demandes à notre mère..."

# game/events/other.rpy:39
translate french back_shoping_838f5698:

    # Max_00 "Мам?"
    Max_00 "maman?"

# game/events/other.rpy:41
translate french back_shoping_ff699a77:

    # Ann_01 "Алиса, мы это уже обсуждали. Я против платьев, где у тебя всё видно. То, что оно длинное, не значит, что твою задницу не видно через разрез до ушей..." nointeract
    Ann_01 "Alice, on en a déjà parlé. Je suis contre les robes où l'on peut tout voir. Ce n'est pas parce que c'est long que l'on ne voit pas son cul à travers la fente jusqu'aux oreilles...." nointeract

# game/events/other.rpy:46
translate french back_shoping_ec104f51:

    # Alice_02 "Ну, теперь остаётся только мечтать. Спасибо тебе мама за чудесный шоппинг... Лизе тоже понравилось, видимо..."
    Alice_02 "Eh bien, maintenant tout ce que nous pouvons faire est de rêver. Merci maman pour cette merveilleuse expérience de shopping... Lisa l'a aimé aussi, apparemment..."

# game/events/other.rpy:47
translate french back_shoping_e23a2e06:

    # Max_01 "А что с Лизой? Купальник купили?"
    Max_01 "Et pour Lisa ? Tu as acheté un maillot de bain ?"

# game/events/other.rpy:49
translate french back_shoping_dbb7dd3e:

    # Lisa_09 "Нет. Мама сказала, что тот который я хотела, слишком дорогой, а остальные слишком открытые..." nointeract
    Lisa_09 "Non. Maman a dit que celui que je voulais était trop cher et que les autres étaient trop ouverts..." nointeract

# game/events/other.rpy:51
translate french back_shoping_4ac118f9:

    # Ann_01 "Купальник не может стоить таких денег, а она выбрала дизайнерский какой-то. Как будто мы тут миллионеры все..."
    Ann_01 "Un maillot de bain ne peut pas coûter si cher et elle a choisi des modèles de marque. C'est comme si nous étions tous millionnaires ici..."

# game/events/other.rpy:52
translate french back_shoping_0901fee4:

    # Max_05 "Ясно. А насколько открытыми были остальные?"
    Max_05 "Je vois. Et à quel point les autres étaient-ils ouverts ?"

# game/events/other.rpy:55
translate french back_shoping_b5471cb2:

    # Ann_05 "Ну тебе бы точно понравилось, как и всем мужикам, кто её бы увидел, но Лиза ещё маленькая, чтобы в таком виде появляться на публике. Так что..."
    Ann_05 "Tu l'aimerais certainement, comme tous les hommes qui la verraient, mais Lisa est trop jeune pour apparaître en public comme ça. Alors..."

# game/events/other.rpy:56
translate french back_shoping_542cfe6c:

    # Max_03 "Всё ясно..."
    Max_03 "Tout est clair..."

# game/events/other.rpy:57
translate french back_shoping_88944574:

    # Alice_13 "В общем, бесполезно потратили время. Но Эрик обещал, что на следующий шоппинг купит нам всё, что мы захотим, да, мам?"
    Alice_13 "En somme, une perte de temps. Mais Eric a promis qu'au prochain shopping, il nous achèterait tout ce que nous voulions, n'est-ce pas, maman ?"

# game/events/other.rpy:58
translate french back_shoping_3081eefb:

    # Max_10 "Что?!"
    Max_10 "Quoi?!"

# game/events/other.rpy:60
translate french back_shoping_87e46818:

    # Ann_01 "Алиса, хватит мечтать. Это всё выглядит так, как будто вы хотите из него выкачать как можно больше денег. Успокойтесь и будьте скромнее..." nointeract
    Ann_01 "Alice, arrête de rêver. On dirait que vous voulez lui soutirer le plus d'argent possible. Calmez-vous et soyez humble..." nointeract

# game/events/other.rpy:62
translate french back_shoping_b5a1abfc:

    # Ann_05 "Вот, Макс понимает, что это некрасиво клянчить деньги у других..."
    Ann_05 "Ici, Max comprend que ce n'est pas bien de demander de l'argent aux autres..."

# game/events/other.rpy:63
translate french back_shoping_ff1fed11:

    # Max_09 "Кстати, где он?"
    Max_09 "Au fait, où est-il ?"

# game/events/other.rpy:66
translate french back_shoping_7fae385c:

    # Ann_00 "Эрик подъедет к ужину. Сейчас у него какие-то дела. Не переживай, сегодня ты его увидишь."
    Ann_00 "Eric arrivera à temps pour le dîner. Il est au milieu de quelque chose en ce moment. Ne vous inquiétez pas, vous le verrez aujourd'hui."

# game/events/other.rpy:67
translate french back_shoping_e80e3f94:

    # Max_08 "Да я и не переживаю..."
    Max_08 "Je ne suis pas inquiet..."

# game/events/other.rpy:68
translate french back_shoping_b742f90d:

    # Ann_05 "Вот и отлично. Ладно, поболтаем позже..."
    Ann_05 "C'est bien. ok, on en reparle plus tard..."

# game/events/other.rpy:69
translate french back_shoping_7852d704:

    # Max_00 "Ага..."
    Max_00 "oui..."

# game/events/other.rpy:75
translate french back_shoping_1deadd07:

    # Ann_05 "Привет, Макс! Мы вернулись..."
    Ann_05 "Bonjour, Max ! Nous sommes de retour..."

# game/events/other.rpy:76
translate french back_shoping_a28c1031:

    # Max_04 "Рассказывайте, что купили?"
    Max_04 "Dis-nous, qu'as-tu acheté ?"

# game/events/other.rpy:85
translate french back_shoping_dc000c03:

    # Lisa_03 "А мне Эрик подарил купальник! Именно такой, как я и хотела! Красный, представляешь?!"
    Lisa_03 "Et Eric m'a acheté un maillot de bain ! Juste comme je le voulais ! Rouge, tu peux le croire ?!"

# game/events/other.rpy:86
translate french back_shoping_f936ff3c:

    # Max_11 "Ясно..."
    Max_11 "Je vois..."

# game/events/other.rpy:87
translate french back_shoping_45a6e528:

    # Lisa_02 "Что? Ты за меня не рад? Я же теперь смогу загорать в нормальном виде!"
    Lisa_02 "Quoi ? Tu n'es pas content pour moi ? Je vais pouvoir prendre le soleil normalement maintenant !"

# game/events/other.rpy:88
translate french back_shoping_c856577b:

    # Max_09 "Конечно, рад... Что-то ещё хорошее купили?"
    Max_09 "Bien sûr, je suis... As-tu acheté autre chose de beau ?"

# game/events/other.rpy:90
translate french back_shoping_920128b5:

    # Lisa_02 "Да так, вскую ерунду... Для нас, девочек. Тебе это не интересно..."
    Lisa_02 "Juste des bêtises... Pour nous, les filles. Tu n'es pas intéressé..."

# game/events/other.rpy:91
translate french back_shoping_f47e0d84:

    # Max_00 "Понятно..."
    Max_00 "D’accord..."

# game/events/other.rpy:93
translate french back_shoping_7a400341:

    # Alice_07 "Ты забыла про самое главное, Лиза. Мне Эрик купил платье! Представляешь?!"
    Alice_07 "Tu as oublié la chose la plus importante, Lisa. Eric m'a acheté une robe ! Tu peux le croire ?!"

# game/events/other.rpy:94
translate french back_shoping_31015994:

    # Max_10 "Да, это чудесно..."
    Max_10 "oui, c’est merveilleux..."

# game/events/other.rpy:95
translate french back_shoping_295e6d4e:

    # Alice_04 "Вот именно! Теперь я смогу ходить по клубам, а не сидеть дома вечерами! Эрик такой молодец. Я в восторге!"
    Alice_04 "Exactement ! Maintenant, je peux aller en boîte au lieu de rester à la maison le soir ! Eric est si bon. Je suis ravie !"

# game/events/other.rpy:96
translate french back_shoping_52ba99cd:

    # Max_11 "Поздравляю..."
    Max_11 "Félicitations..."

# game/events/other.rpy:103
translate french back_shoping_be1ec23c:

    # Ann_07 "Макс, ты какой-то грустный. Что-то случилось? Или мне показалось?"
    Ann_07 "Max, tu as l'air triste. Il s'est passé quelque chose ? Ou c'était juste moi ?"

# game/events/other.rpy:104
translate french back_shoping_4d9d39c8:

    # Max_00 "Показалось, мам..."
    Max_00 "C'est juste une idée, maman..."

# game/events/other.rpy:105
translate french back_shoping_60b98ba6:

    # Alice_02 "Не переживай. Ты всегда можешь заработать и купить себе сам всё, что хочешь..."
    Alice_02 "Ne t'inquiètes pas. Tu peux toujours gagner de l'argent et t'acheter ce que tu veux..."

# game/events/other.rpy:106
translate french back_shoping_53937e67:

    # Max_09 "Спасибо за совет..."
    Max_09 "Merci pour le conseil..."

# game/events/other.rpy:121
translate french MeetingEric_ff8aa8d1:

    # Ann_00 "Макс, подойди пожалуйста. К нам приехал Эрик. Знакомься..."
    Ann_00 "Max, s'il te plaît, viens ici. Eric est venu nous rendre visite. Nous voir..."

# game/events/other.rpy:124
translate french MeetingEric_56e6606a:

    # Ann_05 "Знакомься, это - Эрик. Эрик, это Макс, мой сын..." nointeract
    Ann_05 "Je vous présente Eric. Eric, voici Max, mon fils...." nointeract

# game/events/other.rpy:129
translate french MeetingEric_deda115c:

    # Eric_01 "Мне тоже. Как жизнь, Макс, чем занимаешься?" nointeract
    Eric_01 "Moi aussi. Quoi de neuf, Max, qu'est-ce que tu fais ?" nointeract

# game/events/other.rpy:131
translate french MeetingEric_fae75ac4:

    # Eric_06 "Отличные слова, Макс. Ну, надеюсь, найдёшь то, что ищешь..."
    Eric_06 "De grands mots, Max. Eh bien, j'espère que tu trouveras ce que tu cherches..."

# game/events/other.rpy:132
translate french MeetingEric_ec11cf76:

    # Max_03 "Ага, спасибо..."
    Max_03 "Oui, merci..."

# game/events/other.rpy:138
translate french MeetingEric_aa815859:

    # Eric_00 "Понятно. Воздух пинаешь... Ну ничего, в своё время всё изменится..."
    Eric_00 "Je vois. Brasser de l'air... Ce n'est pas grave, tout va changer en temps voulu..."

# game/events/other.rpy:139
translate french MeetingEric_62426c0e:

    # Max_08 "Я не пинаю воздух..."
    Max_08 "Je ne brasse pas de l'air..."

# game/events/other.rpy:145
translate french MeetingEric_6deddf9e:

    # Eric_09 "Ого, какие мы дружелюбные, ну что же, не буду тебя утомлять своими вопросами..."
    Eric_09 "Wow, comme tu es sympathiques, eh bien, je ne vais pas t'ennuyer avec mes questions..."

# game/events/other.rpy:146
translate french MeetingEric_2db9f65a:

    # Max_09 "Ну и ладно..."
    Max_09 "D’accord..."

# game/events/other.rpy:150
translate french MeetingEric_cf7f1be0:

    # Eric_09 "Очень приятно, Макс. А ты у нас не очень разговорчивый, да?" nointeract
    Eric_09 "C'est un plaisir, Max. Tu n'es pas un grand bavard, n'est-ce pas ?" nointeract

# game/events/other.rpy:152
translate french MeetingEric_fda4595b:

    # Eric_06 "Вот это да! Молодец, Макс. Вот это правильный подход."
    Eric_06 "Waouh ! Bravo, Max. C’est la bonne approche."

# game/events/other.rpy:153
translate french MeetingEric_25275504:

    # Max_01 "Спасибо..."
    Max_01 "Merci..."

# game/events/other.rpy:156
translate french MeetingEric_fdedac6a:

    # Eric_02 "Всё ясно с тобой. У нас тут скромный мальчик. Ну, не буду тебя утомлять больше..."
    Eric_02 "Je vois ce que tu es. Nous avons un garçon modeste ici. Eh bien, je ne vais pas te fatiguer plus ..."

# game/events/other.rpy:157
translate french MeetingEric_18637ae8:

    # Max_00 "Отлично..."
    Max_00 "Excellent..."

# game/events/other.rpy:163
translate french MeetingEric_e5d94a12:

    # Eric_02 "Ань, а ты не говорила, что у тебя сын такой агрессивный..."
    Eric_02 "Anna, et toi n'avez pas dit que vous aviez un fils aussi agressif ..."

# game/events/other.rpy:164
translate french MeetingEric_58916f16:

    # Max_09 "Ты и не спрашивал..."
    Max_09 "Tu n'as pas demandé ..."

# game/events/other.rpy:171
translate french MeetingEric_e662491e:

    # Eric_02 "Извини, Макс, что не оправдал твои ожидания. В любом случае, приятно с тобой познакомиться..." nointeract
    Eric_02 "Désolé, Max, cela n'a pas répondu à tes attentes. En tout cas, c'est un plaisir de te rencontrer..." nointeract

# game/events/other.rpy:176
translate french MeetingEric_5febc517:

    # Eric_05 "Макс, давай сразу на ты. Никаких формальностей. Это же семейный ужин. Так что, давайте будем проще..."
    Eric_05 "Max, parlons directement de toi. Aucune formalité. C’est un dîner de famille. Alors, soyons plus simples..."

# game/events/other.rpy:177
translate french MeetingEric_696b9e70:

    # Max_01 "Как скажешь..."
    Max_01 "Comme tu veux..."

# game/events/other.rpy:183
translate french MeetingEric_44fb47c0:

    # Eric_01 "Понятно. Видимо, Макс слишком скромный. Ну ничего страшного, я в его возрасте тоже смущался..."
    Eric_01 "Compréhensible. Apparemment, Max est trop modeste. Bon, ça va, j’étais gêné à son âge aussi..."

# game/events/other.rpy:184
translate french MeetingEric_c0dc2202:

    # Max_08 "И ничего я не смущаюсь..."
    Max_08 "Et je ne suis gêné par rien..."

# game/events/other.rpy:188
translate french MeetingEric_68dfdf17:

    # Eric_09 "И что со мной не так? Вот так с первого взгляда делаешь выводы о человеке? Не очень умно... Но для твоего возраста это нормально..." nointeract
    Eric_09 "Et qu'est-ce qui ne va pas avec moi? Donc, à première rencontre, tu fais des conclusions sur une personne? Pas très intelligent ... mais pour ton âge, c'est normal..." nointeract

# game/events/other.rpy:199
translate french MeetingEric_good_53d048fe:

    # Ann_07 "Ну, я рада, что вы познакомились и, вроде бы, нашли общий язык. Прошу, Эрик, к столу. Мы завтракаем и ужинаем на свежем воздухе, на веранде, пойдём покажу." nointeract
    Ann_07 "Eh bien, je suis content que tu aies appris et, il semble que nous avons trouvé un langage commun. S'il te plaît, Eric, à la table. Nous avons le petit déjeuner et le dîner à l'air frais, sur la véranda, allons-y." nointeract

# game/events/other.rpy:209
translate french MeetingEric_middle_6e04cd04:

    # Ann_17 "Макс, мы с тобой потом поговорим. Ну, Эрик, проходи к столу. Мы обычно ужинаем, да и завтракаем на открытом воздухе, тут у нас веранда..." nointeract
    Ann_17 "Max, je vais te parler plus tard. Eh bien, Eric, vas à la table. Nous dînons habituellement et le petit-déjeuner à l'extérieur, nous avons une véranda..." nointeract

# game/events/other.rpy:219
translate french MeetingEric_bad_2c10cf49:

    # Ann_20 "Макс! Ты почему себя так ведёшь? Это не только тебя не красит, но и меня позорит, что я тебя так воспитала!" nointeract
    Ann_20 "Max! Pourquoi te comportes-tu de cette façon? Cela ne te désert pas seulement, mais ça te diminue à mes yeux de toute l'aide que je t'ai apporté!" nointeract

# game/events/other.rpy:224
translate french MeetingEric_bad_341bbd07:

    # Ann_01 "Вот, так уже лучше. Ладно, надеюсь, это недоразумение из-за голодного желудка. Пойдёмте все к столу, пора ужинать..."
    Ann_01 "Donc, c'est mieux. D'accord, j'espère que c'est un malentendu en raison de l'estomac affamé. Allons tout à la table, il est temps de dîner ..."

# game/events/other.rpy:227
translate french MeetingEric_bad_9101ec96:

    # Ann_01 "Нет, Макс, это точно не нормально. Спишем на то, что ты голодный... Пойдём, Эрик, покажу где мы ужинаем, да и завтракаем. Макс, ты тоже иди ужинать."
    Ann_01 "Non, Max, ce n'est certainement pas normal. Notons que tu as faim ... Allons-y, Eric, je vais te montrer où nous dînerons et nous aurons le petit-déjeuner. Max, tu y vas aussi."

# game/events/other.rpy:230
translate french MeetingEric_bad_c246e424:

    # Ann_01 "Молчишь? Нечего сказать или стыдно? Будем считать, что ты просто голодный, а не специально хотел нахамить Эрику. Ладно, пойдёмте ужинать..."
    Ann_01 "Silencieux? Inutile de rien dire ou par honte? Nous supposerons que tu as faim, et que tu n'es pas spécifiquement contre Eric. D'accord, allons dîner..."

# game/events/other.rpy:243
translate french Kira_arrival_ce3f4211:

    # Ann_05 "Ну что, дети, дождались? Встречайте, к нам приехала тётя Кира!"
    Ann_05 "Eh bien, les enfants, vous avez attendu ? Tata Kira est là pour nous voir !"

# game/events/other.rpy:244
translate french Kira_arrival_9816e105:

    # Max_05 "Супер!"
    Max_05 "Super!"

# game/events/other.rpy:246
translate french Kira_arrival_437eebba:

    # Kira_01 "Это кто тут у нас? Макс? Я тебя не узнала! Почти настоящий мужчина!" nointeract
    Kira_01 "Qui est là? Max ? Je ne t'avais pas reconnu ! Un demi homme, presque un vrai homme!" nointeract

# game/events/other.rpy:250
translate french Kira_arrival_761ba430:

    # Kira_06 "Да я шучу, Макс. Конечно, ты уже мужчина. Вижу, всё у тебя в порядке. И как ты только держишься среди таких девчонок? Кстати, где Лиза и Алиса?"
    Kira_06 "Je plaisante, Max. Bien sûr, tu es déjà un homme. Je vois que tu te débrouilles bien. Comment tu fais pour rester en contact avec des filles comme ça ? Au fait, où sont Lisa et Alice ?"

# game/events/other.rpy:251
translate french Kira_arrival_f2f8c543:

    # Max_04 "Уже бегут..."
    Max_04 "Déjà en cours..."

# game/events/other.rpy:255
translate french Kira_arrival_4c447413:

    # Kira_07 "Лиза? Алиса? Неужели, это вы? Все такие большие, с меня ростом!"
    Kira_07 "Lisa ? Alice ? C'est vraiment vous ? Elles sont toutes les deux si grandes, elles sont aussi grandes que moi !"

# game/events/other.rpy:256
translate french Kira_arrival_41dea78a:

    # Lisa_03 "Тётя Кира! Ура! Я так соскучилась!"
    Lisa_03 "tante Kira! Hourra! Tu nous as tellement manquée !"

# game/events/other.rpy:257
translate french Kira_arrival_34dba009:

    # Max_02 "Хорошую подушку ты нашла..."
    Max_02 "Joli oreiller que tu as trouvé..."

# game/events/other.rpy:258
translate french Kira_arrival_c8bed868:

    # Alice_07 "Я так рада, что ты приехала! Ты же надолго, да?"
    Alice_07 "Je suis si heureuse que tu sois venue ! Tu vas être ici longtemps, n'est-ce pas ?"

# game/events/other.rpy:259
translate french Kira_arrival_94209dce:

    # Kira_04 "Что, уже выгоняете?!"
    Kira_04 "Quoi, tu me mets déjà à la porte ?!"

# game/events/other.rpy:261
translate french Kira_arrival_9a868553:

    # Ann_04 "Что ты, Кира, оставайся хоть навсегда! Я тоже очень рада тебя видеть, сестрёнка!" nointeract
    Ann_04 "Aller, Kira, reste pour toujours ! C'est bon de te voir aussi, sœurette!" nointeract

# game/events/other.rpy:263
translate french Kira_arrival_837dc5ce:

    # Lisa_05 "Нет! Она моя! Никому не отдам!"
    Lisa_05 "Non ! Elle est à moi ! Je ne la donnerais à personne !"

# game/events/other.rpy:264
translate french Kira_arrival_7dd083c1:

    # Max_00 "Ну и ладно. Я тогда... потом."
    Max_00 "Peu importe. J'ai alors... après."

# game/events/other.rpy:267
translate french Kira_arrival_f8793ac7:

    # Ann_01 "Кира, ты меня удивляешь! Не в мини-юбке... да ещё и такая нарядная! Неужели, прямо в таком виде летела в самолёте?"
    Ann_01 "Kira, tu me surprends ! Pas en mini-jupe... Et si habillé, aussi ! C'était vraiment comme ça dans l'avion ?"

# game/events/other.rpy:268
translate french Kira_arrival_239a2d0b:

    # Max_07 "Да, кстати..."
    Max_07 "Oh, au fait..."

# game/events/other.rpy:270
translate french Kira_arrival_4bf7c736:

    # Kira_05 "Нет, что ты. Я прилетела пару дней назад и остановилась в гостинице. Просто... нужно было разобраться кое с чем. Но с делами покончено и специально ради вас я наряжалась всё утро!" nointeract
    Kira_05 "Non, tu ne le fais pas. J'ai pris l'avion il y a quelques jours et je suis resté à l'hôtel. C'est juste que... il y avait des choses à régler. Mais j'en ai fini avec les affaires et je me suis habillée toute la matinée juste pour toi!" nointeract

# game/events/other.rpy:274
translate french Kira_arrival_2171ddd4:

    # Kira_02 "Ага. Не буду утомлять вас подробностями, это никому не интересно. Скажу лишь, что всё улажено и теперь я свободна для своей семьи!"
    Kira_02 "Ouaip. Je ne vais pas t'ennuyer avec les détails, ça n'intéresse personne. Tout ce que je peux dire, c'est que tout est réglé et que je suis libre pour ma famille maintenant !"

# game/events/other.rpy:278
translate french Kira_arrival_3a81a6f4:

    # Kira_03 "Да, я прошу прощения, что не предупредила, но не могла завалиться к вам, чтобы тут же убежать. Но я со всем разобралась, всё в порядке, теперь я тут!"
    Kira_03 "Oui, je m'excuse de ne pas vous avoir prévenu, mais je ne pouvais pas débarquer pour m'enfuir immédiatement. Mais j'ai tout arrangé, c'est bon, je suis là maintenant !"

# game/events/other.rpy:282
translate french Kira_arrival_end_yard_5810a3fd:

    # Max_07 "А мне любопытно, что за дела?"
    Max_07 "Et je suis curieux, c'est quoi le problème ?"

# game/events/other.rpy:283
translate french Kira_arrival_end_yard_b57b009f:

    # Ann_02 "Макс, ну хватит пытать свою тётю. Она сама расскажет, если захочет, верно? А теперь пойдёмте все в дом, скоро будем завтракать."
    Ann_02 "Max, arrête de torturer ta tante. Elle vous le dira si elle le veut, n'est-ce pas ? Maintenant rentrons tous à l'intérieur, nous allons bientôt prendre le petit déjeuner."

# game/events/other.rpy:293
translate french Kira_arrival_end_yard_61f344a0:

    # Kira_01 "Так как у меня было время подготовиться к встрече с вами, я приехала не с пустыми руками и привезла вам кое-что..."
    Kira_01 "Comme j'ai eu le temps de me préparer à vous rencontrer, je ne suis pas venu les mains vides et je vous ai apporté quelque chose..."

# game/events/other.rpy:294
translate french Kira_arrival_end_yard_27b5d9a4:

    # Max_03 "Ух-ты!"
    Max_03 "Waouh !"

# game/events/other.rpy:295
translate french Kira_arrival_end_yard_5c3c88a7:

    # Kira_07 "Я уже в курсе вашей истории с потерянным вещами. Поэтому, я уточнила у вашей мамы ваши размеры и купила лёгкую домашнюю одежду. И, кажется, правильно сделала. В такую жару, дома, лучше носить что-нибудь лёгкое!"
    Kira_07 "Je suis déjà au courant de votre histoire avec les objets perdus. Donc, j'ai vérifié avec ta mère tes tailles et j'ai acheté des vêtements légers pour la maison. Et je pense avoir fait le bon choix. Par cette chaleur, à la maison, il est préférable de porter quelque chose de léger !"

# game/events/other.rpy:296
translate french Kira_arrival_end_yard_37f87db2:

    # Lisa_02 "Ой! Я обожаю новые вещички. А что ты мне привезла, тётя Кира, а?"
    Lisa_02 "Aïe ! J'aime les nouvelles choses. Qu'est-ce que tu m'as apporté, tante Kira, hein ?"

# game/events/other.rpy:297
translate french Kira_arrival_end_yard_b3e5b52d:

    # Max_01 "Я думал у нас Алиса шмотки любит..."
    Max_01 "Je croyais qu'Alice n'aimait pas les vêtements..."

# game/events/other.rpy:298
translate french Kira_arrival_end_yard_e217d4e7:

    # Lisa_01 "Макс, какой ты глупый, кошмар! Шмотки любят все девушки! Ну так что там?"
    Lisa_01 "Max, tu es si stupide, tu es un cauchemar ! Toutes les filles aiment les vêtements ! Alors, qu'est-ce que c'est ?"

# game/events/other.rpy:300
translate french Kira_arrival_end_yard_d90987d8:

    # Kira_05 "Так, вы все давайте держите новую одежду, переодевайтесь и встречаемся за столом, где вы завтракаете. Посмотрим, всем ли всё подошло. А тебе, Макс, я ничего не купила. Подумала, что ты можешь и в шортах ходить. Но подарок тебе сделаю, позже решим какой..." nointeract
    Kira_05 "Ok, aller tous chercher vos nouveaux vêtements, changez-vous et rendez-vous à la table du petit-déjeuner. Voyons si tout vous va. Et pour toi, Max, je n'ai rien acheté. J'ai pensé que tu pourrais aussi bien porter des shorts. Mais je vais te faire un cadeau, on décidera plus tard quoi..." nointeract

# game/events/other.rpy:309
translate french Kira_arrival_end_yard_3af78784:

    # Max_11 "{m}Ну где они все?! Если я начну завтракать без них, то потом все только и будут говорить о моей невоспитанности. Давайте уже быстрее!{/m}"
    Max_11 "{m}Où sont-ils tous?! Si je commence à déjeuner sans eux, alors tout le monde parlera de mon manque d'éducation. Nous allons déjà plus vite!{/m}"

# game/events/other.rpy:314
translate french Kira_arrival_end_yard_e9e6d4fa:

    # Lisa_02 "Ну как вам? Такой классный топ и юбочка и мой любимый цвет, представляете!" nointeract
    Lisa_02 "Qu'en pensez-vous ? Un haut et une jupe si cool et ma couleur préférée, imaginez!" nointeract

# game/events/other.rpy:316
translate french Kira_arrival_end_yard_f609e252:

    # Ann_01 "Да, вот только меня немного смущает длина юбки. Видимо, Кира, ты на свой вкус выбирала. Лиза, покрутись, пожалуйста..."
    Ann_01 "Oui, sauf que je suis un peu confuse par la longueur de la jupe. Apparemment, Kira, tu as choisi pour ton goût. Lisa, peux-tu faire tourner..."

# game/events/other.rpy:320
translate french Kira_arrival_end_yard_cd5b2e68:

    # Lisa_03 "Мне кажется, что не слишком коротко. В самый раз! И теперь так свободно и легко... Спасибо тебе, тётя Кира!"
    Lisa_03 "Je ne pense pas que ce soit trop court. Juste ce qu'il faut ! Et maintenant, c'est tellement joli et facile... Merci, tante Kira !"

# game/events/other.rpy:321
translate french Kira_arrival_end_yard_21c31287:

    # Max_03 "Да, длина в самый раз..."
    Max_03 "Oui, la longueur est juste bonne..."

# game/events/other.rpy:324
translate french Kira_arrival_end_yard_a5573412:

    # Alice_02 "Ну всё, теперь моя очередь! Как вам? Лично мне очень нравится! Давно надо было раздеться."
    Alice_02 "C'est ça, maintenant c'est mon tour ! Qu'en pensez-vous ? Personnellement, je l'aime beaucoup ! J'aurais dû me déshabiller."

# game/events/other.rpy:325
translate french Kira_arrival_end_yard_f057f5be:

    # Max_04 "Именно!"
    Max_04 "Exactement !"

# game/events/other.rpy:327
translate french Kira_arrival_end_yard_d2a8a875:

    # Ann_02 "Я не поняла. А трусы то на тебе хоть есть? Ты же не планируешь в таком виде ходить дома, Алиса?" nointeract
    Ann_02 "Je n'ai pas compris. Est-ce que tu portes au moins des sous-vêtements ? Tu n'as pas l'intention de te promener dans la maison en ressemblant à ça, Alice?" nointeract

# game/events/other.rpy:329
translate french Kira_arrival_end_yard_c8223c08:

    # Alice_03 "Спасибо, Макс! Мам, а что такого? Ты не представляешь как жарко было в джинсах, а теперь хоть тело дышит. Да и жара такая на улице и дома..."
    Alice_03 "Merci, Max ! Maman, c'est quoi le problème ? Tu ne peux pas imaginer la chaleur qu'il faisait en jean, mais maintenant au moins mon corps peut respirer. Et il fait si chaud dehors et à la maison..."

# game/events/other.rpy:330
translate french Kira_arrival_end_yard_12a4e2fa:

    # Ann_01 "Ладно, а что там сзади? Ну-ка покрутись..."
    Ann_01 "Ok, qu'est-ce qu'il y a derrière ? Tournes..."

# game/events/other.rpy:332
translate french Kira_arrival_end_yard_4357e252:

    # Alice_05 "Мам, да всё в порядке. На мне шорты, ничего не видно. Вот, смотри..."
    Alice_05 "Maman, c'est bon. Je porte des shorts, je ne peux rien voir. Tiens, regarde..."

# game/events/other.rpy:334
translate french Kira_arrival_end_yard_4fd7558f:

    # Ann_12 "Алиса, у тебя же всё видно, аж грудь вываливается! Смотри, как Макс таращится. Кажется, он даже завис..."
    Ann_12 "Alice, tu peux tout voir, tes seins tombent ! Regarde Max qui regarde fixement. Il semble même être figé..."

# game/events/other.rpy:335
translate french Kira_arrival_end_yard_f70abaf0:

    # Max_07 "Э... Ну... да..."
    Max_07 "Eh... Eh bien... Oui..."

# game/events/other.rpy:336
translate french Kira_arrival_end_yard_761bc9e3:

    # Alice_02 "Мам, я дома могу одеваться так, как мне нравится. Ну и что, что Макс. Я из-за него не буду целыми днями ходить в парандже! Мне удобно, выглядит отлично! Ну а если и увидит что-то, пусть радуется, мне всё равно..."
    Alice_02 "Maman, je peux m'habiller comme je veux à la maison. Et alors, si c'est Max. Je ne porterais pas une burka toute la journée à cause de lui! Je suis à l'aise, ça a l'air super ! Et s'il voit quelque chose, qu'il soit heureux, je m'en fiche..."

# game/events/other.rpy:337
translate french Kira_arrival_end_yard_82a7c555:

    # Max_02 "Отличная речь, сестрёнка!"
    Max_02 "Beau discours, ma sœur !"

# game/events/other.rpy:341
translate french Kira_arrival_end_yard_11f0d126:

    # Ann_04 "Ну, теперь и моя очередь. Как вам? Не слишком откровенно?" nointeract
    Ann_04 "Eh bien, maintenant c'est mon tour. Qu'en penses-tu? Pas trop flagrant?" nointeract

# game/events/other.rpy:348
translate french Kira_arrival_end_yard_27e62e75:

    # Ann_07 "Мне кажется, шортики слишком короткие... Я в них еле влезла, если честно. Надо худеть..." nointeract
    Ann_07 "Je pense que les shorts sont trop courts... Je pouvais à peine rentrer dedans, pour être honnête. Tu devrais perdre du poids..." nointeract

# game/events/other.rpy:354
translate french Kira_arrival_end_yard_a4137f9f:

    # Kira_03 "Ань, всё сидит на тебе просто отлично. И очень идёт этот цвет, кстати... Ну а теперь моя очередь. Сейчас покажу, что себе прикупила..."
    Kira_03 "Anna, tout te va très bien. Et c'est une très belle couleur, d'ailleurs... Maintenant, c'est mon tour. Laisses-moi te montrer ce que j'ai..."

# game/events/other.rpy:355
translate french Kira_arrival_end_yard_bfefabb8:

    # Max_01 "Интересно..."
    Max_01 "Intéressant..."

# game/events/other.rpy:359
translate french Kira_arrival_end_yard_95b07e68:

    # Kira_05 "Ну, как я вам? Хороша? И Ань, ни слова больше. Тебе всё кажется слишком открытое или откровенное." nointeract
    Kira_05 "Comment tu me trouves ? C'est bon ? Et Anna, pas un mot de plus. Tout te semble trop ouvert ou révélateur." nointeract

# game/events/other.rpy:366
translate french Kira_arrival_end_yard_2417a0cf:

    # Kira_07 "Вот, слышала, Ань, детям нравится. Естественно, если в семье есть мужчина, это накладывает свои ограничения, но нельзя же быть настолько зажатыми..."
    Kira_07 "Ici, j'ai entendu dire, Anna, que les enfants adorent ça. Naturellement, s'il y a un homme dans la famille, cela impose ses propres limites, mais il ne faut pas être si coincée..."

# game/events/other.rpy:367
translate french Kira_arrival_end_yard_5d16cce1:

    # Max_04 "Верно!"
    Max_04 "c’est vrai!"

# game/events/other.rpy:368
translate french Kira_arrival_end_yard_af415c55:

    # Ann_04 "Ладно, посмотрим. Кира, мне кажется, что не стоило тратиться на подарки. Мы рады, что ты сама смогла к нам приехать... Но всё равно, большое спасибо!"
    Ann_04 "Ok, voyons. Kira, il me semble que cela ne vaut pas la peine de dépenser des cadeaux. Nous sommes heureux que tu puisses venir chez nous ... mais pas toujours, merci beaucoup!"

# game/events/other.rpy:369
translate french Kira_arrival_end_yard_438c6fcd:

    # Max_07 "Давайте уже завтракать, а то остыло почти всё..."
    Max_07 "Prenons déjà le petit-déjeuner, il refroidit presque..."

# game/events/other.rpy:404
translate french follow_cameras_eric_4a1b4511:

    # Max_09 "{m}Неутомимый Эрик! Вали уже обратно к маме в комнату и засыпай. А то у меня тоже есть дела у Алисы в комнате...{/m}" nointeract
    Max_09 "{m}Infatigable Eric! Retourne dans la chambre de ta mère et dors. J'ai aussi des affaires avec Alice dans la chambre...{/m}" nointeract

# game/events/other.rpy:424
translate french follow_cameras_eric_2da5afe7:

    # Max_01 "{m}Отлично! Пора...{/m}" nointeract
    Max_01 "{m}Bien! C'est l'heure...{/m}" nointeract

# game/events/other.rpy:436
translate french frame_eric_60e127e3:

    # Max_07 "{m}О, да! Моя старшая сестрёнка выглядит потрясающе... На изгибы её совершенно обнажённого тела хочется смотреть вечно!{/m}" nointeract
    Max_07 "{m}oh, oui! Ma grande soeur a l'air incroyable... Les courbes de son corps complètement nu veulent regarder pour toujours!{/m}" nointeract

# game/events/other.rpy:438
translate french frame_eric_58e230bc:

    # Max_04 "{m}Ого! Мне повезло, что Алиса спит спиной к окну... И не подозревает, что демонстрирует свою голенькую попку для меня во всей красе.{/m}" nointeract
    Max_04 "{m}Wow! J'ai de la chance qu'Alice dort le dos à la fenêtre... Et il ne se doutait pas qu'il montrait son cul nu pour moi dans toute sa splendeur.{/m}" nointeract

# game/events/other.rpy:440
translate french frame_eric_f84d87b6:

    # Max_01 "{m}Обалденно! Сестрёнка спит выгнув спину, отчего её голая грудь торчит, как два холмика... Соблазнительное зрелище...{/m}" nointeract
    Max_01 "{m}Génial! La soeur dort en cambrant le dos, ce qui fait que sa poitrine nue dépasse comme deux monticules... Un spectacle séduisant...{/m}" nointeract

# game/events/other.rpy:450
translate french frame_eric_c7dc4684:

    # Max_07 "{m}Уж прости, Алиса, ничего личного. Мне позарез нужно показать всем вам, а особенно маме, что Эрик собирается тут с вами всеми делать...{/m}" nointeract
    Max_07 "{m}désolé, Alice, rien de personnel. Je dois vous montrer à tous, et surtout à maman, ce qu'Eric va faire avec vous...{/m}" nointeract

# game/events/other.rpy:457
translate french frame_eric_40044bf4:

    # Max_20 "{m}Только бы Алиса не проснулась! Иначе я отправлюсь не в военный лагерь, а сразу на кладбище... Но сейчас лучше думать не об этом, а о том, как сладко я бы проникал своим членом во все её самые интересные дырочки... Д-а-а... Это было бы очень приятно...{/m}" nointeract
    Max_20 "{m}si Alice ne se réveillait pas! Sinon, je n'irai pas au camp militaire, mais directement au cimetière... Mais maintenant, il vaut mieux ne pas y penser, mais à quel point je voudrais pénétrer ma bite dans tous ses trous les plus intéressants... D-a-A... Ce serait très agréable...{/m}" nointeract

# game/events/other.rpy:464
translate french frame_eric_f720febd:

    # Max_08 "{m}Фух! Вроде, не проснулась... Надеюсь, этого хватит, чтобы утром Алиса забила тревогу. Наверняка, она сразу подумает, что это я её чем-то испачкал. И вот тогда-то я и покажу снимки с Эриком, а уже всю картину воедино она сложит сама. Если я, конечно, смогу уйти незамеченным в свою комнату...{/m}" nointeract
    Max_08 "{m}En De la sorte, ne s'est réveillée... J'espère que ça suffira pour qu'Alice sonne l'alarme ce matin. Elle va sûrement penser que c'est moi qui l'ai salie. Et c'est là que je vais montrer les photos avec Eric, et déjà l'ensemble de l'image, elle va mettre elle-même. Si, bien sûr, je peux passer inaperçu dans ma chambre...{/m}" nointeract

# game/events/other.rpy:482
translate french showdown_with_eric_c59ea561:

    # Alice_17 "Макс! Ну-ка быстро просыпайся, а не то я тебе всыплю! Признавайся, чем ты меня ночью измазал?"
    Alice_17 "Max! Réveille-toi vite, sinon je te sers! Avouez-moi ce que vous m'avez fait cette nuit?"

# game/events/other.rpy:483
translate french showdown_with_eric_bd921574:

    # Max_07 "Измазал?! А почему сразу я?!"
    Max_07 "?! Et pourquoi j'ai tout de suite?!"

# game/events/other.rpy:484
translate french showdown_with_eric_38d64641:

    # Alice_16 "А кто ещё это может быть?! Здесь вообще без вариантов и гадалка не нужна! Так что говори, чем измазал и пошли к маме..."
    Alice_16 "qui d'autre pourrait-il être?! Ici, il n'y a pas d'options et une diseuse de bonne aventure n'est pas nécessaire! Ainsi que vous le dites, que измазал et est allé à sa mère..."

# game/events/other.rpy:485
translate french showdown_with_eric_4d7a3f63:

    # Lisa_10 "Алиса, хватит уже кричать! Я же сплю..."
    Lisa_10 "Alice, arrête de crier! Je dors..."

# game/events/other.rpy:486
translate french showdown_with_eric_5508b396:

    # Max_00 "Да, Алиса, ты как-то слишком возбуждена. Тебя может неслабо так шокировать то, что я тебе скажу. И даже покажу."
    Max_00 "oui, Alice, tu es trop excitée. Tu pourrais être tellement choquée par ce que je vais te dire. Et je vais même vous montrer."

# game/events/other.rpy:487
translate french showdown_with_eric_43ec76cb:

    # Alice_06 "Так я и знала, что это гадость какая-то! Ну так что это?"
    Alice_06 "donc je savais que c'était une sorte de merde! Eh bien, de sorte que c'est?"

# game/events/other.rpy:488
translate french showdown_with_eric_06f743d3:

    # Max_01 "Это Эрик с тобой кое-чем... поделился..."
    Max_01 "c'est Eric avec toi... a partagé..."

# game/events/other.rpy:496
translate french showdown_with_eric_fc0ce1b6:

    # Alice_12 "Макс, хватит уже... И что мне этот снимок должен сказать? Хотя... не поняла... Он дрочит что ли?! А где это он?"
    Alice_12 "Max, ça suffit... Qu'est-ce que cette photo a à me dire? Quoique... perdre... Il se branle?! Où est-il?"

# game/events/other.rpy:497
translate french showdown_with_eric_c64627b9:

    # Max_07 "А ты присмотрись и отгадаешь где."
    Max_07 "Et vous regardez de plus près et devinez où."

# game/events/other.rpy:498
translate french showdown_with_eric_e376d7c6:

    # Alice_14 "Ну... Это получается... Эй! Это он что, у моего окна стоит?"
    Alice_14 "Eh bien... Cela s'avère... Hé! Est-ce qu'il est devant ma fenêtre?"

# game/events/other.rpy:499
translate french showdown_with_eric_d5628f21:

    # Max_04 "Не только..."
    Max_04 "Pas seulement..."

# game/events/other.rpy:507
translate french showdown_with_eric_dea06daf:

    # Lisa_13 "Вы что тут с утра пораньше разглядываете? Ой! Эрик! Прямо у тебя в комнате, Алиса..."
    Lisa_13 "vous regardez quoi ce matin? Oh! Eric! Dans ta chambre, Alice..."

# game/events/other.rpy:508
translate french showdown_with_eric_3271758d:

    # Alice_15 "Он что, правда в моей комнате и... дрочит... на меня?!"
    Alice_15 "Il est vrai dans ma chambre et... branle... sur moi?!"

# game/events/other.rpy:509
translate french showdown_with_eric_ff78a8b8:

    # Max_02 "Ну да. Видишь, не такой уж я и извращенец, правда?"
    Max_02 "Oui. Tu vois, je ne suis pas si pervers, non?"

# game/events/other.rpy:510
translate french showdown_with_eric_db6785a5:

    # Alice_06 "То есть, то... что у меня было на ноге, это..."
    Alice_06 "C'est-à-dire, alors... ce que j'avais sur ma jambe, c'est..."

# game/events/other.rpy:511
translate french showdown_with_eric_80a21b22:

    # Max_10 "Да, Алиса. Оно самое."
    Max_10 "Oui, Alice. C'est le plus."

# game/events/other.rpy:518
translate french showdown_with_eric_9082d505:

    # Alice_17 "Вот же подонок озабоченный! Подожди... А ты какого хрена это всё фотографировал?!"
    Alice_17 "c'est un enfoiré! Attends... Qu'est-ce que tu as pris en photo?!"

# game/events/other.rpy:519
translate french showdown_with_eric_e40e9eac:

    # Max_09 "Видишь ли, Эрик мне много раз угрожал проблемами и ссылкой в военный лагерь, если я буду ему мешать в подобных... делах. Вот и пришлось подстраховаться."
    Max_09 "tu Vois, Eric m'a menacé à plusieurs reprises de problèmes et d'exil dans un camp militaire si je l'empêchais dans ce genre de choses... affaires. Donc, je devais me protéger."

# game/events/other.rpy:520
translate french showdown_with_eric_3d070281:

    # Alice_16 "И часто этот онанист чёртов ко мне в комнату ходит по ночам?"
    Alice_16 "et souvent ce connard va dans ma chambre la nuit?"

# game/events/other.rpy:521
translate french showdown_with_eric_a26a29c6:

    # Max_07 "Я бы сказал, что с завидным постоянством!"
    Max_07 "Je dirais avec une Constance enviable!"

# game/events/other.rpy:522
translate french showdown_with_eric_3d7646d5:

    # Alice_12 "Ублюдок какой! А ты чего молчал всё это время?!"
    Alice_12 "quel Enfoiré! Pourquoi tu n'as rien dit tout ce temps?!"

# game/events/other.rpy:523
translate french showdown_with_eric_60498e82:

    # Max_15 "Так ведь, если говорит Эрик, то это святая истина, а если я, то это чепуха какая-то."
    Max_15 "Eh bien, si Eric dit, c'est la Sainte vérité, et si je le fais, c'est un non-sens."

# game/events/other.rpy:525
translate french showdown_with_eric_6d1dd214:

    # Alice_17 "Вот я Эрику с мамой сейчас устрою! Макс, пошли за мной!" nointeract
    Alice_17 "je vais arranger Eric avec ma mère maintenant! Max, suivez-moi!" nointeract

# game/events/other.rpy:536
translate french showdown_with_eric_40a33569:

    # Ann_13 "Алиса! Ты зачем так врываешься?! Стучаться надо..."
    Ann_13 "Alice! Pourquoi creusez-vous comme ça?! Il faut frapper..."

# game/events/other.rpy:537
translate french showdown_with_eric_db864631:

    # Alice_16 "Не волнуйся, мам, я обязательно сейчас по кое-кому постучу! Знаешь, что твой мужчина вытворяет по ночам?"
    Alice_16 "ne t'inquiète pas, maman, je vais frapper quelqu'un maintenant! Tu sais ce que ton homme fait la nuit?"

# game/events/other.rpy:538
translate french showdown_with_eric_d1c15c50:

    # Ann_17 "Это что за тон, Алиса! Конечно, знаю. Ой, вы все здесь! Что происходит?"
    Ann_17 "quel ton, Alice! Bien sûr que je le sais. Oh, vous êtes tous là! Qu'est-ce qui se passe ?"

# game/events/other.rpy:539
translate french showdown_with_eric_86aba788:

    # Max_07 "У Алисы есть претензии к Эрику..."
    Max_07 "Alice a des réclamations contre Eric..."

# game/events/other.rpy:547
translate french showdown_with_eric_b57755ec:

    # Alice_17 "И ещё какие! Эрик дрочит на меня по ночам! Это как вообще называется? Я думала у нас Макс извращенец, но ты, Эрик..."
    Alice_17 "et plus encore! Eric me branle la nuit! C'est généralement appelé? Je pensais que Max était un pervers, mais toi, Eric..."

# game/events/other.rpy:548
translate french showdown_with_eric_f1dc38c8:

    # Ann_16 "Ты что такое вообще несёшь, Алиса?! Обвинять в таком Эрика, это..."
    Ann_16 "qu'est-ce que tu portes, Alice?! Blâmer Eric pour ça, c'est ça..."

# game/events/other.rpy:549
translate french showdown_with_eric_9f3d0dd5:

    # Alice_18 "Мам! У Макса есть фотографии! А это животное лохматое ещё и обкончало меня этой ночью..."
    Alice_18 "Maman! Max a des photos! Et cet animal hirsute m'a aussi fini cette nuit..."

# game/events/other.rpy:550
translate french showdown_with_eric_368c6e02:

    # Eric_09 "Так, Алиса, давай успокаивайся!"
    Eric_09 "alors, Alice, calme-toi!"

# game/events/other.rpy:560
translate french showdown_with_eric_9e66f9ab:

    # Alice_17 "Ну-ка заткнулся! Меня лучше не злить! Сейчас Макс покажет маме снимки и мы послушаем, как ты это объяснишь..."
    Alice_17 "tais-toi! Je ferais mieux de ne pas me fâcher! Max va montrer les photos à maman et on va t'écouter expliquer ça..."

# game/events/other.rpy:561
translate french showdown_with_eric_4f9368bb:

    # Ann_14 "И что вы хотите мне тут показать? Я не понимаю... Как такое вообще может быть?! Эрик, будь добр, объясни это..."
    Ann_14 "et que voulez-vous me montrer? Je ne comprends pas... Comment cela peut-il être?! Eric, explique-moi ça..."

# game/events/other.rpy:562
translate french showdown_with_eric_37815d99:

    # Eric_13 "Ань, не знаю, что там за снимки, но могу уверить тебя - это фотошоп!"
    Eric_13 "Anh, je ne sais pas quelles sont les images, mais je peux vous assurer-c'est Photoshop!"

# game/events/other.rpy:563
translate french showdown_with_eric_517352a4:

    # Max_09 "В таком случае, я за то, чтобы провести экспертизу их подлинности!"
    Max_09 "dans ce cas, je suis pour que l'examen de leur authenticité!"

# game/events/other.rpy:564
translate french showdown_with_eric_54354826:

    # Ann_15 "Эрик! У меня в голове не укладывается то, что я вижу... Господи! Ты же дрочишь на мою дочь! Как ты мог такое делать, мерзавец... Лиза, а с тобой всё в порядке?! Эрик ничего с тобой не делал?"
    Ann_15 "Eric! Je n'ai pas ce que je vois dans ma tête... Seigneur! Tu te branles sur ma fille! Comment as-tu pu faire ça, salaud?.. Lisa, ça va?! Eric ne t'a rien fait?"

# game/events/other.rpy:565
translate french showdown_with_eric_6266c673:

    # Lisa_10 "Нет... Правда, он предлагал мне закрепить на практике те уроки сексуального воспитания, что вы для меня проводили."
    Lisa_10 "Non... Certes, il m'a suggéré de mettre en pratique les leçons d'éducation sexuelle que vous avez faites pour moi."

# game/events/other.rpy:574
translate french showdown_with_eric_ad4584a4:

    # Alice_18 "Ах так! Значит, ты свои ручищи к нашей маленькой Лизе уже тянешь? Вот... получай, скотина..."
    Alice_18 "Ah! Alors, tu es déjà en train de tirer tes mains sur notre petite Lisa? Voici... prends ça, connard..."

# game/events/other.rpy:575
translate french showdown_with_eric_8277ef23:

    # Ann_20 "Эрик, ты совсем что ли больной?! Как ты мог такое делать с моими девочками! Живо выметайся из нашего дома, пока я полицию не вызвала!"
    Ann_20 "Eric, es-tu malade?! Comment as-tu pu faire ça à mes filles? Sors de chez nous avant que j'appelle la police!"

# game/events/other.rpy:578
translate french showdown_with_eric_847ca8ec:

    # Eric_10 "Какую ещё полицию?! Я ничего не делал. Это всё Макс!"
    Eric_10 "Quelle autre police?! Je n'ai rien fait. C'est Max!"

# game/events/other.rpy:579
translate french showdown_with_eric_8cb800af:

    # Lisa_13 "Не надо всё на Макса сваливать! Он хороший..."
    Lisa_13 "ne va pas tout foutre sur Max! Est-il bon..."

# game/events/other.rpy:580
translate french showdown_with_eric_5dcf637a:

    # Alice_16 "Я раньше от души могла навалять Максу, но сейчас я в такой ярости, что запросто наваляю такому здоровому извращенцу, как ты!"
    Alice_16 "j'avais l'habitude d'avoir du cœur à empiler Max, mais maintenant je suis tellement furieux que j'empoche facilement un pervers en bonne santé comme toi!"

# game/events/other.rpy:589
translate french showdown_with_eric_aece1638:

    # Ann_19 "Эрик, проваливай! Я не хочу тебя больше видеть!"
    Ann_19 "Eric, dégage! Je ne veux plus te revoir!"

# game/events/other.rpy:590
translate french showdown_with_eric_4a405f54:

    # Eric_09 "Вижу, вы все на взводе, так что я поговорю с вами, когда успокоитесь... А сейчас я оденусь и отправлюсь на работу."
    Eric_09 "je Vois que vous êtes tous en état d'alerte, alors je vous parlerai quand vous vous calmerez... Je vais m'habiller et aller travailler."

# game/events/other.rpy:591
translate french showdown_with_eric_2ec75356:

    # Ann_20 "Я сказала, {b}ПОШЁЛ ВОН НЕМЕДЛЕННО!!!{/b}"
    Ann_20 "j'ai dit, {b}SORS TOUT de suite!!!{/b}"

# game/events/other.rpy:592
translate french showdown_with_eric_cf71befd:

    # Alice_17 "И полотенце можешь оставить себе, грязное животное! Убирайся!"
    Alice_17 "et tu peux garder la serviette, sale animal! Sortez!"

# game/events/other.rpy:602
translate french showdown_with_eric_f3afe214:

    # Max_00 "Мам, ты только не расстраивайся так из-за него. Всякое бывает..."
    Max_00 "Maman, ne te fâche pas avec lui. Tout arrive..."

# game/events/other.rpy:603
translate french showdown_with_eric_b377cce2:

    # Ann_14 "Всё никак в голове не укладывается, что Эрик тут с вами творил... И что я за мать после этого? Я в шоке..."
    Ann_14 "Ce n'est pas ce qu'Eric a fait avec vous... Et quelle mère suis-je après ça? Je suis choquée..."

# game/events/other.rpy:604
translate french showdown_with_eric_90ee4fd2:

    # Alice_13 "Уж лучше узнать поздно, чем никогда. А у нас всё ещё лучше... Мы узнали, пока ещё не стало поздно. Так что не переживай, мам, мы все думали, что Эрик... уж точно не такой..."
    Alice_13 "mieux vaut savoir tard que jamais. Et nous sommes toujours meilleurs... Nous avons appris avant qu'il ne soit trop tard. Alors ne t'inquiète pas, maman, on pensait tous Eric... certainement pas comme ça..."

# game/events/other.rpy:605
translate french showdown_with_eric_167ce82b:

    # Max_09 "Хорошо, что я своим мужским взглядом сразу понял, что он мутный тип!"
    Max_09 "c'est Bien que j'ai immédiatement compris avec mon regard masculin qu'il était un type boueux!"

# game/events/other.rpy:606
translate french showdown_with_eric_ca2a104b:

    # Lisa_09 "Так это получается, что? Про бумажник Эрик наврал?"
    Lisa_09 "donc il se trouve que? Eric a menti sur son portefeuille?"

# game/events/other.rpy:607
translate french showdown_with_eric_b84c6e58:

    # Max_08 "Конечно! Он это придумал, чтобы вы меня в военный лагерь сослали. А то я стал слишком мешаться."
    Max_08 "Bien sûr! Il a inventé ça pour que vous m'exiliez dans un camp militaire. Je suis devenu trop gêné."

# game/events/other.rpy:608
translate french showdown_with_eric_35096e7a:

    # Ann_13 "Ты прости меня, сынок, я должна была прислушиваться к тому, что ты говорил. Что же теперь делать?"
    Ann_13 "pardonne-moi, fiston, j'aurais dû écouter ce que tu disais. Que faire maintenant?"

# game/events/other.rpy:609
translate french showdown_with_eric_e862c93b:

    # Max_01 "Жить, как жили до Эрика! Ничего сложного..."
    Max_01 "Vivre comme avant Eric! Rien de compliqué..."

# game/events/other.rpy:610
translate french showdown_with_eric_6ff07095:

    # Ann_12 "Нет, Макс, я о другом... Он же мой начальник! Как мне теперь быть? Ох, не знаю..."
    Ann_12 "non, Max, je parle d'autre chose... C'est mon patron! Comment puis-je maintenant être? Oh, je ne sais pas..."

# game/events/other.rpy:611
translate french showdown_with_eric_2621818a:

    # Max_07 "Д-а-а... Это проблемка. Ты главное не слушай его оправдания, а то он умеет убеждать, как ты сама убедилась прекрасно. А теперь давайте, все вместе, поддержим маму!"
    Max_07 "D-a-A... C'est un poseur. Tu n'écoutes pas ses excuses, sinon il sait convaincre, comme tu l'as fait toi-même. Maintenant, tous ensemble, soutenons maman!"

# game/events/other.rpy:618
translate french showdown_with_eric_f96123e9:

    # Ann_13 "Ой, дорогие мои, вы меня так раздавите! Но вы всё равно самые лучшие, кто у меня есть... Простите ещё раз за то, что я привела в наш прекрасный дом этого человека и всё испортила."
    Ann_13 "Oh, mes chers, vous me écrasez tellement! Mais vous êtes toujours les meilleurs que j'ai... Pardonnez-moi encore d'avoir amené cet homme dans notre belle maison et d'avoir tout gâché."

# game/events/other.rpy:619
translate french showdown_with_eric_5a9bcc35:

    # Lisa_01 "Не бери в голову, мам. Не вини себя, всё ведь хорошо. Мы с тобой."
    Lisa_01 "N'y pense pas, maman. Ne te blâme pas, tout va bien. Nous sommes avec toi."

# game/events/other.rpy:620
translate french showdown_with_eric_a2587239:

    # Alice_05 "И если он начнёт устраивать тебе проблемы на работе, то мы все так же дружно, как сейчас, соберёмся и отпинаем его!"
    Alice_05 "et s'il commence à vous faire des problèmes au travail, alors nous sommes toujours aussi amicaux que maintenant, nous nous réunirons et le lâcherons!"

# game/events/other.rpy:621
translate french showdown_with_eric_e1846793:

    # Max_09 "Думаю, Эрик не настолько туп, чтобы строить маме козни. Особенно, если она будет с ним жёстче..."
    Max_09 "je Pense qu'Eric n'est pas assez stupide pour construire un stratagème à maman. Surtout si elle est plus dure avec lui..."

# game/events/other.rpy:622
translate french showdown_with_eric_d14050f1:

    # Ann_17 "Ох, ребята, я постараюсь... Хотя, мне даже думать о нём сейчас противно, не то что видеть. Надеюсь, я не потеряю работу."
    Ann_17 "Oh, les gars, je vais essayer... Bien que je ne pense même pas à lui maintenant, ce n'est pas quelque chose que je vois. J'espère ne pas perdre mon travail."

# game/events/other.rpy:623
translate french showdown_with_eric_72f9e7fc:

    # Max_08 "Может, удастся перевестись в какой-нибудь другой отдел?"
    Max_08 "peut-on être transféré dans un autre département?"

# game/events/other.rpy:624
translate french showdown_with_eric_032de050:

    # Ann_02 "Надеюсь... Ну всё, мне уже дышать нечем. Пора делами заниматься. Я вас очень люблю..."
    Ann_02 "j'Espère... Je n'ai plus rien à respirer. C'est l'heure des affaires. Je vous aime beaucoup..."

# game/events/other.rpy:625
translate french showdown_with_eric_90be8708:

    # Lisa_05 "И мы тебя!"
    Lisa_05 "Et nous sommes vous!"

# game/events/other.rpy:649
translate french dictaphone_showdown_with_eric_ff0dd3e4:

    # Ann_13 "Макс! Лиза! Алиса! Ну зачем так неожиданно влетать?! Так и заикой стать не долго..."
    Ann_13 "Max! Lisa! Alice! Pourquoi voler si soudainement?! Et le bégaiement ne sera pas long..."

# game/events/other.rpy:650
translate french dictaphone_showdown_with_eric_6110e6be:

    # Lisa_13 "Извини, мама... Но сейчас кто-то действительно станет заикой! Ты хоть знаешь, какие планы строит Эрик?"
    Lisa_13 "Désolé, maman... Mais maintenant, quelqu'un va vraiment devenir un bégaiement! Tu sais ce que fait Eric?"

# game/events/other.rpy:651
translate french dictaphone_showdown_with_eric_c78bbf14:

    # Ann_15 "Это что за тон, Лиза?! И что это за вопрос? Что происходит?!"
    Ann_15 "c'est quoi ce ton, Lisa?! Et quelle est cette question? Qu'est-ce qui se passe ?!"

# game/events/other.rpy:652
translate french dictaphone_showdown_with_eric_16776269:

    # Alice_13 "Мам, я понятия не имею... Это всё они."
    Alice_13 "maman, je n'en ai aucune idée... C'est eux tous."

# game/events/other.rpy:653
translate french dictaphone_showdown_with_eric_4f2b686c:

    # Max_09 "Лиза тут кое-что узнала... И теперь у неё есть претензии к Эрику..."
    Max_09 "Lisa a appris quelque chose... Et maintenant, elle a des revendications sur Eric..."

# game/events/other.rpy:663
translate french dictaphone_showdown_with_eric_b894aaee:

    # Lisa_12 "И ещё какие! Мам, помнишь наши совместные уроки в твоей комнате? Так вот Эрик решил расширить программу моего сексуального образования до практических занятий с ним наедине!"
    Lisa_12 "et plus encore! Maman, tu te souviens de nos cours ensemble dans ta chambre? Donc Eric a décidé d'étendre mon programme d'éducation sexuelle à des exercices pratiques avec lui seul!"

# game/events/other.rpy:664
translate french dictaphone_showdown_with_eric_cf4823cd:

    # Ann_17 "Я ни о чём таком не знаю. В каком смысле, практических?"
    Ann_17 "Je ne sais rien de ça. Comment ça, pratique?"

# game/events/other.rpy:665
translate french dictaphone_showdown_with_eric_6a9d0065:

    # Lisa_10 "В таком, чтобы я повторяла всё то, что делала ты. Это как вообще называется? Я думала у нас Макс извращенец, но ты, Эрик..."
    Lisa_10 "Pour que je répète ce que tu as fait. C'est généralement appelé? Je pensais que Max était un pervers, mais toi, Eric..."

# game/events/other.rpy:666
translate french dictaphone_showdown_with_eric_cefb4f10:

    # Ann_16 "Ты что такое вообще несёшь, Лиза?! Обвинять в таком Эрика, это... Я уверена, ты просто что-то не так поняла. Верно, Эрик?"
    Ann_16 "qu'est-ce que tu portes, Lisa?! Blâmer Eric pour ça, c'est ça... Je suis sûre que tu as mal compris. C'Est Ça, Eric?"

# game/events/other.rpy:667
translate french dictaphone_showdown_with_eric_26987fb2:

    # Eric_13 "Однозначно!"
    Eric_13 "sans Équivoque!"

# game/events/other.rpy:668
translate french dictaphone_showdown_with_eric_4d03abad:

    # Lisa_09 "Мам, с каких пор ты мне не веришь? Макс специально ходил разговаривать с Эриком и Эрик таких мерзостей ему наговорил про всех... Особенно, про меня!"
    Lisa_09 "maman, Depuis quand tu ne me crois pas? Max est allé parler à Eric et Eric lui a parlé de ces abominations à propos de tout le monde... Surtout à propos de moi!"

# game/events/other.rpy:669
translate french dictaphone_showdown_with_eric_1f1b1dd8:

    # Ann_19 "Что?! Это уже явный перебор, Лиза! Мне или нужны доказательства, или тебя придётся хорошенько наказать!"
    Ann_19 "Quoi?! C'est un peu exagéré, Lisa! Moi ou avez besoin d'une preuve, ou vous aurez une bonne punir!"

# game/events/other.rpy:670
translate french dictaphone_showdown_with_eric_02d9ccd0:

    # Eric_09 "Так, Лиза, давай успокаивайся! Ань, я сейчас поговорю с ней наедине. Макс видимо совсем страх потерял, своей младшей сестрёнке каких-то сказок понарассказывал."
    Eric_09 "alors, Lisa, calmez-vous! Anh, je vais lui parler en privé. Max a apparemment complètement perdu la peur,sa petite soeur a raconté des contes de fées."

# game/events/other.rpy:675
translate french dictaphone_showdown_with_eric_8684e2a4:

    # Lisa_13 "Не собираюсь я с тобой наедине больше оставаться! Макс всё записал на диктофон в телефоне. Сейчас послушаем..."
    Lisa_13 "Je ne vais plus rester seul avec toi! Max a tout enregistré sur l'enregistreur de son téléphone. On va l'écouter..."

# game/events/other.rpy:676
translate french dictaphone_showdown_with_eric_60fce6d7:

    # "{color=[orange]}(все слушают запись...){/color}"
    "{color =[orange]} (tout le monde écoute l'enregistrement...){/color}"

# game/events/other.rpy:677
translate french dictaphone_showdown_with_eric_1c22e674:

    # Ann_14 "Это же... Неужели это действительно правда?! Эрик, будь добр, объясни это..."
    Ann_14 "C'est pareil... Est-ce vraiment vrai?! Eric, explique-moi ça..."

# game/events/other.rpy:678
translate french dictaphone_showdown_with_eric_3b4d9713:

    # Eric_13 "Ань, я не знаю, что и где это записано, но могу тебя уверить - это неправда! Уверен на все сто, что эта запись подделка."
    Eric_13 "Anh, je ne sais pas quoi et où c'est écrit, mais je peux vous assurer que ce n'est pas vrai! Je suis sûr que ce disque est faux."

# game/events/other.rpy:679
translate french dictaphone_showdown_with_eric_8dffdb7a:

    # Max_09 "В таком случае, я за то, чтобы провести экспертизу её подлинности!"
    Max_09 "dans ce cas, je suis pour que l'examen de leur authenticité!"

# game/events/other.rpy:686
translate french dictaphone_showdown_with_eric_c1994ec4:

    # Alice_18 "Ах вот так, значит! Ты свои ручищи к нашей маленькой Лизе уже тянешь? Вот... получай, скотина..."
    Alice_18 "ah! Tu Tires tes mains sur notre petite Lisa? Voici... prends ça, connard..."

# game/events/other.rpy:687
translate french dictaphone_showdown_with_eric_8aba10b5:

    # Ann_15 "Эрик! У меня просто в голове не укладывается... Как ты мог на такое решиться, мерзавец... Лиза, неужели это действительно правда?"
    Ann_15 "Eric! Je ne me sens pas bien dans ma tête... Comment as-tu pu faire ça, salaud?.. Lisa, est-ce vraiment vrai?"

# game/events/other.rpy:688
translate french dictaphone_showdown_with_eric_1d2fc481:

    # Max_07 "Разве это не очевидно, мам? Или ты думаешь, что мы тебя так жестоко разыграть решили?"
    Max_07 "n'est-ce pas évident, maman? Ou tu penses qu'on t'a si cruellement joué?"

# game/events/other.rpy:689
translate french dictaphone_showdown_with_eric_0d5ce9a0:

    # Ann_20 "Эрик, ты совсем что ли больной?! Как ты мог, строя отношения со мной, ещё и планировать развращать моих детей! Живо выметайся из нашего дома, пока я полицию не вызвала!"
    Ann_20 "Eric, es-tu malade?! Comment as-tu pu, en construisant une relation avec moi, planifier aussi de corrompre mes enfants! Sors de chez nous avant que j'appelle la police!"

# game/events/other.rpy:690
translate french dictaphone_showdown_with_eric_847ca8ec:

    # Eric_10 "Какую ещё полицию?! Я ничего не делал. Это всё Макс!"
    Eric_10 "Quelle autre police?! Je n'ai rien fait. C'est Max!"

# game/events/other.rpy:691
translate french dictaphone_showdown_with_eric_8cb800af:

    # Lisa_13 "Не надо всё на Макса сваливать! Он хороший..."
    Lisa_13 "ne va pas tout foutre sur Max! Est-il bon..."

# game/events/other.rpy:701
translate french dictaphone_showdown_with_eric_2a7e5ec0:

    # Alice_16 "Совсем недавно я могла от души навалять Максу, но сейчас я в такой ярости, что запросто наваляю такому здоровому извращенцу, как ты!"
    Alice_16 "tout récemment, j'ai pu faire tomber Max de mon cœur, mais maintenant je suis tellement furieux que j'empoche facilement un pervers en bonne santé comme toi!"

# game/events/other.rpy:702
translate french dictaphone_showdown_with_eric_aece1638:

    # Ann_19 "Эрик, проваливай! Я не хочу тебя больше видеть!"
    Ann_19 "Eric, dégage! Je ne veux plus te revoir!"

# game/events/other.rpy:703
translate french dictaphone_showdown_with_eric_beb7cd04:

    # Eric_09 "Вижу, вы все на взводе, так что я поговорю с вами, когда успокоитесь... А сейчас я, пожалуй, отправлюсь к себе домой."
    Eric_09 "je Vois que vous êtes tous en état d'alerte, alors je vous parlerai quand vous vous calmerez... Et maintenant, je vais peut-être retourner chez lui."

# game/events/other.rpy:704
translate french dictaphone_showdown_with_eric_2ec75356:

    # Ann_20 "Я сказала, {b}ПОШЁЛ ВОН НЕМЕДЛЕННО!!!{/b}"
    Ann_20 "j'ai dit, {b}SORS TOUT de suite!!!{/b}"

# game/events/other.rpy:705
translate french dictaphone_showdown_with_eric_33de351b:

    # Alice_17 "Убирайся из нашей жизни, грязное животное!"
    Alice_17 "sors de notre vie, sale animal!"

# game/events/other.rpy:714
translate french dictaphone_showdown_with_eric_f3afe214:

    # Max_00 "Мам, ты только не расстраивайся так из-за него. Всякое бывает..."
    Max_00 "Maman, ne te fâche pas avec lui. Tout arrive..."

# game/events/other.rpy:715
translate french dictaphone_showdown_with_eric_346fcb3b:

    # Ann_14 "Всё никак в голове не укладывается, что Эрик тут с вами собирался творить... И что я за мать после этого? Я в шоке..."
    Ann_14 "Ce n'est pas dans ma tête ce qu'Eric allait faire avec vous... Et quelle mère suis-je après ça? Je suis choquée..."

# game/events/other.rpy:716
translate french dictaphone_showdown_with_eric_90ee4fd2:

    # Alice_13 "Уж лучше узнать поздно, чем никогда. А у нас всё ещё лучше... Мы узнали, пока ещё не стало поздно. Так что не переживай, мам, мы все думали, что Эрик... уж точно не такой..."
    Alice_13 "mieux vaut savoir tard que jamais. Et nous sommes toujours meilleurs... Nous avons appris avant qu'il ne soit trop tard. Alors ne t'inquiète pas, maman, on pensait tous Eric... certainement pas comme ça..."

# game/events/other.rpy:717
translate french dictaphone_showdown_with_eric_167ce82b:

    # Max_09 "Хорошо, что я своим мужским взглядом сразу понял, что он мутный тип!"
    Max_09 "c'est Bien que j'ai immédiatement compris avec mon regard masculin qu'il était un type boueux!"

# game/events/other.rpy:718
translate french dictaphone_showdown_with_eric_382d50ea:

    # Lisa_09 "И между прочим, про бумажник Эрик наврал..."
    Lisa_09 "et au fait, le portefeuille d'Eric a menti..."

# game/events/other.rpy:719
translate french dictaphone_showdown_with_eric_28be9f6a:

    # Max_08 "Кстати, да! Он это придумал, чтобы вы меня в военный лагерь сослали. А то я стал слишком мешаться."
    Max_08 "au Fait, oui! Il a inventé ça pour que vous m'exiliez dans un camp militaire. Je suis devenu trop gêné."

# game/events/other.rpy:720
translate french dictaphone_showdown_with_eric_35096e7a:

    # Ann_13 "Ты прости меня, сынок, я должна была прислушиваться к тому, что ты говорил. Что же теперь делать?"
    Ann_13 "pardonne-moi, fiston, j'aurais dû écouter ce que tu disais. Que faire maintenant?"

# game/events/other.rpy:721
translate french dictaphone_showdown_with_eric_e862c93b:

    # Max_01 "Жить, как жили до Эрика! Ничего сложного..."
    Max_01 "Vivre comme avant Eric! Rien de compliqué..."

# game/events/other.rpy:722
translate french dictaphone_showdown_with_eric_6ff07095:

    # Ann_12 "Нет, Макс, я о другом... Он же мой начальник! Как мне теперь быть? Ох, не знаю..."
    Ann_12 "non, Max, je parle d'autre chose... C'est mon patron! Comment puis-je maintenant être? Oh, je ne sais pas..."

# game/events/other.rpy:723
translate french dictaphone_showdown_with_eric_87eb1c15:

    # Max_07 "Д-а-а... Это проблемка. Ты главное не слушай его оправдания, а то он умеет убеждать, как ты сама прекрасно убедилась. А теперь давайте, все вместе, поддержим маму!"
    Max_07 "D-a-A... C'est un poseur. Tu n'écoutes pas ses excuses, sinon il sait convaincre, comme tu l'as parfaitement convaincu. Maintenant, tous ensemble, soutenons maman!"

# game/events/other.rpy:731
translate french dictaphone_showdown_with_eric_f96123e9:

    # Ann_13 "Ой, дорогие мои, вы меня так раздавите! Но вы всё равно самые лучшие, кто у меня есть... Простите ещё раз за то, что я привела в наш прекрасный дом этого человека и всё испортила."
    Ann_13 "Oh, mes chers, vous me écrasez tellement! Mais vous êtes toujours les meilleurs que j'ai... Pardonnez-moi encore d'avoir amené cet homme dans notre belle maison et d'avoir tout gâché."

# game/events/other.rpy:732
translate french dictaphone_showdown_with_eric_5a9bcc35:

    # Lisa_01 "Не бери в голову, мам. Не вини себя, всё ведь хорошо. Мы с тобой."
    Lisa_01 "N'y pense pas, maman. Ne te blâme pas, tout va bien. Nous sommes avec toi."

# game/events/other.rpy:733
translate french dictaphone_showdown_with_eric_a2587239:

    # Alice_05 "И если он начнёт устраивать тебе проблемы на работе, то мы все так же дружно, как сейчас, соберёмся и отпинаем его!"
    Alice_05 "et s'il commence à vous faire des problèmes au travail, alors nous sommes toujours aussi amicaux que maintenant, nous nous réunirons et le lâcherons!"

# game/events/other.rpy:734
translate french dictaphone_showdown_with_eric_e1846793:

    # Max_09 "Думаю, Эрик не настолько туп, чтобы строить маме козни. Особенно, если она будет с ним жёстче..."
    Max_09 "je Pense qu'Eric n'est pas assez stupide pour construire un stratagème à maman. Surtout si elle est plus dure avec lui..."

# game/events/other.rpy:735
translate french dictaphone_showdown_with_eric_d14050f1:

    # Ann_17 "Ох, ребята, я постараюсь... Хотя, мне даже думать о нём сейчас противно, не то что видеть. Надеюсь, я не потеряю работу."
    Ann_17 "Oh, les gars, je vais essayer... Bien que je ne pense même pas à lui maintenant, ce n'est pas quelque chose que je vois. J'espère ne pas perdre mon travail."

# game/events/other.rpy:736
translate french dictaphone_showdown_with_eric_72f9e7fc:

    # Max_08 "Может, удастся перевестись в какой-нибудь другой отдел?"
    Max_08 "peut-on être transféré dans un autre département?"

# game/events/other.rpy:737
translate french dictaphone_showdown_with_eric_032de050:

    # Ann_02 "Надеюсь... Ну всё, мне уже дышать нечем. Пора делами заниматься. Я вас очень люблю..."
    Ann_02 "j'Espère... Je n'ai plus rien à respirer. C'est l'heure des affaires. Je vous aime beaucoup..."

# game/events/other.rpy:738
translate french dictaphone_showdown_with_eric_90be8708:

    # Lisa_05 "И мы тебя!"
    Lisa_05 "Et nous sommes vous!"

# game/events/other.rpy:818
translate french failed_dinner_b8e2672f:

    # Eric_00 "Итак, настал момент истины. Я дал тебе, Макс, достаточно времени, чтобы ты вернул мне мои деньги при всех. Ну что, ты готов покаяться и исправить свою ошибку?"
    Eric_00 "donc, le moment de vérité est venu. Je t'ai donné, Max, assez de temps pour que Tu me rendes mon argent avec tout le monde. Tu es prêt à te repentir et à corriger ton erreur?"

# game/events/other.rpy:822
translate french failed_dinner_d3811ad8:

    # Max_15 "Да пошёл ты... Нет у меня твоих денег, и ты это прекрасно знаешь!"
    Max_15 "va te faire foutre... Je n'ai pas ton argent, et tu le sais très bien!"

# game/events/other.rpy:825
translate french failed_dinner_851dce21:

    # Max_15 "Да пошёл ты... Я ни за что не стану возвращать тебе то, чего не брал! И признавать себя вором тоже не стану!"
    Max_15 "va te faire foutre... Je ne te rendrai jamais ce que je n'ai pas pris! Et je ne vais pas me reconnaître comme un voleur non plus!"

# game/events/other.rpy:827
translate french failed_dinner_2c4314ba:

    # Eric_09 "Что ж, значит продолжаем отпираться... Ладно... Как ты видишь, Ань, я сделал всё, что было в моих силах, но это твой сын и тебе принимать решение."
    Eric_09 "alors on continue à se défaire... D'accord... Comme tu vois, Anh, j'ai fait tout ce qui était en mon pouvoir, mais c'est ton fils et c'est à toi de décider."

# game/events/other.rpy:828
translate french failed_dinner_e02fe136:

    # Ann_14 "Да-да... Прости, Макс, но ты очень меня разочаровал и... нам не остаётся другого выбора, кроме как отправить тебя в военный лагерь. Надеюсь, там из тебя сделают ответственного человека."
    Ann_14 "Oui... Désolé, Max, mais tu m'as vraiment déçu... nous n'avons pas d'autre choix que de vous envoyer dans un camp militaire. J'espère que ça fera de toi une personne responsable."

# game/events/other.rpy:829
translate french failed_dinner_03357d46:

    # Max_13 "Что?! Кому это, \"нам\"? Вам с Эриком? Это же не ты сейчас говоришь, а он! Как ты можешь выгонять из дома родного сына?"
    Max_13 "Quoi?! Qui est-ce, \"nous\"? Toi et Eric? Ce n'est pas toi qui parle, c'est lui! Comment peux-tu expulser ton fils de chez toi?"

# game/events/other.rpy:830
translate french failed_dinner_d29fb4a6:

    # Ann_17 "Прекрати, Макс! Я устала слушать твои оправдания. Ты даже не представляешь, до какой степени мне стыдно, что мой единственный сын - вор!"
    Ann_17 "Arrête, Max! J'en ai marre d'entendre tes excuses. Tu ne sais même pas à quel point j'ai honte que mon fils unique soit un voleur!"

# game/events/other.rpy:831
translate french failed_dinner_c43f223d:

    # Max_14 "Но мам, это всё неправда! Не говори так! Это же Эрик всё подстроил!"
    Max_14 "mais maman, ce n'est pas vrai! Ne dis pas ça! C'est Eric qui a tout arrangé!"

# game/events/other.rpy:832
translate french failed_dinner_5a9be7a6:

    # Eric_13 "Да, стыдно за тебя, Макс, ты разбиваешь сердце собственной матери... Какой позор..."
    Eric_13 "oui, j'ai honte de toi, Max, tu brises le cœur de ta mère... Quelle honte..."

# game/events/other.rpy:833
translate french failed_dinner_c7cc51b7:

    # Max_16 "Заткнись, Эрик! Мам, неужели ты не видишь, что он тобой манипулирует?!"
    Max_16 "Tais-Toi, Eric! Maman, tu ne vois pas qu'il te manipule?!"

# game/events/other.rpy:834
translate french failed_dinner_81894a93:

    # Ann_18 "Не хочу ничего слышать про это. Всё. Собираешь вещи и едешь. Давно надо было это сделать, но я тебя всё жалела. В результате докатились, воруешь в собственном доме! А что будет дальше?"
    Ann_18 "Je ne veux rien entendre à ce sujet. C'est tout. Tu fais tes bagages et tu conduis. J'aurais dû le faire il y a longtemps, mais j'ai eu pitié de toi. En conséquence, vous volez dans votre propre maison! Qu'est-ce qui va se passer ensuite?"

# game/events/other.rpy:835
translate french failed_dinner_ffb1b7ea:

    # Max_11 "Но мам..."
    Max_11 "Mais maman..."

# game/events/other.rpy:836
translate french failed_dinner_b3c54058:

    # Ann_16 "Всё, Макс, разговор окончен!"
    Ann_16 "tout le monde, Max, la conversation est terminée!"

# game/events/other.rpy:838
translate french failed_dinner_4d7ce5c3:

    # Eric_01 "Не переживай, Макс, я позабочусь обо всех, пока ты будешь тянуть лямку в армии..." nointeract
    Eric_01 "ne t'inquiète pas, Max, Je m'occuperai de tout le monde pendant que tu Tires la sangle dans l'armée..." nointeract

# game/events/other.rpy:845
translate french failed_dinner_fe32100d:

    # Max_14 "{m}Вот так и закончилась моя история. Больше мне не верили и не хотели меня видеть. Конечно, если бы не Эрик, всё было бы хорошо, но этот гад... Теперь ему ничто не помешает сделать с моей семьёй всё, что он задумал... Вот если бы я мог вернуться в прошлое... Но есть ли в этом смысл?{/m}" nointeract
    Max_14 "{m}c'est ainsi que mon histoire s'est terminée. Ils ne me croyaient plus et ne voulaient plus me voir. Bien sûr, sans Eric, tout serait bien, mais ce connard... Maintenant, rien ne l'empêche de faire ce qu'il a prévu avec ma famille... Si seulement je pouvais revenir dans le passé... Mais cela a-t-il un sens?{/m}" nointeract

# game/events/other.rpy:852
translate french failed_dinner_79acb07c:

    # Max_14 "{m}Вот так и закончилась моя история. От всей этой несправедливости я немного слетел с катушек и почти размазал Эрика по балкону... Почти... Теперь ему ничто не помешает сделать с моей семьёй всё, что он задумал... Вот если бы я мог вернуться в прошлое... Но есть ли в этом смысл?{/m}" nointeract
    Max_14 "{m}c'est ainsi que mon histoire s'est terminée. De toute cette injustice, je me suis un peu envolé et j'ai presque barbouillé Eric sur le balcon... Presque... Maintenant, rien ne l'empêche de faire ce qu'il a prévu avec ma famille... Si seulement je pouvais revenir dans le passé... Mais cela a-t-il un sens?{/m}" nointeract

# game/events/other.rpy:869
translate french left_cream_68a6f9b5:

    # Max_10 "{m}Ну вот, крем закончился. Надо ещё купить.{/m}"
    Max_10 "{m}Eh bien, la crème est finie. Nous devons en acheter plus.{/m}"

# game/events/other.rpy:875
translate french left_cream_61f00467:

    # Max_08 "{m}Осталось мало крема, в следующий раз может не хватить, лучше купить заранее.{/m}"
    Max_08 "{m}Il reste peu de crème, la prochaine fois que ce ne sera peut-être pas suffisant, il est préférable d’en acheter à l’avance.{/m}"

translate french strings:

    # game/events/other.rpy:10
    old "Удачи"
    new "Bonne chance"

    # game/events/other.rpy:40
    old "Да, жаль, что не купили его..."
    new "Oui, c'est dommage que je ne l'ai pas acheté ..."

    # game/events/other.rpy:40
    old "Я бы с удовольствием на это посмотрел..."
    new "J’adorerais regarder ça..."

    # game/events/other.rpy:48
    old "Слишком дорогой это сколько?"
    new "Est-ce trop cher?"

    # game/events/other.rpy:48
    old "Насколько открытые?"
    new "Comment ouvert?"

    # game/events/other.rpy:59
    old "Верно, сами разберёмся без Эрика"
    new "C’est vrai, découvrons-le sans Eric"

    # game/events/other.rpy:59
    old "Кстати, где он?"
    new "Au fait, où est-il ?"

    # game/events/other.rpy:123
    old "Очень приятно..."
    new "Très bien..."

    # game/events/other.rpy:123
    old "Я думал он выше..."
    new "Je pensais qu'il était au-dessus ..."

    # game/events/other.rpy:128
    old "Всё отлично. Ищу себя, так сказать..."
    new "Tout va bien. Je le cherche, pour ainsi dire..."

    # game/events/other.rpy:128
    old "Да то тем, то этим..."
    new "Oui, alors..."

    # game/events/other.rpy:128
    old "Отвечаю вот на дурацкие вопросы..."
    new "Je réponds à des questions stupides..."

    # game/events/other.rpy:149
    old "Стараюсь больше слушать..."
    new "J’essaie d’écouter plus..."

    # game/events/other.rpy:149
    old "Типа того..."
    new "Comme ça..."

    # game/events/other.rpy:149
    old "Да было бы о чём говорить..."
    new "Oui, il y aurait de quoi parler..."

    # game/events/other.rpy:170
    old "Да, мне тоже. Извините..."
    new "Oui moi aussi. Désolé..."

    # game/events/other.rpy:170
    old "А мне вот не очень..."
    new "Et je ne suis pas très ..."

    # game/events/other.rpy:187
    old "Да мне всё равно..."
    new "Je m’en fous..."

    # game/events/other.rpy:198
    old "{i}идти к столу{/i}"
    new "{i}aller à la table{/i}"

    # game/events/other.rpy:218
    old "Извини, Эрик, я просто не в духе..."
    new "Désolé, Eric, je ne suis tout simplement pas dans présent dans ma tête..."

    # game/events/other.rpy:218
    old "Нормально себя веду..."
    new "Je me comporte normalement ..."

    # game/events/other.rpy:245
    old "Здравствуй, тётя Кира!"
    new "Bonjour, tante Kira!"

    # game/events/other.rpy:245
    old "Что значит почти? Я настоящий мужчина!"
    new "Qu'est-ce qui signifie presque? Je suis un vrai homme!"

    # game/events/other.rpy:260
    old "А мне можно тоже обнять свою тётю?"
    new "Et je peux aussi embrasser ma tante?"

    # game/events/other.rpy:269
    old "В гостинице?"
    new "À l'hôtel?"

    # game/events/other.rpy:269
    old "Дела?"
    new "À faire?"

    # game/events/other.rpy:269
    old "Так ты два дня здесь?"
    new "Donc tues ici pour deux jours?"

    # game/events/other.rpy:299
    old "Да не стоит..."
    new "Pas de quoi..."

    # game/events/other.rpy:299
    old "Хорошо, тётя Кира!"
    new "Très bien, tante Kira!"

    # game/events/other.rpy:313
    old "Выглядит отлично!"
    new "Ça a l’air génial!"

    # game/events/other.rpy:313
    old "Ну-ка, повернись..."
    new "Eh bien, retournes-toi..."

    # game/events/other.rpy:326
    old "А что такого? Пусть ходит, я не против..."
    new "Quel est le problème ? Laisses-le partir, ça ne me dérange pas..."

    # game/events/other.rpy:340
    old "Нет, мам, всё отлично!"
    new "Non, maman, ça va!"

    # game/events/other.rpy:340
    old "А можешь немного повернуться?"
    new "Tu peux te retourner un peu ?"

    # game/events/other.rpy:347
    old "Ты знаешь, длина оптимальная!"
    new "Tu sais, la longueur est optimale !"

    # game/events/other.rpy:347
    old "Мне нравится, мам!"
    new "J'adore ça, maman !"

    # game/events/other.rpy:358
    old "Мне нравится. Просто и со вкусом!"
    new "Je l'aime bien. Simple et de bon goût !"

    # game/events/other.rpy:358
    old "Отличные... вещички!"
    new "Super... ces choses !"

    # game/events/other.rpy:358
    old "И почти ничего не видно..."
    new "Et on ne voit presque rien..."

    # game/events/other.rpy:442
    old "{i}подставить Эрика{/i}"
    new "{i}remplacer Eric {/i}"

    # game/events/other.rpy:449
    old "{i}осторожно дрочить на Алису{/i}"
    new "{i}doucement se branler sur Alice {/i}"

    # game/events/other.rpy:456
    old "{i}кончить Алисе на ноги{/i}"
    new "{i}sperme sur Alice sur ses pieds {/i}"

    # game/events/other.rpy:463
    old "{i}осторожно идти спать{/i}"
    new "{i}allez doucement au lit{/i}"

    # game/events/other.rpy:808
    old "{g}Отношения Макса с семьёй значительно улучшились.{/g}"
    new "{g}la Relation de Max avec sa famille s'est considérablement améliorée.{/g}"

    # game/events/other.rpy:837
    old "{i}собираться в военный лагерь{/i}"
    new "{i}se rassembler dans un camp militaire {/i}"

    # game/events/other.rpy:837
    old "{i}накинуться на Эрика{/i}"
    new "{i}jeter sur Eric {/i}"

    # game/events/other.rpy:854
    old "{i}загрузить игру{/i}"
    new "{i}télécharger le jeu {/i}"

    # game/events/other.rpy:854
    old "{i}вернуться в главное меню{/i}"
    new "{i}retour au menu principal {/i}"

    # game/events/other.rpy:854
    old "{i}выйти из игры{/i}"
    new "{i}quitter le jeu {/i}"

