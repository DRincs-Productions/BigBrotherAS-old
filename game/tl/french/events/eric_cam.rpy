# TODO: Translation updated at 2022-07-09 12:49

# game/events/eric_cam.rpy:13
translate french cam0_eric_ann_sleep_b363be42:

    # Max_07 "{m}Как же повезло, что у меня такая горячая мама... Стойте-ка, а Эрик где?! Может, пошёл в ванную комнату? Или ещё куда...{/m}"
    Max_07 "{m}quelle chance d'avoir une maman si chaude... Attends, où est Eric?! Peut, est allé à la salle de bains? Ou encore où...{/m}"

# game/events/eric_cam.rpy:16
translate french cam0_eric_ann_sleep_c5aa1a5f:

    # Max_07 "{m}Ага! Мама одна... Значит, Эрик почти наверняка сейчас дрочит на Алису. Нужно проверить...{/m}"
    Max_07 "{m}Aha! Maman est seule... Donc, Eric est presque sûr de se branler sur Alice en ce moment. Je dois vérifier...{/m}"

# game/events/eric_cam.rpy:28
translate french cam0_eric_ann_sleep_73c88870:

    # Max_07 "{m}Как же повезло, что у меня такая горячая мама... А Эрик опять где-то полуночничает...{/m}"
    Max_07 "{m}quelle chance d'avoir une maman si chaude... Eric est encore minuit...{/m}"

# game/events/eric_cam.rpy:33
translate french cam0_eric_ann_sleep_97963875:

    # Max_01 "{m}Как же повезло, что у меня такая горячая мама... Выглядит потрясающе, аж глаза отрывать не хочется!{/m}"
    Max_01 "{m}quelle chance d'avoir une maman si chaude... Ça a l'air incroyable, je ne veux pas arracher les yeux!{/m}"

# game/events/eric_cam.rpy:37
translate french cam0_eric_ann_sleep_84da07d8:

    # Max_01 "{m}Мама и Эрик спят совсем голые. Ни стыда, ни совести...{/m}"
    Max_01 "{m}Maman et Eric dorment complètement nus. Pas de honte, pas de conscience...{/m}"

# game/events/eric_cam.rpy:47
translate french cam0_eric_ann_shower_f2cae77e:

    # Max_09 "{m}Мамы и Эрика не видно через эту камеру... Может посмотреть через другую?{/m}"
    Max_09 "{m}Maman et Eric ne sont pas visibles à travers cette caméra... Peut-être regarder à travers l'autre?{/m}"

# game/events/eric_cam.rpy:49
translate french cam0_eric_ann_shower_25391a36:

    # Max_09 "{m}Мамы и Эрика не видно через эту камеру...{/m}"
    Max_09 "{m}Maman et Eric ne sont pas visibles à travers cette caméra...{/m}"

# game/events/eric_cam.rpy:75
translate french cam0_eric_ann_shower_5424f711:

    # Max_07 "{m}Вот это да... Похоже намечается что-то большее, чем просто принять душ!{/m}"
    Max_07 "{m}C'est ça... On dirait qu'il y a plus qu'une douche!{/m}"

# game/events/eric_cam.rpy:80
translate french cam0_eric_ann_shower_03b39878:

    # Max_08 "{m}Да уж, устроился Эрик хорошо... Мама отсасывает ему с таким наслаждением, аж оторваться не может!{/m}"
    Max_08 "{m}oui, Eric s'est bien installé... Maman lui suce avec un tel plaisir, déjà ne peut pas se détacher!{/m}"

# game/events/eric_cam.rpy:83
translate french cam0_eric_ann_shower_aa6dfd62:

    # Max_04 "{m}Охх... Вот же Эрику повезло... Ведь у мамы такие нежные и ласковые руки!{/m}"
    Max_04 "{m}Ohh... Eric a eu de la chance... Après tout, ma mère a des mains si douces et affectueuses!{/m}"

# game/events/eric_cam.rpy:86
translate french cam0_eric_ann_shower_709adb56:

    # Max_06 "{m}Охренеть! Вот это страсть! Они так увлечены друг другом... И похоже, маме это очень нравится!{/m}"
    Max_06 "{m}Putain! C'est une passion! Ils sont tellement passionnés l'un par l'autre... Et on dirait que maman aime ça!{/m}"

# game/events/eric_cam.rpy:89
translate french cam0_eric_ann_shower_4746ba6b:

    # Max_05 "{m}Ого! Эрик трахает маму сзади, да так активно... И... кажется, ей это очень нравится, она даже двигается ему навстречу... и изнывает от страсти!{/m}"
    Max_05 "{m}Wow! Eric baise maman par derrière, oui si activement... Et... il semble qu'elle l'aime beaucoup, elle se déplace même à sa rencontre... et il est épuisé par la passion!{/m}"

# game/events/eric_cam.rpy:94
translate french cam0_eric_ann_shower_4d366672:

    # Max_09 "{m}Вот чёрт! Эрик кончает маме прямо на лицо, как в каком-то порно! Причём, ей это настолько нравится, что она улыбается и ловит его сперму своим ртом!{/m}"
    Max_09 "{m}Voici les caractéristiques de la! Eric éjacule maman directement sur son visage comme dans du porno! Et, elle l'aime tellement qu'elle sourit et attrape son sperme avec sa bouche!{/m}"

# game/events/eric_cam.rpy:97
translate french cam0_eric_ann_shower_df0cc161:

    # Max_01 "{m}Ну да! Кто бы сомневался, что Эрик не продержится слишком долго. Мама своё дело знает!{/m}"
    Max_01 "{m}eh Bien, oui! Qui aurait douté qu'Eric ne durerait pas trop longtemps. Maman sait ce que c'est!{/m}"

# game/events/eric_cam.rpy:100
translate french cam0_eric_ann_shower_c373b66f:

    # Max_07 "{m}Ох, чёрт... Эрик уже кончил... Хорошо, что не в маму... Счастливый сукин сын... И она ещё улыбается?!{/m}"
    Max_07 "{m}Oh, merde... Eric a fini... C'est bien de ne pas aller chez maman... Fils de pute chanceux... Et elle sourit encore?!{/m}"

# game/events/eric_cam.rpy:103
translate french cam0_eric_ann_shower_09186916:

    # Max_08 "{m}Чёрт возьми... он уже кончил... Счастливый ублюдок... забрызгал маме всю спину с попкой своей спермой!{/m}"
    Max_08 "{m}bon sang... il a fini... Joyeux enfoiré... éclaboussé maman tout le dos avec son cul avec son sperme!{/m}"

# game/events/eric_cam.rpy:120
translate french cam1_eric_ann_shower_cea35296:

    # Max_07 "{m}Охх... Боже мой, какие нежности. Похоже, сейчас что-то начнётся...{/m}"
    Max_07 "{m}Ohh... Oh mon Dieu, quelle tendresse. On dirait que quelque chose va commencer...{/m}"

# game/events/eric_cam.rpy:123
translate french cam1_eric_ann_shower_7b4dc000:

    # Max_10 "{m}Моя мама снова отсасывает этому... Эрику! Да с такой страстью! Ей что, действительно так нравится это делать или она его настолько любит?{/m}"
    Max_10 "{m}Ma mère le suce encore... Eric! Oui avec une telle passion! Est-ce qu'elle aime vraiment faire ça ou est-ce qu'elle l'aime tellement?{/m}"

# game/events/eric_cam.rpy:126
translate french cam1_eric_ann_shower_8267cfe3:

    # Max_08 "{m}О Боже! Как бы я мечтал оказаться на месте это счастливого ублюдка! Когда её мокрая попка так красиво скачет на члене, голова начинает идти кругом!{/m}"
    Max_08 "{m}Oh Mon Dieu! Comme si j'avais rêvé d'être à la place de cet heureux bâtard! Quand son cul humide chevauche si bien la bite, sa tête commence à tourner!{/m}"

# game/events/eric_cam.rpy:129
translate french cam1_eric_ann_shower_fb18430e:

    # Max_10 "{m}Ничего себе! Вот это они вытворяют! Эрик трахает маму, разложив её у зеркала как какую-то шлюшку...{/m}"
    Max_10 "{m}wow! C'est ce qu'ils se lever! Eric baise maman en l'étalant près du miroir comme une salope...{/m}"

# game/events/eric_cam.rpy:132
translate french cam1_eric_ann_shower_0a7c41ee:

    # Max_08 "{m}Чёрт возьми! Она приняла всю его сперму себе в рот и на лицо, и теперь с такой жадностью и удовольствием слизывает её с его члена...{/m}"
    Max_08 "{m}bon sang! Elle a pris tout son sperme dans sa bouche et sur son visage, et maintenant, avec tant de cupidité et de plaisir, la lèche de sa bite...{/m}"

# game/events/eric_cam.rpy:135
translate french cam1_eric_ann_shower_ae2539a9:

    # Max_10 "{m}А вот и финал не заставил себя ждать! И похоже всем всё понравилось, как и всегда...{/m}"
    Max_10 "{m}et voici la finale ne s'est pas fait attendre! Et il semble que tout le monde a aimé, comme toujours...{/m}"

# game/events/eric_cam.rpy:140
translate french cam1_eric_ann_shower_f2cae77e:

    # Max_09 "{m}Мамы и Эрика не видно через эту камеру... Может посмотреть через другую?{/m}"
    Max_09 "{m}Maman et Eric ne sont pas visibles à travers cette caméra... Peut-être regarder à travers l'autre?{/m}"

# game/events/eric_cam.rpy:149
translate french cam0_eric_resting_b4d3f901:

    # Max_01 "{m}Кажется, подмешанное Эрику в еду средство испортило ему всё настроение и он не рискует идти к моей сестрёнке... Именно на это я и рассчитывал!{/m}"
    Max_01 "{m}Il semble que le remède glissé dans la nourriture d'Eric l'ait mis de mauvaise humeur et il ne se risque pas à aller voir ma sœur C'est exactement ce sur quoi je comptais !{/m}"

# game/events/eric_cam.rpy:151
translate french cam0_eric_resting_9b8dcbac:

    # Max_01 "{m}О, \"В мире животных\" показывают! То ли шимпанзе на стероидах, то ли горилла...{/m}"
    Max_01 "{m}Oh, \"dans le monde des animaux\" montrent! Soit un chimpanzé sous stéroïdes, soit un gorille...{/m}"

# game/events/eric_cam.rpy:171
translate french cam0_eric_ann_tv_adbf2bd8:

    # Max_01 "{m}Мама с Эриком смотрят какой-то фильм. Наверняка, опять порнушку...{/m}"
    Max_01 "{m}Maman et Eric regardent un film. À coup sûr, encore une fois порнушку...{/m}"

# game/events/eric_cam.rpy:173
translate french cam0_eric_ann_tv_cf0927e9:

    # Max_01 "{m}Мама с Эриком смотрят какой-то фильм. Интересно, какой?{/m}"
    Max_01 "{m}Maman et Eric regardent un film. Je me demande lequel?{/m}"

# game/events/eric_cam.rpy:179
translate french cam0_eric_ann_tv_0268c438:

    # Max_08 "{m}Как же повезло Эрику! Я бы тоже с огромной радостью так посмотрел что угодно... если бы мамины нежные и умелые ручки ласкали мой член...{/m}"
    Max_08 "{m}quelle chance Eric! Moi aussi c'est avec une joie bien regardé quoi que ce soit... si les mains tendres et habiles de ma mère caressaient ma bite...{/m}"

# game/events/eric_cam.rpy:181
translate french cam0_eric_ann_tv_6ba07bce:

    # Max_08 "{m}Блин, мама, тебе что, настолько неинтересно то, что происходит на экране или ты просто любишь отсасывать Эрику?!{/m}"
    Max_08 "{m}Putain, maman, est-ce que tu ne t'intéresses pas à ce qui se passe à l'écran ou est-ce que tu aimes juste sucer Erica?!{/m}"

# game/events/eric_cam.rpy:187
translate french cam0_eric_ann_tv_d86ae6d5:

    # Max_09 "{m}Ну конечно, ведь просто так смотреть, что происходит на экране они не будут, обязательно надо вот это вот делать...{/m}"
    Max_09 "{m}Eh bien, bien sûr, parce que juste pour regarder ce qui se passe à l'écran, ils ne le feront pas, vous devez absolument le faire...{/m}"

# game/events/eric_cam.rpy:191
translate french cam0_eric_ann_tv_8045496a:

    # Max_10 "{m}А, ну да, без полотенца однозначно лучше... Но вы же в гостиной! А если зайдёт кто-то, а вы тут развлекаетесь...{/m}"
    Max_10 "{m}A, Eh bien, oui, sans une serviette est certainement mieux... Mais vous êtes dans le salon! Et si quelqu'un entre et que vous vous amusez...{/m}"

# game/events/eric_cam.rpy:201
translate french cam0_eric_ann_fucking_5719e975:

    # Max_09 "{m}Утомлённые мама с Эриком лежат на кровати. Благо, они не только трахаются всё время, но ещё и разговаривают...{/m}"
    Max_09 "{m}maman et Eric sont allongés sur le lit. Heureusement, non seulement ils baisent tout le temps, mais ils parlent aussi...{/m}"

# game/events/eric_cam.rpy:213
translate french cam0_eric_ann_fucking_e1e7d2c7:

    # Max_07 "{m}Через окно подсмотреть мне не дали, значит заценим через камеру... А они тут развлекаются, только в путь!{/m}"
    Max_07 "{m}à Travers la fenêtre, je n'ai pas été autorisé à espionner, alors nous allons regarder à travers la caméra... Et ils s'amusent ici, seulement en route!{/m}"

# game/events/eric_cam.rpy:216
translate french cam0_eric_ann_fucking_e24d430b:

    # Max_07 "{m}Подсмотреть через окно я не рискнул, могут заметить, а вот через камеру могу наслаждаться безнаказанно, сколько захочу... И посмотреть есть на что!{/m}"
    Max_07 "{m}Espionner à travers la fenêtre, je n'ai pas osé, ils peuvent remarquer, mais à travers la caméra, je peux profiter en toute impunité autant que je veux... Et voir il y a quoi!{/m}"

# game/events/eric_cam.rpy:219
translate french cam0_eric_ann_fucking_30904857:

    # Max_07 "{m}Ого! Вот это я удачно заглянул! Всё равно, что порно посмотреть...{/m}"
    Max_07 "{m}Wow! J'ai bien regardé! C'est comme regarder du porno...{/m}"

# game/events/eric_cam.rpy:228
translate french cam0_eric_ann_fucking_42470dcb:

    # Max_08 "{m}Ну вот они и кончили. По крайней мере, Эрик так точно, а вот мама...{/m}"
    Max_08 "{m}Eh bien, ils ont fini. Au moins, Eric est si précis, mais maman...{/m}"

# game/events/eric_cam.rpy:249
translate french cam0_sexed_lisa_451f5f08:

    # Max_08 "{m}Жаль камеры не передают звук. Нужно будет обязательно спросить Лизу, о чём они разговаривали!{/m}"
    Max_08 "{m}C'est dommage que les caméras ne transmettent pas le son. Je dois demander à Lisa de quoi ils ont parlé !{/m}"

# game/events/eric_cam.rpy:259
translate french cam0_sexed_lisa_71c1a2f8:

    # Max_07 "{m}Лиза пришла на урок к маме и Эрику. Посмотрим, что будет...{/m}"
    Max_07 "{m}Lisa est en classe avec notre mère et Eric. Voyons ce qui se passe...{/m}"

# game/events/eric_cam.rpy:267
translate french cam0_sexed_lisa_c0ac64d9:

    # Max_09 "{m}Вот же Эрик! Член он свой вывесил... Интересно, что они ей там рассказывают?{/m}"
    Max_09 "{m}C'est Eric ! Il a sorti sa bite... Je me demande ce qu'ils lui disent ?{/m}"

# game/events/eric_cam.rpy:275
translate french cam0_sexed_lisa_91d200d1:

    # Max_08 "{m}Ага, дрочка пошла... И уговорил же Эрик маму на всё это...{/m}"
    Max_08 "{m}Ouais, se branler... Et Eric a convaincu ma mère de tout ça...{/m}"

# game/events/eric_cam.rpy:285
translate french cam0_sexed_lisa_71c1a2f8_1:

    # Max_07 "{m}Лиза пришла на урок к маме и Эрику. Посмотрим, что будет...{/m}"
    Max_07 "{m}Lisa est en classe avec notre mère et Eric. Voyons ce qui se passe...{/m}"

# game/events/eric_cam.rpy:293
translate french cam0_sexed_lisa_525949f3:

    # Max_09 "{m}Мама при Лизе уже надрачивает Эрику как будто так и надо! Интересно, что они ей там рассказывают при этом?{/m}"
    Max_09 "{m}Maman est déjà en train de branler Eric devant Lisa comme si c'était la bonne chose à faire ! Je me demande ce qu'ils lui disent?{/m}"

# game/events/eric_cam.rpy:301
translate french cam0_sexed_lisa_0dd5dbf3:

    # Max_08 "{m}Ага, мама повышает градус показом своей шикарной груди... И уговорил же Эрик маму на всё это!{/m}"
    Max_08 "{m}Oui, maman fait monter la température en montrant ses magnifiques seins... Et Eric a convaincu ma mère de tout ça!{/m}"

# game/events/eric_cam.rpy:311
translate french cam0_sexed_lisa_71c1a2f8_2:

    # Max_07 "{m}Лиза пришла на урок к маме и Эрику. Посмотрим, что будет...{/m}"
    Max_07 "{m}Lisa est en classe avec notre mère et Eric. Voyons ce qui se passe...{/m}"

# game/events/eric_cam.rpy:319
translate french cam0_sexed_lisa_43160aa4:

    # Max_09 "{m}Подрочить Эрику на глазах Лизы в образовательных целях? Конечно, обычное дело, не обращайте внимания... Интересно, что они ей там рассказывают при этом?{/m}"
    Max_09 "{m}Branler Eric devant Lisa dans un but éducatif? Bien sûr, les trucs habituels, peu importe... Je me demande ce qu'ils lui disent ?{/m}"

# game/events/eric_cam.rpy:327
translate french cam0_sexed_lisa_36fcd85f:

    # Max_08 "{m}О, от такого я бы не отказался! Только от одних мыслей о маминых руках, нежно массирующих мои шары, хочется кончить...{/m}"
    Max_08 "{m}Oh, je ne serais pas contre ça! Rien que la pensée des mains de ma mère massant doucement mes couilles me donne envie de jouir...{/m}"

# game/events/eric_cam.rpy:337
translate french cam0_sexed_lisa_71c1a2f8_3:

    # Max_07 "{m}Лиза пришла на урок к маме и Эрику. Посмотрим, что будет...{/m}"
    Max_07 "{m}Lisa est en classe avec notre mère et Eric. Voyons ce qui se passe...{/m}"

# game/events/eric_cam.rpy:345
translate french cam0_sexed_lisa_4bfb346c:

    # Max_09 "{m}Всё дрочат и дрочат... Видимо, закрепляют материал. Ну сколько можно?!{/m}"
    Max_09 "{m}Tout le monde se branle et se branle... Apparemment, ils sont en train de consolider leur relation. Combien de fois ?!{/m}"

# game/events/eric_cam.rpy:353
translate french cam0_sexed_lisa_6918e27b:

    # Max_08 "{m}О, Эрик не выдержал и кончил... Хотя, вряд ли. Скорее всего, они решили показать Лизе для чего это всё было нужно.{/m}"
    Max_08 "{m}Oh, Eric n'en pouvait plus et il a jouit... mais pas beaucoup, cependant. Ils ont plutôt décidé de montrer à Lisa de quoi il s'agissait.{/m}"

# game/events/eric_cam.rpy:363
translate french cam0_eric_shat_4bc4874c:

    # Max_09 "{m}Ничего толком не видно... Стоит взглянуть через другую камеру...{/m}"
    Max_09 "{m}Rien n'est vraiment visible... Je devrais regarder à travers une autre caméra...{/m}"

# game/events/eric_cam.rpy:365
translate french cam0_eric_shat_d2f3bb58:

    # Max_09 "{m}Ничего толком не видно...{/m}"
    Max_09 "{m}Rien n'est vraiment visible...{/m}"

# game/events/eric_cam.rpy:379
translate french cam1_eric_shat_58fccd25:

    # Max_02 "{m}Эрик во всю оценивает эффективность слабительного, которое я ему подмешал... Надеюсь, эта обезьяна знает, как пользоваться туалетной бумагой, потому что банановых листьев у нас тут нет!{/m}"
    Max_02 "{m}Eric évalue l'efficacité du laxatif que je lui ai mélangé... J'espère que ce singe sait utiliser du papier toilette, car nous n'avons pas de feuilles de bananier ici!{/m}"

# game/events/eric_cam.rpy:381
translate french cam1_eric_shat_a8ebe479:

    # Max_02 "{m}Ты что, Эрик, съел что-то не то, что ли?! Бедняга. Ну, держись там...{/m}"
    Max_02 "{m}tu as mangé quelque chose de mal?! Pauvre diable. Eh bien, reste là...{/m}"

# game/events/eric_cam.rpy:390
translate french cam0_eric_ann_resting_cb75645d:

    # Max_02 "{m}Похоже, потрахушек сегодня не намечается... Интересно, с чего бы это?!{/m}"
    Max_02 "{m}On dirait qu'il n'y a pas de baise ce soir... Je me demande pourquoi !{/m}"

# game/events/eric_cam.rpy:400
translate french cam0_eric_sleep_bce0a0b5:

    # Max_07 "{m}Остаётся надеяться, что я подмешал ему не слишком много успокоительного... Главное, что он сейчас ни к кому не пристаёт.{/m}"
    Max_07 "{m}J'espère juste que je ne lui ai pas donné trop de sédatifs... L'essentiel est qu'il ne drague plus personne maintenant.{/m}"

# game/events/eric_cam.rpy:410
translate french cam0_eric_ann_try_fucking_5da5d134:

    # Max_01 "{m}Не стоит? Ну ничего, бывает... Но всё равно приятно осознавать, что это происходит из-за моих действий.{/m}"
    Max_01 "{m}Tu ne devrais pas ? Ce n'est pas grave, ça arrive... Mais c'est toujours agréable de savoir que c'est à cause de mes actions.{/m}"

# game/events/eric_cam.rpy:421
translate french cam_before_frame_eric_cb1c7bf7:

    # Max_07 "{m}Отлично! Эрик именно там, где мне и нужно... Нужно дождаться, когда он вернётся к моей маме, и затем уже спокойно его подставлять...{/m}" nointeract
    Max_07 "{m}Bien! Eric est exactement là où j'ai besoin... Il faut attendre qu'il revienne chez ma mère, et ensuite le piéger calmement...{/m}" nointeract

# game/events/eric_cam.rpy:437
translate french cam_before_frame_eric_2da5afe7:

    # Max_01 "{m}Отлично! Пора...{/m}" nointeract
    Max_01 "{m}Bien! C'est l'heure...{/m}" nointeract

# game/events/eric_cam.rpy:447
translate french cam0_eric_kira_night_swim_4bc4874c:

    # Max_09 "{m}Ничего толком не видно... Стоит взглянуть через другую камеру...{/m}"
    Max_09 "{m}Rien n'est vraiment visible... Je devrais regarder à travers une autre caméra...{/m}"

# game/events/eric_cam.rpy:449
translate french cam0_eric_kira_night_swim_14337802:

    # Max_09 "{m}Ничего не разглядеть... Нужно установить камеру, которая охватила бы весь бассейн...{/m}"
    Max_09 "{m}rien à voir... Nous devons installer une caméra qui couvrirait toute la piscine...{/m}"

# game/events/eric_cam.rpy:459
translate french cam1_eric_kira_night_swim_45123771:

    # Max_10 "{m}Вот чёрт! Похоже, Кира ублажает Эрика, чтобы он нас не заложил маме...{/m}"
    Max_10 "{m}Voici les caractéristiques de la! On dirait que Kira fait plaisir à Eric pour qu'il ne nous mette pas à ma mère...{/m}"

translate french strings:

    # game/events/eric_cam.rpy:436
    old "{i}идти к комнате Алисы{/i}"
    new "{i}aller à la chambre d'Alice {/i}"

