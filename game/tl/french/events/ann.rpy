# TODO: Translation updated at 2022-09-25 19:46

# game/events/ann.rpy:11
translate french ann_sleep_6173ff68:

    # Max_00 "{m}В это время мама обычно спит.\nМне кажется, не стоит её будить...{/m}" nointeract
    Max_00 "{m}à ce moment-là, maman dort habituellement.\nje pense que je ne devrais pas la réveiller...{/m}" nointeract

# game/events/ann.rpy:18
translate french ann_sleep_e0ca90ce:

    # Max_09 "Хм... Мамы нет. Где же она?"
    Max_09 "Hum... Maman n'est pas là. Où est-elle ?"

# game/events/ann.rpy:27
translate french ann_sleep_2e3b88bc:

    # Max_01 "{m}Класс! Мама спит... Даже не верится, что у этой конфетки трое детей... В жизни бы в такое не поверил!{/m}" nointeract
    Max_01 "{m}Classe! Maman dort... Je ne peux même pas croire que ce bonbon a trois enfants... Dans la vie, je n'aurais pas cru ça!{/m}" nointeract

# game/events/ann.rpy:29
translate french ann_sleep_e9347a52:

    # Max_04 "{m}О, да! Какая у мамы попка! Всё-таки хорошо, что здесь так жарко и все спят не укрываясь... Просто супер!{/m}" nointeract
    Max_04 "{m}oh, oui! Quel cul de maman! C'est quand même une bonne chose qu'il fasse si chaud ici et que tout le monde dorme sans se cacher... Juste super!{/m}" nointeract

# game/events/ann.rpy:31
translate french ann_sleep_f158a828:

    # Max_07 "{m}Обалденно! Как же повезло, что у меня такая горячая мама... Выглядит потрясающе, аж глаза отрывать не хочется!{/m}" nointeract
    Max_07 "{m}Génial! Quelle chance d'avoir une maman si chaude... Ça a l'air incroyable, je ne veux pas arracher les yeux!{/m}" nointeract

# game/events/ann.rpy:34
translate french ann_sleep_78b3f9d0:

    # Max_01 "{m}Класс! Мама спит в ночнушке... Даже не верится, что у этой конфетки трое детей... В жизни бы в такое не поверил!{/m}" nointeract
    Max_01 "{m}Classe! Maman dort dans une nuisette... Je ne peux même pas croire que ce bonbon a trois enfants... Dans la vie, je n'aurais pas cru ça!{/m}" nointeract

# game/events/ann.rpy:36
translate french ann_sleep_19a34828:

    # Max_04 "{m}О, да! Какая у мамы попка! Всё-таки хорошо, что здесь так жарко и все спят не укрываясь... Её попку даже немного видно через ночнушку!{/m}" nointeract
    Max_04 "{m}oh, oui! Quel cul de maman! C'est quand même une bonne chose qu'il fasse si chaud ici et que tout le monde dorme sans se cacher... Son cul est même un peu visible à travers la nuisette!{/m}" nointeract

# game/events/ann.rpy:38
translate french ann_sleep_1cc5b28f:

    # Max_07 "{m}Обалденно! Как же повезло, что у меня такая горячая мама... В этой ночнушке она выглядит потрясающе, аж глаза отрывать не хочется!{/m}" nointeract
    Max_07 "{m}Génial! Quelle chance d'avoir une maman si chaude... Dans cette nuisette, elle a l'air incroyable, je ne veux pas arracher les yeux!{/m}" nointeract

# game/events/ann.rpy:48
translate french ann_sleep_d64da859:

    # Max_03 "{m}Чёрт, у меня самая аппетитная мама на свете! Вот бы снять с неё всё белье и пристроиться сзади... Но лучше потихоньку уходить, пока она не проснулась.{/m}" nointeract
    Max_03 "{m}merde, J'ai la maman la plus appétissante du monde! Je voudrais lui enlever tous ses sous-vêtements et l'attacher à l'arrière... Mais il vaut mieux partir lentement avant qu'elle ne se réveille.{/m}" nointeract

# game/events/ann.rpy:50
translate french ann_sleep_eedcea80:

    # Max_02 "{m}Ухх! Так и хочется прижаться к этой обворожительной попке и шалить всю ночь... Но пора уходить, а то она может проснуться.{/m}" nointeract
    Max_02 "{m}uhh! Et je veux me blottir dans ce cul charmant et coquine toute la nuit... Mais il est temps de partir, sinon elle pourrait se réveiller.{/m}" nointeract

# game/events/ann.rpy:52
translate french ann_sleep_abe8ecd4:

    # Max_05 "{m}Вот это да! От вида этих раздвинутых ножек становится всё равно, что она моя мама... Слишком соблазнительно! Только бы она сейчас не проснулась...{/m}" nointeract
    Max_05 "{m}C'est ça! La vue de ces jambes écartées donne l'impression qu'elle est ma mère... Trop séduisant! Si seulement elle se réveillait...{/m}" nointeract

# game/events/ann.rpy:55
translate french ann_sleep_249e8b49:

    # Max_03 "{m}Чёрт, у меня самая аппетитная мама на свете! Вот бы пристроиться сзади и запустить руки под эту сорочку... Но лучше потихоньку уходить, пока она не проснулась.{/m}" nointeract
    Max_03 "{m}merde, J'ai la maman la plus appétissante du monde! Je voudrais m'attacher par derrière et mettre mes mains sous cette chemise... Mais il vaut mieux partir lentement avant qu'elle ne se réveille.{/m}" nointeract

# game/events/ann.rpy:57
translate french ann_sleep_3eaf0f95:

    # Max_02 "{m}Ухх! Так и хочется задрать её сорочку, прижаться к этой обворожительной попке и шалить всю ночь... Но пора уходить, а то она может проснуться.{/m}" nointeract
    Max_02 "{m}uhh! Et je veux soulever sa chemise, se blottir contre ce beau cul et être méchant toute la nuit... Mais il est temps de partir, sinon elle pourrait se réveiller.{/m}" nointeract

# game/events/ann.rpy:59
translate french ann_sleep_80d64f9d:

    # Max_05 "{m}Вот это да! От вида этих раздвинутых ножек становится всё равно, что она моя мама... Слишком уж соблазнительно она выглядит в этой сорочке! Только бы она сейчас не проснулась...{/m}" nointeract
    Max_05 "{m}C'est ça! La vue de ces jambes écartées donne l'impression qu'elle est ma mère... Elle a l'air trop séduisante dans cette chemise! Si seulement elle se réveillait...{/m}" nointeract

# game/events/ann.rpy:74
translate french ann_shower_71ff78e8:

    # Max_00 "{m}Я уже попался сегодня на подглядывании за мамой. Не стоит злить её ещё больше.{/m}"
    Max_00 "{m}Je suis déjà tombé sur la surveillance de ma mère aujourd'hui. Ne la fâchez pas encore plus.{/m}"

# game/events/ann.rpy:77
translate french ann_shower_f74760e1:

    # Max_00 "{m}Я уже подсматривал сегодня за мамой. Не стоит искушать судьбу слишком часто.{/m}"
    Max_00 "{m}J'ai déjà regardé ma mère aujourd'hui. Ne tentez pas le destin trop souvent.{/m}"

# game/events/ann.rpy:80
translate french ann_shower_8b54d4a6:

    # Max_00 "{m}Сегодня мама и так сегодня едва не поймала меня. Не стоит искушать судьбу слишком часто.{/m}"
    Max_00 "{m}aujourd'hui, ma mère m'a presque attrapé aujourd'hui. Ne tentez pas le destin trop souvent.{/m}"

# game/events/ann.rpy:84
translate french ann_shower_686b9c2d:

    # Max_00 "{m}Мама сейчас принимает душ...{/m}" nointeract
    Max_00 "{m}Maman prend une douche maintenant...{/m}" nointeract

# game/events/ann.rpy:90
translate french ann_shower_ecc96d65:

    # Max_00 "{m}Похоже, мама принимает душ...{/m}" nointeract
    Max_00 "{m}on Dirait que maman prend une douche...{/m}" nointeract

# game/events/ann.rpy:101
translate french ann_shower_ladder_76e8b72b:

    # Max_04 "{m}Посмотрим, что у нас тут...{/m}"
    Max_04 "{m}voyons Ce qu'on a ici...{/m}"

# game/events/ann.rpy:114
translate french ann_shower_ladder_22ab693f:

    # Max_07 "{m}Да-а... Распахнутый халатик на маме - это просто изумительное шоу! Такие соблазнительные сосочки... да ещё и так близко... Ммм...{/m}"
    Max_07 "{m}Oui... Robe ouverte sur maman-c'est juste un spectacle incroyable! Ces papilles séduisantes... et si proche... Mmm...{/m}"

# game/events/ann.rpy:116
translate french ann_shower_ladder_31453ab1:

    # Max_05 "{m}О, да! Мама решила не надевать трусики и правильно сделала, потому что увидеть эту киску с утра пораньше - просто сказка!{/m}"
    Max_05 "{m}oh, oui! Maman a décidé de ne pas mettre de culotte et l'a fait correctement, car voir cette chatte tôt le matin n'est qu'un conte de fées!{/m}"

# game/events/ann.rpy:118
translate french ann_shower_ladder_e12e58b5:

    # Max_03 "{m}Вот это повезло... Мама в одних лишь трусиках, а её упругая грудь предстаёт передо мной во всей своей красе! Так бы любовался и любовался ей...{/m}"
    Max_03 "{m}c'est de la chance... Maman n'est qu'une culotte, et sa poitrine élastique apparaît devant moi dans toute sa splendeur! Donc, je l'admirerais et l'admirerais...{/m}"

# game/events/ann.rpy:120
translate french ann_shower_ladder_1cf5ec43:

    # Max_06 "{m}Ничего себе! Такое зрелище не каждый раз увидишь - она же совершенно голая! Только бы со стремянки не упасть от такого вида... Как было бы круто потискать все её округлости!{/m}"
    Max_06 "{m}wow! Vous ne verrez pas un tel spectacle à chaque fois - elle est complètement nue! Si seulement de l'escabeau ne tombe pas de cette vue... Comme ce serait cool de presser toutes ses rondeurs!{/m}"

# game/events/ann.rpy:125
translate french ann_shower_ladder_1409147d:

    # Max_07 "{m}Мои зрители явно пропускают много всего интересного! Мне однозначно стоит установить сюда ещё одну камеру...{/m}"
    Max_07 "{m}Mes téléspectateurs manquent évidemment beaucoup de choses intéressantes! Je devrais installer une autre caméra ici...{/m}"

# game/events/ann.rpy:126
translate french ann_shower_ladder_46e55bf6:

    # Max_00 "{m}Лучше бы мне уже уйти, пока никто не увидел...{/m}"
    Max_00 "{m}j'aurais Dû partir avant que personne ne le voie...{/m}"

# game/events/ann.rpy:136
translate french ann_shower_start_peeping_fc5db481:

    # Max_09 "{m}Лучше вообще свести подглядывания к минимуму, пока я не избавлюсь от Эрика. Чтобы никого ещё больше не расстраивать...{/m}"
    Max_09 "{m}il Vaut mieux minimiser les voyeurs jusqu'à ce que je me débarrasse d'Eric. Pour ne déranger personne d'autre...{/m}"

# game/events/ann.rpy:138
translate french ann_shower_start_peeping_dda24c4d:

    # Max_01 "{m}Хорошо, что это не распространяется на Киру...{/m}" nointeract
    Max_01 "{m}Eh bien, cela ne s'applique pas à Kira...{/m}" nointeract

# game/events/ann.rpy:145
translate french ann_shower_start_peeping_d307b6b4:

    # Max_07 "{m}Ух, аж завораживает! Повезло же, что у меня такая сексуальная мама... Надеюсь, она меня не заметит...{/m}" nointeract
    Max_07 "{m}Wow, déjà fascinant! J'ai de la chance d'avoir une mère aussi sexy... J'espère qu'elle ne me remarquera pas...{/m}" nointeract

# game/events/ann.rpy:165
translate french ann_shower_alt_peepeng_9519b19b:

    # Max_03 "[undetect!t]{m}Обалдеть можно! Не каждый день выпадает такое счастье, любоваться этой красотой! Её большая упругая грудь и стройная фигурка просто загляденье...{/m}"
    Max_03 "[undetect!t]{m}c'est Génial! Pas tous les jours tombe un tel bonheur, admirer cette beauté! Ses gros seins élastiques et sa silhouette mince sont juste un coup d'oeil...{/m}"

# game/events/ann.rpy:167
translate french ann_shower_alt_peepeng_bb7b0422:

    # Max_05 "[undetect!t]{m}О, да! Зрелище просто потрясающее... Такой сочной попке может позавидовать любая женщина! Какая мокренькая...{/m}"
    Max_05 "[undetect!t]{m}Oh, oui! Le spectacle est incroyable... Un tel cul juteux peut envier n'importe quelle femme! Quelle мокренькая...{/m}"

# game/events/ann.rpy:180
translate french ann_shower_closer_peepeng_9519b19b:

    # Max_03 "[undetect!t]{m}Обалдеть можно! Не каждый день выпадает такое счастье, любоваться этой красотой! Её большая упругая грудь и стройная фигурка просто загляденье...{/m}"
    Max_03 "[undetect!t]{m}c'est Génial! Pas tous les jours tombe un tel bonheur, admirer cette beauté! Ses gros seins élastiques et sa silhouette mince sont juste un coup d'oeil...{/m}"

# game/events/ann.rpy:182
translate french ann_shower_closer_peepeng_bb7b0422:

    # Max_05 "[undetect!t]{m}О, да! Зрелище просто потрясающее... Такой сочной попке может позавидовать любая женщина! Какая мокренькая...{/m}"
    Max_05 "[undetect!t]{m}Oh, oui! Le spectacle est incroyable... Un tel cul juteux peut envier n'importe quelle femme! Quelle мокренькая...{/m}"

# game/events/ann.rpy:195
translate french ann_shower_not_luck_7fca7b55:

    # Ann_14 "[spotted!t]Сынок, я вообще-то всё вижу! Понимаю, тебе интересно, но меня это несколько... смущает. Не мешай маме." nointeract
    Ann_14 "[Fils, je peux vraiment tout voir ! Je sais que vous êtes curieux, mais je suis un peu... embarrassant. Ne dérange pas ta mère." nointeract

# game/events/ann.rpy:207
translate french ann_shower_not_luck_6222d56c:

    # Max_12 "{color=[orange]}{i}Кажется, мама что-то заподозрила!{/i}{/color}\n{m}Упс... надо бежать, пока она меня не увидела!{/m}"
    Max_12 "{color=[orange]}{i}je Pense que maman a suspecté quelque chose!{/i} {/color}\n{m}.. je dois partir avant qu'elle ne me voie!{/m}"

# game/events/ann.rpy:216
translate french ann_shower_not_luck_9b36810d:

    # Ann_15 "[spotted!t]Макс!!! Что ты здесь делаешь? А ну быстро отвернись!!!" nointeract
    Ann_15 "[spotted!t]Max ! !! Qu'est-ce que tu fais ici ? Fais demi-tour maintenant !!!" nointeract

# game/events/ann.rpy:227
translate french ann_shower_serious_talk_9d79db42:

    # Ann_19 "Ты что, подглядываешь за мной? Тебе должно быть стыдно! Нас ждёт серьёзный разговор..." nointeract
    Ann_19 "Tu m'espionnes ? Tu devrais avoir honte de toi! Nous allons avoir une discussion sérieuse...." nointeract

# game/events/ann.rpy:230
translate french ann_shower_serious_talk_5cc0688c:

    # Ann_12 "[succes!t]Случайность, говоришь? Ну ладно, поверю. А теперь бегом отсюда!"
    Ann_12 "[succes!t]Un accident, tu dis? D'accord, je vais te croire. Maintenant, sors d'ici !"

# game/events/ann.rpy:231
translate french ann_shower_serious_talk_b81f7229:

    # Max_04 "Ага, хорошо, мам!"
    Max_04 "oui, d’accord, maman!"

# game/events/ann.rpy:234
translate french ann_shower_serious_talk_65550d20:

    # Ann_16 "[failed!t]Случайно пробрался сюда, спрятался и глазеешь тут? Случайно?! А ну-ка марш отсюда! Перед завтраком поговорим!"
    Ann_16 "[failed!t]Se faufiler accidentellement ici, se cacher et reluquer ? Accidentellement ? ! Sors d'ici ! Nous allons en parler avant le petit-déjeuner !"

# game/events/ann.rpy:235
translate french ann_shower_serious_talk_e071f814:

    # Max_10 "Хорошо..."
    Max_10 "D’accord..."

# game/events/ann.rpy:237
translate french ann_shower_serious_talk_1f891e5d:

    # Ann_12 "Что, думаешь извинился и всё, можно снова подглядывать? Нет, Макс. В этот раз всё так просто не пройдёт. Сейчас иди отсюда, а перед завтраком поговорим!"
    Ann_12 "Quoi, tu penses que tu t'es excusé et que c'est tout, tu peux regarder à nouveau? Non, Max. Ce ne sera pas aussi facile cette fois-ci. Maintenant sors d'ici, et nous en reparlerons avant le petit déjeuner !"

# game/events/ann.rpy:238
translate french ann_shower_serious_talk_1c3cc022:

    # Max_11 "Хорошо..."
    Max_11 "D’accord..."

# game/events/ann.rpy:240
translate french ann_shower_serious_talk_f1aa69a0:

    # Ann_13 "Что?! Ну всё, Макс, ты попал! Быстро вернулся в дом, а перед завтарком поговорим ещё на эту тему!"
    Ann_13 "Quoi ? ! C'est ça, Max, tu es dans la merde! Retournes vite dans te chambre, et nous en reparlerons avant le petit-déjeuner !"

# game/events/ann.rpy:241
translate french ann_shower_serious_talk_471df3d0:

    # Max_14 "Хорошо..."
    Max_14 "D’accord..."

# game/events/ann.rpy:283
translate french ann_dressed_734f1c10:

    # Max_09 "{m}Сегодня суббота, день шоппинга. Видимо, мама собирается...{/m}" nointeract
    Max_09 "{m}Aujourd'hui est samedi, le jour du shopping. Apparemment, maman va le faire...{/m}" nointeract

# game/events/ann.rpy:285
translate french ann_dressed_269233ce:

    # Max_09 "{m}Сейчас 10 часов, а значит, мама собирается на работу...{/m}" nointeract
    Max_09 "{m}il est 10 heures, ce qui signifie que maman va travailler...{/m}" nointeract

# game/events/ann.rpy:289
translate french ann_dressed_b21abe2e:

    # Ann "{b}Анна:{/b} Кто там?" nointeract
    Ann "{b}Anna:{/b} Qui est là ?" nointeract

# game/events/ann.rpy:292
translate french ann_dressed_bf2abaef:

    # Ann "{b}Анна:{/b} Нет, Макс. Я переодеваюсь. Подожди немного, дорогой."
    Ann "{b}Anna:{/b} Non, Max. Je suis en train de me changer. Attends un peu, chéri."

# game/events/ann.rpy:294
translate french ann_dressed_2e55a9e3:

    # Ann "{b}Анна:{/b} Макс, я не одета. Собираюсь на работу. Подожди немного, дорогой."
    Ann "{b}Anna:{/b} Max, je ne suis pas habillée. Je vais au travail. Attends un peu, chéri."

# game/events/ann.rpy:295
translate french ann_dressed_f3117b6b:

    # Max_00 "Хорошо, мам."
    Max_00 "D’accord, maman."

# game/events/ann.rpy:298
translate french ann_dressed_2f691176:

    # Ann "{b}Анна:{/b} Макс, я не одета. Собираюсь на шопинг. Подожди немного, дорогой."
    Ann "{b}Anna:{/b} Max, je ne suis pas habillée. Je vais faire du shopping. Attends un peu, chéri."

# game/events/ann.rpy:299
translate french ann_dressed_f3117b6b_1:

    # Max_00 "Хорошо, мам."
    Max_00 "D’accord, maman."

# game/events/ann.rpy:335
translate french ann_dressed_a3f1e45d:

    # Max_01 "{m}Ничего себе, вот это зрелище! Это я удачно выбрал момент... Но пора уходить, а то вдруг увидит меня в зеркало!{/m}"
    Max_01 "{m}Wow, quel spectacle! C'est moi qui ai choisi le moment... Mais il est temps de partir, ou tout à coup il me voit dans le miroir!{/m}"

# game/events/ann.rpy:350
translate french ann_dressed_stay_in_room_1c7b7b81:

    # Eric_09 "Макс, не мешай взрослым! В твоём положении это просто верх наглости! Найди себе занятие..." nointeract
    Eric_09 "Max, ne dérange pas les adultes! Dans ta position, c'est juste le dessus de l'insolence! Trouve-toi une leçon..." nointeract

# game/events/ann.rpy:352
translate french ann_dressed_stay_in_room_ff7e7143:

    # Ann_00 "Сынок, мы с Эриком собирались переодеться. Не мешай взрослым, займись чем-нибудь..." nointeract
    Ann_00 "Fiston, Eric et moi allions nous changer. N'interfère pas avec les adultes, fais quelque chose..." nointeract

# game/events/ann.rpy:356
translate french ann_dressed_stay_in_room_4e1690c6:

    # Ann_12 "Сынок, я собиралась переодеться. Не мешай маме. Займись чем-нибудь полезным..." nointeract
    Ann_12 "fiston, J'allais me changer. Ne dérange pas ta mère. Fais quelque chose d'utile..." nointeract

# game/events/ann.rpy:358
translate french ann_dressed_stay_in_room_0e69876a:

    # Ann_00 "Сынок, я собиралась переодеться. Иди пока, займись чем-нибудь..." nointeract
    Ann_00 "fiston, J'allais me changer. Idn jusqu'à présent, faites quelque chose..." nointeract

# game/events/ann.rpy:362
translate french ann_dressed_stay_in_room_659d5d18:

    # Ann_12 "Нет, конечно! Ишь ты, что удумал! Завязывай с этими глупостями и дай маме спокойно переодеться." nointeract
    Ann_12 "non, bien sûr! C'est toi qui as pensé! Arrête ces conneries et laisse ta mère se changer." nointeract

# game/events/ann.rpy:364
translate french ann_dressed_stay_in_room_e4ef4b48:

    # Ann_05 "Приятно слышать, сынок. Спасибо, что подметил." nointeract
    Ann_05 "C'est bon à entendre, fils. Merci de l'avoir souligné." nointeract

# game/events/ann.rpy:379
translate french ann_dressed_moment0_1c7b7b81:

    # Eric_09 "Макс, не мешай взрослым! В твоём положении это просто верх наглости! Найди себе занятие..." nointeract
    Eric_09 "Max, ne dérange pas les adultes! Dans ta position, c'est juste le dessus de l'insolence! Trouve-toi une leçon..." nointeract

# game/events/ann.rpy:381
translate french ann_dressed_moment0_ff7e7143:

    # Ann_00 "Сынок, мы с Эриком собирались переодеться. Не мешай взрослым, займись чем-нибудь..." nointeract
    Ann_00 "Fiston, Eric et moi allions nous changer. N'interfère pas avec les adultes, fais quelque chose..." nointeract

# game/events/ann.rpy:388
translate french ann_dressed_moment0_104e65c7:

    # Ann_00 "Так, сынок... Ты можешь погулять? А то мне нужно переодеться..." nointeract
    Ann_00 "Ok, fiston... Tu peux sortir? Je dois me changer..." nointeract

# game/events/ann.rpy:390
translate french ann_dressed_moment0_ad13a757:

    # Ann_13 "Конечно! Это как-то неправильно, если мать будет переодеваться при своём ребёнке. Так что, пожалуйста, выйди ненадолго." nointeract
    Ann_13 "Bien sûr! C'est en quelque sorte faux si une mère se change avec son enfant. Alors s'il te plaît, sors un peu." nointeract

# game/events/ann.rpy:392
translate french ann_dressed_moment0_42ae37fd:

    # Ann_12 "Ишь ты, что удумал! Завязывай с этими глупостями и дай маме спокойно переодеться." nointeract
    Ann_12 "t'As fait quoi? Arrête ces conneries et laisse ta mère se changer." nointeract

# game/events/ann.rpy:394
translate french ann_dressed_moment0_19c5e329:

    # Ann_01 "Спасибо, Макс. Если ты что-то хотел, то я недолго..." nointeract
    Ann_01 "Merci, Max. .." nointeract

# game/events/ann.rpy:396
translate french ann_dressed_moment0_659d5d18:

    # Ann_12 "Нет, конечно! Ишь ты, что удумал! Завязывай с этими глупостями и дай маме спокойно переодеться." nointeract
    Ann_12 "non, bien sûr! C'est toi qui as pensé! Arrête ces conneries et laisse ta mère se changer." nointeract

# game/events/ann.rpy:398
translate french ann_dressed_moment0_e4ef4b48:

    # Ann_05 "Приятно слышать, сынок. Спасибо, что подметил." nointeract
    Ann_05 "C'est bon à entendre, fils. Merci de l'avoir souligné." nointeract

# game/events/ann.rpy:402
translate french ann_dressed_moment0_04c8c654:

    # Ann_13 "Ой, Макс! Где именно?!"
    Ann_13 "Oh, Max ! Où exactement ? !"

# game/events/ann.rpy:403
translate french ann_dressed_moment0_90f23736:

    # Max_02 "А нет, это были твои прекрасные сосочки!"
    Max_02 "Et non, c'était tes magnifiques tétons !"

# game/events/ann.rpy:406
translate french ann_dressed_moment0_8e8d5338:

    # Ann_05 "Ну и шуточки у тебя, Макс! Завязывай с этими глупостями и дай маме спокойно переодеться." nointeract
    Ann_05 "Quelle blague tu fais, Max ! Arrêtez ces bêtises et laissez Mum changer en paix." nointeract

# game/events/ann.rpy:408
translate french ann_dressed_moment0_84eb80a2:

    # Ann_05 "Макс! Тебе что, мало того, что ты уже увидел? Завязывай с этими глупостями и дай маме спокойно переодеться." nointeract
    Ann_05 "Max ! Tu n'en as pas déjà vu assez ? Arrêtez ces bêtises et laissez votre mère changer en paix." nointeract

# game/events/ann.rpy:412
translate french ann_dressed_moment0_ad028372:

    # Ann_05 "Только под ноги смотри, сынок. А то мало ли куда лбом врежешься..." nointeract
    Ann_05 "Fais attention à ce que tu fais, fiston. Tu ne veux pas te cogner le front..." nointeract

# game/events/ann.rpy:427
translate french ann_dressed_moment2_a9e5c75c:

    # Ann_15 "Макс! Я же учила тебя стучаться! {p=3}{nw}"
    Ann_15 "Max! Je t'ai appris à frapper! {p=3}{nw}"

# game/events/ann.rpy:429
translate french ann_dressed_moment2_babcea3a:

    # Ann_15 "Макс! А стучаться кто будет?! {p=3}{nw}"
    Ann_15 "Max! Qui va frapper?! {p=3}{nw}"

# game/events/ann.rpy:431
translate french ann_dressed_moment2_126ffe76:

    # Ann_13 "Ой, Макс! Почему не стучишься? {p=3}{nw}"
    Ann_13 "Oh, Max ! Pourquoi tu ne frappes pas ? {p=3}{nw}"

# game/events/ann.rpy:441
translate french ann_dressed_moment2_aa9214ef:

    # Ann_14 "Нельзя вот так без предупреждения врываться в комнату! Что-то случилось?" nointeract
    Ann_14 "vous ne Pouvez pas entrer dans la pièce sans prévenir! Quelque chose s'est passé?" nointeract

# game/events/ann.rpy:444
translate french ann_dressed_moment2_c391d1e4:

    # Ann_12 "Прекращай уже без предупреждения врываться в комнату! Или у тебя что-то срочное?" nointeract
    Ann_12 "Arrêtez d'entrer dans la pièce sans avertissement! Ou tu as quelque chose d'urgent?" nointeract

# game/events/ann.rpy:447
translate french ann_dressed_moment2_2cd5729c:

    # Ann_02 "Ты что-то хотел или просто от безделья маешься?"
    Ann_02 "Tu voulais quelque chose ou tu es juste en train de traîner ?"

# game/events/ann.rpy:450
translate french ann_dressed_moment2_2cd5729c_1:

    # Ann_02 "Ты что-то хотел или просто от безделья маешься?"
    Ann_02 "Tu voulais quelque chose ou tu es juste en train de traîner ?"

# game/events/ann.rpy:466
translate french ann_dressed_moment1_1f19d7dd:

    # Ann_15 "Макс! Я же учила тебя стучаться! Нельзя вот так без предупреждения врываться в комнату! Что-то случилось?" nointeract
    Ann_15 "Max! Je t'ai appris à frapper! Vous ne pouvez pas entrer dans la pièce sans prévenir! Quelque chose s'est passé?" nointeract

# game/events/ann.rpy:469
translate french ann_dressed_moment1_e1ca91d3:

    # Ann_15 "Макс! А стучаться кто будет?! Прекращай уже без предупреждения врываться в комнату! Или у тебя что-то срочное?" nointeract
    Ann_15 "Max! Qui va frapper?! Arrête d'entrer dans la pièce sans prévenir! Ou tu as quelque chose d'urgent?" nointeract

# game/events/ann.rpy:472
translate french ann_dressed_moment1_dfc78518:

    # Ann_13 "Ой, Макс! Почему не стучишься? Ты что-то хотел или просто от безделья маешься?"
    Ann_13 "Oh, Max ! Pourquoi tu ne frappes pas ? Tu voulais quelque chose, ou tu ne fais que traîner dans le coin ?"

# game/events/ann.rpy:475
translate french ann_dressed_moment1_dfc78518_1:

    # Ann_13 "Ой, Макс! Почему не стучишься? Ты что-то хотел или просто от безделья маешься?"
    Ann_13 "Oh, Max ! Pourquoi tu ne frappes pas ? Tu voulais quelque chose, ou tu ne fais que traîner dans le coin ?"

# game/events/ann.rpy:481
translate french ann_dressed_lvl_1_3e8f5598:

    # Ann_12 "Очень здорово, Макс! Но сначала, ты закроешь дверь и я спокойно переоденусь, а уже после этого посмотрим, что у тебя там такое срочное..."
    Ann_12 "Très cool, Max ! Mais d'abord, tu fermes la porte et je vais me changer en paix et ensuite nous verrons ce qui est si urgent..."

# game/events/ann.rpy:482
translate french ann_dressed_lvl_1_c8682e5b:

    # Max_00 "Конечно, мам!"
    Max_00 "Bien sûr, maman!"

# game/events/ann.rpy:484
translate french ann_dressed_lvl_1_a94cbdaa:

    # Max_00 "{m}Пожалуй, не стоило вот так врываться к маме... Надеюсь, подарок всё сгладит.{/m}"
    Max_00 "{m}Peut-être que je n'aurais pas dû aller voir ma mère comme ça... J'espère que le cadeau va tout lisser.{/m}"

# game/events/ann.rpy:490
translate french ann_dressed_lvl_1_0e39c2e7:

    # Ann_17 "Что?! Макс! Это что ещё за словечки такие? А ну-ка быстро выйди и закрой дверь!" nointeract
    Ann_17 "Quoi?! Max! C'est quoi ces mots? Sortez et fermez la porte!" nointeract

# game/events/ann.rpy:495
translate french ann_dressed_lvl_1_3f53b608:

    # Ann_17 "Что я слышу! Вряд ли я та женщина, которой стоит такое говорить, Макс. А теперь выйди и закрой дверь!" nointeract
    Ann_17 "Ce que j'entends! Je ne suis pas la femme à qui dire ça, Max." nointeract

# game/events/ann.rpy:500
translate french ann_dressed_lvl_1_d8e67a23:

    # Ann_13 "Как я переодеваюсь?! Сынок, ты что такое говоришь! Давай-ка выйди и закрой за собой дверь." nointeract
    Ann_13 "Comment je me change?! Fiston, qu'est-ce que tu dis? Sors et ferme la porte derrière toi." nointeract

# game/events/ann.rpy:503
translate french ann_dressed_lvl_1_bb28ab00:

    # Ann_12 "Спасибо, конечно. Но... Макс, не мог бы ты подождать за дверью, пока я оденусь?" nointeract
    Ann_12 "Merci, bien sûr. Mais... Max, peux-tu attendre devant la porte pendant que je m'habille?" nointeract

# game/events/ann.rpy:511
translate french ann_dressed_lvl_2_3e8f5598:

    # Ann_12 "Очень здорово, Макс! Но сначала, ты закроешь дверь и я спокойно переоденусь, а уже после этого посмотрим, что у тебя там такое срочное..."
    Ann_12 "Très cool, Max ! Mais d'abord, tu fermes la porte et je vais me changer en paix et ensuite nous verrons ce qui est si urgent..."

# game/events/ann.rpy:512
translate french ann_dressed_lvl_2_c8682e5b:

    # Max_00 "Конечно, мам!"
    Max_00 "Bien sûr, maman!"

# game/events/ann.rpy:514
translate french ann_dressed_lvl_2_a94cbdaa:

    # Max_00 "{m}Пожалуй, не стоило вот так врываться к маме... Надеюсь, подарок всё сгладит.{/m}"
    Max_00 "{m}Peut-être que je n'aurais pas dû aller voir ma mère comme ça... J'espère que le cadeau va tout lisser.{/m}"

# game/events/ann.rpy:520
translate french ann_dressed_lvl_2_d73d7740:

    # Ann_13 "Макс! И не стыдно тебе такое своей маме говорить, а? Ну-ка бегом за дверь, а то мешаешь." nointeract
    Ann_13 "Max! Tu n'as pas honte de dire ça à ta mère, hein? On court par la porte ou on s'en mêle." nointeract

# game/events/ann.rpy:525
translate french ann_dressed_lvl_2_307c57f0:

    # Ann_13 "Даже не знаю, что на это ответить... Ну-ка бегом за дверь, а то засмущал меня." nointeract
    Ann_13 "je ne sais Même pas quoi répondre... J'ai couru par la porte, ou j'ai ri." nointeract

# game/events/ann.rpy:529
translate french ann_dressed_lvl_2_659d5d18:

    # Ann_12 "Нет, конечно! Ишь ты, что удумал! Завязывай с этими глупостями и дай маме спокойно переодеться." nointeract
    Ann_12 "non, bien sûr! C'est toi qui as pensé! Arrête ces conneries et laisse ta mère se changer." nointeract

# game/events/ann.rpy:533
translate french ann_dressed_lvl_2_a255adec:

    # Ann_02 "Приятно слышать. Но маме нужно переодеться... Так что подожди за дверью, хорошо?" nointeract
    Ann_02 "Agréable à entendre. Mais maman doit se changer... Alors attends dehors, d'accord?" nointeract

# game/events/ann.rpy:539
translate french ann_dressed_lvl_3_48432cd3:

    # Max_01 "Нет, просто решил заглянуть. А ты что, всё ещё стесняешься? Это же я!"
    Max_01 "Non, j'ai juste pensé que je pouvais passer. Tu es toujours timide ? C'est moi !"

# game/events/ann.rpy:544
translate french ann_dressed_lvl_3_50567e5f:

    # Ann_04 "Да это я так, по привычке. Если у тебя ничего срочного, то выйди пожалуйста. Мне нужно переодеться..." nointeract
    Ann_04 "J'en fais juste une habitude. S'il n'y a rien d'urgent, s'il vous plaît, sortez. J'ai besoin de changer..." nointeract

# game/events/ann.rpy:546
translate french ann_dressed_lvl_3_fa70085d:

    # Ann_04 "Конечно стесняюсь! Ещё я тут всю себя на показ не выставляла, только из-за того, что ты уже всё видел. Если у тебя ничего срочного, то выйди пожалуйста. Мне нужно переодеться..." nointeract
    Ann_04 "Bien sûr que je le suis ! Aussi, je ne me suis pas mis en valeur ici, juste parce que vous avez déjà tout vu. S'il n'y a rien d'urgent, s'il vous plaît, sortez. J'ai besoin de changer..." nointeract

# game/events/ann.rpy:549
translate french ann_dressed_lvl_3_659d5d18:

    # Ann_12 "Нет, конечно! Ишь ты, что удумал! Завязывай с этими глупостями и дай маме спокойно переодеться." nointeract
    Ann_12 "non, bien sûr! C'est toi qui as pensé! Arrête ces conneries et laisse ta mère se changer." nointeract

# game/events/ann.rpy:551
translate french ann_dressed_lvl_3_e4ef4b48:

    # Ann_05 "Приятно слышать, сынок. Спасибо, что подметил." nointeract
    Ann_05 "C'est bon à entendre, fils. Merci de l'avoir souligné." nointeract

# game/events/ann.rpy:557
translate french ann_dressed_lvl_4_2af495cf:

    # Max_02 "Нет, просто решил заглянуть и полюбоваться на твою шикарную фигуру. Ты же не против?"
    Max_02 "Non, j'ai juste pensé m'arrêter pour admirer votre superbe silhouette. Ça ne vous dérange pas, n'est-ce pas ?"

# game/events/ann.rpy:558
translate french ann_dressed_lvl_4_b937a903:

    # Ann_04 "Ну... Если налюбовался, то выйди пожалуйста. Мне нужно переодеться..." nointeract
    Ann_04 "Eh bien... Si vous avez eu votre dose, sortez. J'ai besoin de changer..." nointeract

# game/events/ann.rpy:561
translate french ann_dressed_lvl_4_84eb80a2:

    # Ann_05 "Макс! Тебе что, мало того, что ты уже увидел? Завязывай с этими глупостями и дай маме спокойно переодеться." nointeract
    Ann_05 "Max ! Tu n'en as pas déjà vu assez ? Arrêtez ces bêtises et laissez votre mère changer en paix." nointeract

# game/events/ann.rpy:565
translate french ann_dressed_lvl_4_ad028372:

    # Ann_05 "Только под ноги смотри, сынок. А то мало ли куда лбом врежешься..." nointeract
    Ann_05 "Fais attention à ce que tu fais, fiston. Tu ne veux pas te cogner le front..." nointeract

# game/events/ann.rpy:575
translate french ann_dressed_gift_a14e771b:

    # Ann_01 "Ну вот, я одета. Ты сказал, что у тебя что-то есть для меня?! О чём это ты?"
    Ann_01 "Voilà, je suis habillé. Tu as dit que tu avais quelque chose pour moi ?! De quoi parles-tu ?"

# game/events/ann.rpy:576
translate french ann_dressed_gift_efc9083b:

    # Max_04 "У меня для тебя подарок! Ночнушка!"
    Max_04 "J'ai un cadeau pour toi! Une nuisette!"

# game/events/ann.rpy:577
translate french ann_dressed_gift_e131ac39:

    # Ann_06 "Ты это серьёзно? Но в честь чего?"
    Ann_06 "Tu es sérieux ? Mais en l'honneur de quoi ?"

# game/events/ann.rpy:578
translate french ann_dressed_gift_860498c1:

    # Max_05 "Просто ты - самая лучшая мама на свете!"
    Max_05 "Tu es juste la meilleure maman du monde !"

# game/events/ann.rpy:579
translate french ann_dressed_gift_56ae1652:

    # Ann_08 "Ох, Макс, ты мне льстишь! Это так... неожиданно! Спасибо тебе мой милый, я очень тронута!"
    Ann_08 "Oh, Max, tu me flattes ! C'est vrai... de manière inattendue ! Merci mon chérie, je suis très touchée !"

# game/events/ann.rpy:580
translate french ann_dressed_gift_60f1c9d7:

    # Max_03 "Может, примеришь?"
    Max_03 "Pourquoi ne l'essaies-tu pas ?"

# game/events/ann.rpy:581
translate french ann_dressed_gift_f5c9d21b:

    # Ann_06 "Примерить? Для тебя? Ну... ладно... Думаю, ты это заслужил. Подожди, пожалуйста, за дверью..."
    Ann_06 "L'essayer ? Pour toi? Eh bien... Ok... Je pense que tu le mérites. Vas attendre devant la porte..."

# game/events/ann.rpy:582
translate french ann_dressed_gift_cc0857da:

    # Max_01 "Хорошо, мам."
    Max_01 "D’accord, maman."

# game/events/ann.rpy:584
translate french ann_dressed_gift_dec66268:

    # Ann "{b}Анна:{/b} Ничего себе, она полупрозрачная! Дорогой, ты же понимаешь, что твоя мама не может показаться в этом перед сыном..."
    Ann "{b}Anna:{/b} Wow, c'est translucide ! Chéri, tu réalises que ta mère ne peut pas montrer sa silhouette devant son fils..."

# game/events/ann.rpy:585
translate french ann_dressed_gift_461f061a:

    # Max_10 "Тебе не понравился подарок?!"
    Max_10 "Tu n'as pas aimé ton cadeau ?!"

# game/events/ann.rpy:586
translate french ann_dressed_gift_0f666c00:

    # Ann "{b}Анна:{/b} Нет, мне очень нравится! Это прекрасный подарок! Только вот, тебе не кажется, что ты ещё слишком мал, чтобы делать подобные подарки?"
    Ann "{b}Anna:{/b} Non, je l'adore ! C'est un merveilleux cadeau ! Seulement, tu ne crois pas que tu es trop jeune pour offrir de tels cadeaux ?"

# game/events/ann.rpy:587
translate french ann_dressed_gift_00ed580c:

    # Max_09 "Я уже большой, мам! Я же от души!"
    Max_09 "Je suis un grand garçon, maman ! C'est du fond du cœur !"

# game/events/ann.rpy:588
translate french ann_dressed_gift_f9e66542:

    # Ann "{b}Анна:{/b} Ох, Макс, ты меня смущаешь, такой откровенный подарок, да ещё родной матери... Но всё равно, я очень это ценю... и ещё раз огромное спасибо!"
    Ann "{b}Anna:{/b} Oh, Max, tu m’embarrasses, un cadeau si sexy, et à ta propre mère... Mais quand même, je l’apprécie vraiment... et merci encore!"

# game/events/ann.rpy:589
translate french ann_dressed_gift_75fcb6dd:

    # Max_02 "Думаю, смотрится она на тебе просто фантастически!"
    Max_02 "je pense que ça a l’air fantastique sur toi!"

# game/events/ann.rpy:593
translate french ann_dressed_gift_d954a734:

    # Ann_08 "Ох... Спасибо за комплимент, мой милый. Сразу видно, что мой сын настоящий мужчина! Иди ко мне, я тебя обниму..."
    Ann_08 "Oh... Merci pour le compliment, mon chéri. Il est évident que mon fils est un vrai homme! Viens ici, je vais te faire un câlin..."

# game/events/ann.rpy:598
translate french ann_dressed_gift_26390c24:

    # Max_05 "{m}О да... У меня действительно лучшая мама на свете! Какая же потрясающая у неё фигура... Так приятно прижиматься к ней... её упругой груди... Эту мечту не хочется отпускать!{/m}"
    Max_05 "{m}Ah oui... J'ai vraiment la meilleure maman du monde! Quelle silhouette éblouissante elle a... C'est si bon de se blottir contre elle... ses seins fermes... C'est un rêve que je ne veux pas lâcher !{/m}"

# game/events/ann.rpy:603
translate french ann_dressed_gift_cda3b4f9:

    # Ann_04 "Ну всё, мой дорогой, нам с девочками ещё нужно успеть пробежаться по магазинам сегодня..." nointeract
    Ann_04 "C'est tout, mon chéri, les filles et moi avons encore des courses à faire aujourd'hui..." nointeract

# game/events/ann.rpy:605
translate french ann_dressed_gift_b6991faf:

    # Ann_04 "Ну всё, мой дорогой, мне уже скоро на работу и нужно успеть сделать ещё кое-какие дела..." nointeract
    Ann_04 "C'est ça, chéri, je dois aller travailler bientôt et il y a des choses à faire..." nointeract

# game/events/ann.rpy:610
translate french ann_dressed_gift_e66adddb:

    # Ann_05 "[succes!t]Ты сегодня очень мил, Макс! За это я тебя даже в щёчку поцелую, чтобы ты почаще старался меня радовать..."
    Ann_05 "[succes!t]Tu es très gentil aujourd'hui, Max ! Je te donnerai même un baiser sur la joue pour ça, pour que tu essaies de me faire plaisir plus souvent..."

# game/events/ann.rpy:613
translate french ann_dressed_gift_05855820:

    # Max_06 "{m}Ого! Это даже больше того, на что я надеялся... И не менее приятно чувствовать прикосновение её губ на своём лице! Блаженно...{/m}"
    Max_06 "{m}Wow ! C'est encore plus que ce que j'avais espéré... Et c'est tout aussi bon de sentir le contact de ses lèvres sur mon visage ! Béatement...{/m}"

# game/events/ann.rpy:619
translate french ann_dressed_gift_c0df5b86:

    # Ann_04 "А теперь иди, сынок... Пора заниматься делами." nointeract
    Ann_04 "Maintenant va, fils... Il est temps de passer aux choses sérieuses." nointeract

# game/events/ann.rpy:639
translate french ann_dressed_fail_f49b0b30:

    # Ann_01 "[_text!t]Макс, мне нужно ещё успеть сделать кое-какие дела... Давай, сынок, иди... Займись чем-нибудь." nointeract
    Ann_01 "[_text!t]Max, j'ai des choses à faire... Vas-y, fils, vas-y... Faites quelque chose..." nointeract

# game/events/ann.rpy:641
translate french ann_dressed_fail_a9cf1e2d:

    # Ann_01 "[_text!t]Макс, я так на работу не успею собраться... Давай, сынок, иди... Пора заниматься делами." nointeract
    Ann_01 "[_text!t]Max, je n'aurai pas le temps de me préparer pour le travail... Vas-y, fils, vas-y... Il est temps de passer aux choses sérieuses." nointeract

# game/events/ann.rpy:649
translate french ann_dressed_loveyou_e41c68c6:

    # Ann_07 "И я тебя, Макс..."
    Ann_07 "Toi aussi, Max..."

# game/events/ann.rpy:653
translate french ann_dressed_goodday_1a01074c:

    # Ann_02 "Спасибо, сынок! И тебе тоже..."
    Ann_02 "Merci, mon fils ! Toi aussi..."

# game/events/ann.rpy:674
translate french ann_dressed_balcony_919b6a05:

    # Max_07 "{m}Хм... Должно быть мама на балконе...{/m}"
    Max_07 "{m}Hmm... Maman doit être sur le balcon...{/m}"

# game/events/ann.rpy:687
translate french ann_dressed_balcony_34f4b4a9:

    # Max_09 "{m}То ли мама просто задумалась, то ли грустит...{/m}" nointeract
    Max_09 "{m}si ma mère a juste réfléchi, si elle est triste...{/m}" nointeract

# game/events/ann.rpy:692
translate french ann_dressed_balcony_2fd8332a:

    # Ann_12 "Ой, Макс! Ты слишком тихо ходишь. А я так, ничего, просто задумалась немного..."
    Ann_12 "Oh, Max! Tu es trop tranquille. Et moi, c'est bon, j'ai juste réfléchi un peu..."

# game/events/ann.rpy:693
translate french ann_dressed_balcony_d6e397b3:

    # Max_07 "Расскажешь, о чём?"
    Max_07 "veux-tu me Dire de quoi?"

# game/events/ann.rpy:707
translate french ann_dressed_balcony_2e72dc0c:

    # Max_05 "{m}Оу! Мама ещё не оделась... Это я вовремя зашёл. Она шикарно смотрится...{/m}" nointeract
    Max_05 "{m}l'uranium appauvri! Maman n'est pas encore habillée... C'est moi qui suis arrivé à l'heure. Elle l'élégant semble...{/m}" nointeract

# game/events/ann.rpy:715
translate french ann_dressed_balcony_1f19d7dd:

    # Ann_15 "Макс! Я же учила тебя стучаться! Нельзя вот так без предупреждения врываться в комнату! Что-то случилось?" nointeract
    Ann_15 "Max! Je t'ai appris à frapper! Vous ne pouvez pas entrer dans la pièce sans prévenir! Quelque chose s'est passé?" nointeract

# game/events/ann.rpy:718
translate french ann_dressed_balcony_e1ca91d3:

    # Ann_15 "Макс! А стучаться кто будет?! Прекращай уже без предупреждения врываться в комнату! Или у тебя что-то срочное?" nointeract
    Ann_15 "Max! Qui va frapper?! Arrête d'entrer dans la pièce sans prévenir! Ou tu as quelque chose d'urgent?" nointeract

# game/events/ann.rpy:726
translate french ann_dressed_balcony_a9e5c75c:

    # Ann_15 "Макс! Я же учила тебя стучаться! {p=3}{nw}"
    Ann_15 "Max! Je t'ai appris à frapper! {p=3}{nw}"

# game/events/ann.rpy:728
translate french ann_dressed_balcony_babcea3a:

    # Ann_15 "Макс! А стучаться кто будет?! {p=3}{nw}"
    Ann_15 "Max! Qui va frapper?! {p=3}{nw}"

# game/events/ann.rpy:731
translate french ann_dressed_balcony_aa9214ef:

    # Ann_14 "Нельзя вот так без предупреждения врываться в комнату! Что-то случилось?" nointeract
    Ann_14 "vous ne Pouvez pas entrer dans la pièce sans prévenir! Quelque chose s'est passé?" nointeract

# game/events/ann.rpy:734
translate french ann_dressed_balcony_c391d1e4:

    # Ann_12 "Прекращай уже без предупреждения врываться в комнату! Или у тебя что-то срочное?" nointeract
    Ann_12 "Arrêtez d'entrer dans la pièce sans avertissement! Ou tu as quelque chose d'urgent?" nointeract

# game/events/ann.rpy:745
translate french ann_dressed_balkon_talk1_b906cb2e:

    # Ann_14 "У меня сейчас такое состояние, что я не знаю, как мне жить дальше... Я... Я ужасная мать..."
    Ann_14 "j'ai un tel état que je ne sais pas comment vivre... Moi... Je suis une mère horrible..."

# game/events/ann.rpy:746
translate french ann_dressed_balkon_talk1_b034aec8:

    # Max_08 "Что?! Как ты можешь такое говорить?"
    Max_08 "Quoi?! Comment peux-tu est-ce à dire?"

# game/events/ann.rpy:747
translate french ann_dressed_balkon_talk1_79bc09d7:

    # Ann_13 "А разве нет? Я... Я была так глупа, что привела в дом проходимца, который не только воспользовался мной, но и чуть не совратил моих девочек!"
    Ann_13 "n'Est-ce pas? Moi... J'étais tellement stupide que j'ai amené un voyou dans la maison qui a non seulement profité de moi, mais a failli séduire mes filles!"

# game/events/ann.rpy:748
translate french ann_dressed_balkon_talk1_de50c165:

    # Max_09 "Мам, прекрати, это уже всё в прошлом... Забудь..."
    Max_09 "maman, arrête, c'est du passé... Oublie..."

# game/events/ann.rpy:749
translate french ann_dressed_balkon_talk1_37635b4b:

    # Ann_14 "Забыть? Забыть, как я чуть не отправила единственного сына в военный лагерь и... который один оказался настолько умным и благородным, что не только не разозлился на меня, а наоборот, спас всю семью от этого ублюдка - Эрика."
    Ann_14 "Oublier? J'ai failli envoyer mon fils unique dans un camp militaire... celui qui était si intelligent et noble que non seulement il ne s'est pas fâché contre moi, mais au contraire, il a sauvé toute la famille de ce bâtard - Eric."

# game/events/ann.rpy:750
translate french ann_dressed_balkon_talk1_ac19285d:

    # Max_07 "Ну хватит... Было и было. Прими это и сделай выводы. Нельзя же всё время теперь ходить такой печальной и заниматься самобичеванием. Ты ни в чём не виновата!"
    Max_07 "ça suffit... C'était et c'était. Prends ça et tire des conclusions. Vous ne pouvez pas tout le temps maintenant aller si triste et faire de l'auto-flagellation. Tu n'es pas coupable de rien!"

# game/events/ann.rpy:751
translate french ann_dressed_balkon_talk1_e8140f63:

    # Ann_17 "Возможно ты прав, но... присутствие Эрика на работе постоянно мне напоминает о тех событиях и мне это ужасно неприятно."
    Ann_17 "Peut-être que tu as raison, mais... la présence d'Eric au travail me rappelle constamment ces événements et c'est terriblement désagréable pour moi."

# game/events/ann.rpy:752
translate french ann_dressed_balkon_talk1_63d3ad78:

    # Max_15 "Да и чёрт с ним! Просто игнорируй его. К тому же, у тебя есть мы, и мы все тебя очень любим!"
    Max_15 "et merde! Ignore-le. En plus, tu nous as, et nous t'aimons tous beaucoup!"

# game/events/ann.rpy:753
translate french ann_dressed_balkon_talk1_6335970c:

    # Ann_02 "Спасибо, мой хороший, за твои тёплые слова и поддержку, мне очень приятно это слышать."
    Ann_02 "Merci, mon bon, pour vos paroles aimables et votre soutien, je suis très heureux de l'entendre."

# game/events/ann.rpy:754
translate french ann_dressed_balkon_talk1_59e5166a:

    # Max_04 "Не стоит благодарить, мам. Я очень тебя люблю! Ты самая замечательная мама на свете! Самая добрая и самая красивая!"
    Max_04 "ne remerciez pas, maman. Je t'aime beaucoup! Tu es la plus belle maman du monde! Le plus gentil et le plus beau!"

# game/events/ann.rpy:757
translate french ann_dressed_balkon_talk1_d77c6ad5:

    # Ann_04 "Ой! Вот же ты маленький льстец, Макс!"
    Ann_04 "Oh! Tu es un peu flatteur, Max!"

# game/events/ann.rpy:758
translate french ann_dressed_balkon_talk1_5e9b1161:

    # Max_07 "Это я-то маленький?"
    Max_07 "c'est moi le petit?"

# game/events/ann.rpy:759
translate french ann_dressed_balkon_talk1_5eb0124c:

    # Ann_05 "Ну, конечно, ты большой, Макс! Совсем взрослый, почти мужчина."
    Ann_05 "bien sûr que tu es grand, Max! Un adulte, presque un homme."

# game/events/ann.rpy:760
translate french ann_dressed_balkon_talk1_f86a1ee3:

    # Max_09 "Что значит почти?"
    Max_09 "Que signifie presque?"

# game/events/ann.rpy:761
translate french ann_dressed_balkon_talk1_2e6b2825:

    # Ann_07 "Ну... То и значит, что полноценным мужчиной ты станешь только после того, как у тебя появится девушка."
    Ann_07 "Eh bien... Cela signifie que vous deviendrez un homme à part entière seulement après que vous ayez une fille."

# game/events/ann.rpy:762
translate french ann_dressed_balkon_talk1_a8851b6c:

    # Max_08 "Серьёзно? А поподробнее можно узнать, как и что делать с ней, чтобы стать мужчиной?"
    Max_08 "Sérieusement? Et plus en détail, vous pouvez apprendre comment et quoi faire avec elle pour devenir un homme?"

# game/events/ann.rpy:763
translate french ann_dressed_balkon_talk1_f041c776:

    # Ann_02 "Макс, что за вопросы? Как будто ты до сих пор не знаешь."
    Ann_02 "Max, quelles sont les questions? Comme si tu ne savais toujours pas."

# game/events/ann.rpy:764
translate french ann_dressed_balkon_talk1_d7d0349c:

    # Max_02 "Да это я просто так спросил... Теперь ты уже не такая печальная и задумчивая, как до моего прихода."
    Max_02 "C'est moi qui ai demandé ça... Maintenant, tu n'es plus aussi triste et réfléchie qu'avant mon arrivée."

# game/events/ann.rpy:765
translate french ann_dressed_balkon_talk1_1c8db60a:

    # Ann_05 "Вот же ты хитрец! Нашёл чем меня отвлечь..."
    Ann_05 "Tu es un malin! J'ai trouvé quelque chose pour me distraire..."

# game/events/ann.rpy:766
translate french ann_dressed_balkon_talk1_3696543d:

    # Max_03 "Могу отвлечь чем-нибудь приятным. Например, массажем спины. Думаю, это тебе бы сейчас помогло. Хочешь?"
    Max_03 "Je peux distraire quelque chose d'agréable. Par exemple, massage du dos. Je pense que ça t'aiderait maintenant. Tu veux?"

# game/events/ann.rpy:767
translate french ann_dressed_balkon_talk1_eb82677e:

    # Ann_04 "Нет, Макс, сейчас мне некогда. Давай вечерком, сынок, у ТВ."
    Ann_04 "non, Max, Je n'ai pas le temps. Allez, fiston, à la télé."

# game/events/ann.rpy:768
translate french ann_dressed_balkon_talk1_a3767c9d:

    # Max_01 "Договорились, мам. И не забывай, если что, я всегда рядом. Обращайся!"
    Max_01 "d'Accord, maman. Et n'oublie pas que je suis toujours là. Tournez-vous!"

# game/events/ann.rpy:769
translate french ann_dressed_balkon_talk1_59d69cc0:

    # Ann_01 "Обязательно!" nointeract
    Ann_01 "Obligatoire!" nointeract

# game/events/ann.rpy:788
translate french ann_dressed_balkon_talk2_640d08ea:

    # Ann_14 "Это всё моя работа... Присутствие Эрика делает её просто невыносимой."
    Ann_14 "C'est mon travail... La présence d'Eric la rend insupportable."

# game/events/ann.rpy:789
translate french ann_dressed_balkon_talk2_799ed018:

    # Max_15 "Он пристаёт к тебе?"
    Max_15 "est-ce qu'il te colle?"

# game/events/ann.rpy:790
translate french ann_dressed_balkon_talk2_624bc582:

    # Ann_18 "Нет, но его ехидная ухмылочка и постоянные намёки о возможных неприятностях, если я вдруг попытаюсь уволиться, сводят меня с ума."
    Ann_18 "Non, mais son sourire malicieux et ses insinuations constantes sur les problèmes possibles, si j'essaie soudainement de démissionner, me rendent fou."

# game/events/ann.rpy:791
translate french ann_dressed_balkon_talk2_0ce3e59e:

    # Max_09 "Просто постарайся не обращать на него внимание."
    Max_09 "essaie juste de ne pas faire attention à lui."

# game/events/ann.rpy:792
translate french ann_dressed_balkon_talk2_a8ecc8ef:

    # Ann_12 "Вряд ли это возможно, сынок, он же мой начальник." nointeract
    Ann_12 "ce n'est Pas possible, fils, c'est mon patron." nointeract

# game/events/ann.rpy:799
translate french ann_dressed_balkon_talk2_9803998b:

    # Ann_13 "Макс, ты что это делаешь? За зад меня обнимаешь?!"
    Ann_13 "Max, qu'est-ce que tu fais? Tu me sers le cul?!"

# game/events/ann.rpy:800
translate french ann_dressed_balkon_talk2_f912aa8d:

    # Max_07 "Ну да. Это я так приободрить тебя решил. Если бы он действительно хотел тебе что-то сделать, то он бы уже давно это сделал."
    Max_07 "Oui. C'est moi qui ai décidé de t'encourager. S'il voulait vraiment te faire quelque chose, il l'aurait fait depuis longtemps."

# game/events/ann.rpy:801
translate french ann_dressed_balkon_talk2_769cdc0b:

    # Ann_17 "Сынок, такое вот твое приободрение не работает. Оно меня скорее смущает..."
    Ann_17 "Fiston, ça ne marche pas comme ça. Ça me gêne plutôt..."

# game/events/ann.rpy:805
translate french ann_dressed_balkon_talk2_0167e576:

    # Max_09 "Ладно, уберу... Просто Эрику нравится играть на нервах, вот и всё, что он может сделать."
    Max_09 "Ok, je vais nettoyer... C'est juste qu'Eric aime jouer sur ses nerfs, c'est tout ce qu'il peut faire."

# game/events/ann.rpy:806
translate french ann_dressed_balkon_talk2_273fc7d0:

    # Ann_12 "Возможно так и есть, но... мне бы очень не хотелось искушать судьбу... это ведь может отразиться и на вас, моих детях, а вы для меня самое дорогое."
    Ann_12 "c'est Peut-être le cas, mais... je ne voudrais pas tenter le destin... cela peut aussi affecter vous, mes enfants, et vous êtes le plus cher pour moi."

# game/events/ann.rpy:807
translate french ann_dressed_balkon_talk2_682e32bc:

    # Max_10 "Ты тоже самое дорогое для меня... для нас... И нам бы не хотелось видеть тебя такой печальной, мам."
    Max_10 "Tu es le plus cher pour moi... pour nous... Et on ne voudrait pas te voir aussi triste, maman."

# game/events/ann.rpy:808
translate french ann_dressed_balkon_talk2_261a6f2e:

    # Ann_02 "Спасибо, Макс. Мне очень приятно это слышать. Как хорошо, что у меня есть такой внимательный и заботливый сын."
    Ann_02 "Merci, Max. C'est bien d'avoir un fils aussi attentionné et attentionné."

# game/events/ann.rpy:809
translate french ann_dressed_balkon_talk2_23203ebd:

    # Max_07 "В любом случае, если ты захочешь сменить место работы и у нас, вдруг, появятся сложности с деньгами, мы все тебя поймём и поддержим."
    Max_07 "Quoi qu'il en soit, si tu veux changer de lieu de travail et que nous avons soudainement des difficultés avec l'argent, nous te comprendrons et te soutiendrons."

# game/events/ann.rpy:810
translate french ann_dressed_balkon_talk2_bed6c909:

    # Ann_04 "Спасибо, мой хороший, я подумаю об этом..."
    Ann_04 "Merci, mon bon, je vais y réfléchir..."

# game/events/ann.rpy:811
translate french ann_dressed_balkon_talk2_d9948e58:

    # Max_09 "Обязательно подумай. Нужно покончить с этим прошлым раз и навсегда!"
    Max_09 "réfléchis. Nous devons mettre fin à ce passé une fois pour toutes!"

# game/events/ann.rpy:812
translate french ann_dressed_balkon_talk2_09e80fa0:

    # Ann_05 "Хм... Глядя на тебя в последнее время, я вижу, что ты очень повзрослел."
    Ann_05 "Hmm... En te regardant ces derniers temps, je vois que tu es très mature."

# game/events/ann.rpy:813
translate french ann_dressed_balkon_talk2_cdbf49b6:

    # Max_04 "Ну, тебе виднее."
    Max_04 "Eh bien, vous savez mieux."

# game/events/ann.rpy:814
translate french ann_dressed_balkon_talk2_650e344f:

    # Ann_08 "Так и есть. И мне нравится то, что я вижу..."
    Ann_08 "C'est vrai. Et j'aime ce que je vois..."

# game/events/ann.rpy:815
translate french ann_dressed_balkon_talk2_40bdc5b1:

    # Max_03 "Главное не унывай, мам. Всё образуется."
    Max_03 "Ne vous découragez pas, maman. Tout se forme."

# game/events/ann.rpy:816
translate french ann_dressed_balkon_talk2_ed9bd79e:

    # Ann_07 "Хорошо! Спасибо, что поддержал меня и развеял тоску."
    Ann_07 "D'accord! Merci de m'avoir soutenu et d'avoir dissipé l'angoisse."

# game/events/ann.rpy:817
translate french ann_dressed_balkon_talk2_1c195d26:

    # Max_01 "Пожалуйста! Если что, я всегда рядом. Обращайся!"
    Max_01 "S’il te plaît! Au contraire, je suis toujours là. Parle!"

# game/events/ann.rpy:818
translate french ann_dressed_balkon_talk2_59d69cc0:

    # Ann_01 "Обязательно!" nointeract
    Ann_01 "Obligatoire!" nointeract

# game/events/ann.rpy:833
translate french ann_dressed_balkon_talk2_1f418bf5:

    # Max_09 "{m}Смущают её мои прикосновения... Ясно. Значит, нужно больше прикасаться тогда, когда это максимально естественно, чтобы она привыкла. Пожалуй, йога - то, что нужно! Стоит попробовать...{/m}"
    Max_09 "{m}elle est Embarrassée par mes touches... Clairement. Donc, vous devez toucher plus quand c'est aussi naturel que possible pour qu'elle s'habitue. Peut-être que le yoga est ce dont vous avez besoin! La peine d'essayer...{/m}"

# game/events/ann.rpy:842
translate french ann_dressed_balkon_r_4897a021:

    # Ann_02 "Ой, Макс! Как ты так тихо подкрадываешься? Да, любуюсь... А ты почему без дела слоняешься?" nointeract
    Ann_02 "Oh, Max! Comment tu te faufiles si doucement? Oui, j'admire... Pourquoi es-tu en train de traîner?" nointeract

# game/events/ann.rpy:845
translate french ann_dressed_balkon_r_0a883de4:

    # Ann_01 "Не то, чтобы мешаешь... Иди лучше чем-нибудь продуктивным займись. Маме пока некогда."
    Ann_01 "Pas que tu interfères... Tu ferais mieux de faire quelque chose de productif. Maman n'a pas encore le temps."

# game/events/ann.rpy:846
translate french ann_dressed_balkon_r_58738e49:

    # Max_00 "Ладно, не буду мешать." nointeract
    Max_00 "d'accord, je ne vais pas vous déranger." nointeract

# game/events/ann.rpy:852
translate french ann_dressed_balkon_r_2280510e:

    # Ann_04 "Нет. Побудь, если хочется. Я не против." nointeract
    Ann_04 "Non. Restez, si vous voulez. Je ne suis pas contre." nointeract

# game/events/ann.rpy:859
translate french ann_dressed_balkon_r_119f09c9:

    # Ann_02 "Как у тебя дела, Макс? Всё хорошо?"
    Ann_02 "comment ça va, Max? Est-ce que tout est bien?"

# game/events/ann.rpy:860
translate french ann_dressed_balkon_r_ca3ee871:

    # Max_04 "Да, порядок. А ты как? Не скучаешь?"
    Max_04 "Oui, Ok. Et toi? Tu ne te manques pas?"

# game/events/ann.rpy:861
translate french ann_dressed_balkon_r_7fdac7ed:

    # Ann_05 "Ох, сынок, с вами разве заскучаешь. Да и на работе дел хватает." nointeract
    Ann_05 "Oh, fiston, tu t'ennuies avec toi. Oui, et au travail, il y a assez de choses à faire." nointeract

# game/events/ann.rpy:866
translate french ann_dressed_balkon_r_55550510:

    # Max_07 "Ты, мам, побольше отдыхать не забывай."
    Max_07 "maman, n'oublie pas de te reposer."

# game/events/ann.rpy:867
translate french ann_dressed_balkon_r_081cb55f:

    # Ann_07 "Это я и делаю сейчас. А ты давай беги, займись чем-нибудь полезным."
    Ann_07 "c'est ce que je fais maintenant. Va faire quelque chose d'utile."

# game/events/ann.rpy:878
translate french ann_dressed_balkon_r_a9420db7:

    # Max_07 "Ты, мам, побольше отдыхать не забывай. У нас слишком прекрасный дом, чтобы грустить."
    Max_07 "maman, n'oublie pas de te reposer. Nous avons une maison trop belle pour être triste."

# game/events/ann.rpy:879
translate french ann_dressed_balkon_r_874166e7:

    # Ann_08 "Да, здесь здорово! Маленький рай. Ты зашёл меня просто вот так подбодрить?"
    Ann_08 "Oui, c'est génial! Petit paradis. Tu es venu me remonter le moral comme ça?"

# game/events/ann.rpy:883
translate french ann_dressed_balkon_r_6b6b7f92:

    # Max_02 "Ну, да. Хотел напомнить, что я всегда рядом."
    Max_02 "Eh bien, oui. Je voulais te rappeler que je suis toujours là."

# game/events/ann.rpy:884
translate french ann_dressed_balkon_r_680063a0:

    # Ann_07 "Спасибо, но лучше беги, займись чем-нибудь полезным. У меня всё в порядке."
    Ann_07 "Merci, mais tu ferais mieux de courir, de faire quelque chose d'utile. Chez moi tout est en ordre."

# game/events/ann.rpy:892
translate french ann_dressed_balkon_r_0cc8bfb1:

    # Max_03 "Ладно, не буду отвлекать. Я тебя люблю, мам!"
    Max_03 "d'accord, je ne vais pas vous distraire. Je t'aime, maman!"

# game/events/ann.rpy:893
translate french ann_dressed_balkon_r_dfe00014:

    # Ann_06 "Я тебя тоже, сынок." nointeract
    Ann_06 "moi aussi, fiston." nointeract

# game/events/ann.rpy:982
translate french ann_bath_fc2b25dc:

    # Max_00 "{m}Видимо, мама принимает ванну...{/m}" nointeract
    Max_00 "{m}Apparemment, maman prend un bain...{/m}" nointeract

# game/events/ann.rpy:992
translate french ann_bath_9d03c202:

    # Max_00 "{m}Эх... жаль, что стекло частично матовое. Так ничего не разглядеть! А если подобраться ближе, то мама может заметить...{/m}"
    Max_00 "{m}Eh... dommage que le verre soit partiellement dépoli. Donc, ne rien voir! Et si vous vous rapprochez, maman peut le remarquer...{/m}"

# game/events/ann.rpy:994
translate french ann_bath_7fffd1ec:

    # Max_09 "{m}Нужно что-нибудь придумать...{/m}" nointeract
    Max_09 "{m}Il faut trouver quelque chose...{/m}" nointeract

# game/events/ann.rpy:1001
translate french ann_bath_4a929722:

    # Max_01 "{m}Надеюсь, что ни у кого не возникнет вопроса, а что же здесь делает стремянка... Как, что? Конечно стоит, мало ли что! А теперь начинается самое интересное...{/m}"
    Max_01 "{m}j'Espère que personne n'aura de question, mais que fait l'escabeau ici... Comment, quoi? Bien sûr, ça vaut le coup! Et maintenant, le plaisir commence...{/m}"

# game/events/ann.rpy:1013
translate french ann_bath_ladder_76e8b72b:

    # Max_04 "{m}Посмотрим, что у нас тут...{/m}"
    Max_04 "{m}voyons Ce qu'on a ici...{/m}"

# game/events/ann.rpy:1023
translate french ann_bath_ladder_3a5db899:

    # Max_03 "{m}Ох, как горячо! Разумеется, я не про воду, а про её внешний вид. Ухх... Мама потрясающе выглядит...{/m}" nointeract
    Max_03 "{m}Oh, c'est chaud! Bien sûr, je ne parle pas de l'eau, mais de son apparence. Uhh... Maman a l'air incroyable...{/m}" nointeract

# game/events/ann.rpy:1029
translate french ann_bath_ladder_b37e9fe1:

    # Max_05 "{m}Ух ты, аж завораживает! Мамины водные процедуры могут посоперничать с самыми горячими эротическими роликами! Эта упругая грудь и эти длинные стройные ножки сведут с ума кого угодно...{/m}" nointeract
    Max_05 "{m}Wow, c'est fascinant! Les procédures d'eau de maman peuvent rivaliser avec les vidéos érotiques les plus chaudes! Cette poitrine élastique et ces longues jambes minces rendront fou n'importe qui...{/m}" nointeract

# game/events/ann.rpy:1038
translate french ann_bath_ladder_b37e9fe1_1:

    # Max_05 "{m}Ух ты, аж завораживает! Мамины водные процедуры могут посоперничать с самыми горячими эротическими роликами! Эта упругая грудь и эти длинные стройные ножки сведут с ума кого угодно...{/m}" nointeract
    Max_05 "{m}Wow, c'est fascinant! Les procédures d'eau de maman peuvent rivaliser avec les vidéos érotiques les plus chaudes! Cette poitrine élastique et ces longues jambes minces rendront fou n'importe qui...{/m}" nointeract

# game/events/ann.rpy:1044
translate french ann_bath_ladder_87fd27ff:

    # Max_07 "{m}Эх! Похоже, самое интересное закончилось... Хотя, смотреть как мама вытирает своё мокрое и соблазнительное тело не менее приятно! Ох, какая же у неё попка...{/m}" nointeract
    Max_07 "{m}Hein! On dirait que le plus intéressant est terminé... Bien que, regarder maman essuyer son corps humide et séduisant n'est pas moins agréable! Oh, quel cul!..{/m}" nointeract

# game/events/ann.rpy:1053
translate french ann_bath_about_intime_0_3aa93926:

    # Ann "{b}Анна:{/b} Кто там? Я принимаю ванну!"
    Ann "{b}Anna:{/b} Qui est là ? Je prends un bain !"

# game/events/ann.rpy:1054
translate french ann_bath_about_intime_0_006aa2f9:

    # Max_07 "Это я, Макс. Можно войти?"
    Max_07 "c'est moi, Max?"

# game/events/ann.rpy:1055
translate french ann_bath_about_intime_0_765e20f4:

    # Ann "{b}Анна:{/b} Зачем, дорогой? Тебе что-то нужно?"
    Ann "{b}Anna: {/b} Pourquoi, chéri? T'a besoin de quelque chose?"

# game/events/ann.rpy:1056
translate french ann_bath_about_intime_0_cc4bed10:

    # Max_09 "Да, мам. Хотел поговорить."
    Max_09 "oui, maman. Voulait parler."

# game/events/ann.rpy:1057
translate french ann_bath_about_intime_0_a05b810e:

    # Ann "{b}Анна:{/b} Если это может подождать, то я через полчасика освобожусь..." nointeract
    Ann "{b}Anna:{/b} Si ça peut attendre, je serai dehors dans une demi-heure..." nointeract

# game/events/ann.rpy:1073
translate french ann_bath_about_intime_0_53cc7aca:

    # Ann_15 "Макс! Ты почему так нагло врываешься! Я же сказала, что скоро освобожусь! Не мог подождать?"
    Ann_15 "Max! Pourquoi tu t'insinues si effrontément? Je t'ai dit que je serais bientôt libre! Tu ne pouvais pas attendre?"

# game/events/ann.rpy:1074
translate french ann_bath_about_intime_0_1af3858f:

    # Max_08 "Нет, нужно кое-что обсудить, пока ты здесь."
    Max_08 "non, il y a quelque chose à discuter pendant que tu es là."

# game/events/ann.rpy:1079
translate french ann_bath_about_intime_0_01bf0619:

    # Ann_14 "И что там у тебя такого срочного?!"
    Ann_14 "qu'y a-t-il de si urgent?!"

# game/events/ann.rpy:1080
translate french ann_bath_about_intime_0_a34fa2a4:

    # Max_07 "Хотел поговорить о девочках!"
    Max_07 "Voulait parler des filles!"

# game/events/ann.rpy:1081
translate french ann_bath_about_intime_0_197357cf:

    # Ann_17 "И ты считаешь, что это настолько важно, что кроме как здесь об этом говорить нельзя, да?"
    Ann_17 "Et vous pensez que c'est si important que vous ne pouvez pas en parler ici, n'est-ce pas?"

# game/events/ann.rpy:1082
translate french ann_bath_about_intime_0_7db297d2:

    # Max_01 "Ну... Не просто о девочках, а том, как им... делать приятно... там, внизу..."
    Max_01 "Eh bien... Pas seulement sur les filles, mais sur leur façon... faire agréable... là-bas..."

# game/events/ann.rpy:1083
translate french ann_bath_about_intime_0_28faf8d1:

    # Ann_02 "Вот это вопросы у тебя, Макс! А что, у тебя девочка появилась?"
    Ann_02 "Ce sont les questions que vous avez, Max! C'est quoi votre fille est apparue?"

# game/events/ann.rpy:1084
translate french ann_bath_about_intime_0_4dd2948d:

    # Max_07 "Ещё нет, но появится же. А я хочу быть готовым! Вы мне с Эриком показали на тех уроках, как себя вести, если женщина... так сказать, ласкает мужчину, а вот как ласкать женщин - нет."
    Max_07 "pas Encore, mais le même apparaîtra. Je veux être prêt! Toi et Eric m'avez montré comment se comporter en femme... pour ainsi dire, caresse un homme, mais comment caresser les femmes - non."

# game/events/ann.rpy:1085
translate french ann_bath_about_intime_0_435b556c:

    # Ann_14 "Ой, сынок, какие же это неудобные вопросы! Я всё ещё до конца не могу осознать, что ты уже почти взрослый."
    Ann_14 "Oh, mon fils, quelles sont ces questions inconfortables! Je n'arrive toujours pas à réaliser que tu es presque adulte."

# game/events/ann.rpy:1086
translate french ann_bath_about_intime_0_b912cfb2:

    # Max_09 "Пора бы уже осознать, мам! И уроки ваши, какие-то односторонние. Как-то неправильно."
    Max_09 "il est temps de réaliser, maman! Et vos leçons, certaines unilatérales. Comme quelque chose de mal."

# game/events/ann.rpy:1087
translate french ann_bath_about_intime_0_24e945aa:

    # Ann_13 "Ну... Что-то я даже не знаю..."
    Ann_13 "Eh bien... Quelque chose que je ne sais même pas..."

# game/events/ann.rpy:1093
translate french ann_bath_about_intime_0_acd6b1b2:

    # Max_07 "Да ладно, я же уже такое видел с вами на тех уроках..."
    Max_07 "Allez, j'ai déjà vu ça avec vous dans ces cours..."

# game/events/ann.rpy:1094
translate french ann_bath_about_intime_0_fdfa6acb:

    # Ann_12 "Ой, Макс, не напоминай. Это всё было исключительно для твоего... сексуального образования..."
    Ann_12 "Oh, Max, ne me le rappelle pas. C'était pour le tien... l'éducation sexuelle..."

# game/events/ann.rpy:1095
translate french ann_bath_about_intime_0_e2b46bea:

    # Max_08 "Так и это тоже для образования! Потому что я хочу не только получать ласку от девочек, но и давать её им. А это, как я понимаю, куда сложнее."
    Max_08 "C'est aussi pour l'éducation! Parce que je veux non seulement recevoir l'affection des filles, mais aussi leur donner. Et cela, si je comprends bien, est beaucoup plus difficile."

# game/events/ann.rpy:1096
translate french ann_bath_about_intime_0_1f40fc0d:

    # Ann_13 "Ох, с учётом того, чему мы с Эриком уже тебя тогда научили, твоя просьба очень логична, сынок. Так что я поговорю об этом с Эриком..."
    Ann_13 "Oh, compte tenu de ce qu'Eric et moi t'avons appris à l'époque, ta demande est très logique, fiston. Donc je vais en parler à Eric..."

# game/events/ann.rpy:1097
translate french ann_bath_about_intime_0_ee64e187:

    # Max_10 "Нет, мам, я хочу только с тобой этому учиться!"
    Max_10 "non, maman, je veux juste apprendre ça avec toi!"

# game/events/ann.rpy:1098
translate french ann_bath_about_intime_0_bd74b95d:

    # Ann_17 "То есть, только со мной?! А Эрик тебе чем не угодил?"
    Ann_17 "Donc, juste avec moi?! Qu'est-ce qui ne te plaît pas, Eric?"

# game/events/ann.rpy:1099
translate french ann_bath_about_intime_0_0314b6cd:

    # Max_09 "Мне кажется, он это... направление не особо любит. Мне же лучше, чтобы такому учила именно женщина, ведь вам виднее."
    Max_09 "je crois que c'est lui... la direction n'aime pas particulièrement. Je préfère que cette femme enseigne, parce que vous êtes plus visible."

# game/events/ann.rpy:1100
translate french ann_bath_about_intime_0_ba4d6bb6:

    # Ann_12 "Виднее, конечно. Но без Эрика я даже не знаю... Не уверена... Ничего, если я подумаю об этом в более... комфортной обстановке?"
    Ann_12 "plus Visible, bien sûr. Mais sans Eric, je ne sais même pas... Je ne suis pas sûre... Rien si j'y pense en plus... un environnement confortable?"

# game/events/ann.rpy:1101
translate french ann_bath_about_intime_0_3a30664f:

    # Max_01 "Да, мам! Я не буду мешать, расслабляйся." nointeract
    Max_01 "oui, maman! Je ne vais pas vous déranger, détendez-vous." nointeract

    old "{i}Отвернуться{/i}"
    new "{i}Se retourner{/i}"

    # game/events/ann.rpy:226
    old "Я не подглядывал. Это случайность!"
    new "Je ne regardais pas. C'est un accident !"

    # game/events/ann.rpy:226
    old "Мам, извини..."
    new "Maman, je suis désolé..."

    # game/events/ann.rpy:226
    old "Попка у тебя - что надо!"
    new "Tu as un super cul !"

    # game/events/ann.rpy:288
    old "Это я, Макс. Можно войти? У меня для тебя кое-что есть."
    new "C'est moi, Max. Je peux entrer ? J'ai quelque chose pour toi."

    # game/events/ann.rpy:360
    old "А посмотреть нельзя?"
    new "Vous ne pouvez pas regarder?"

    # game/events/ann.rpy:360
    old "Да легко! Не буду мешать. Выглядишь, кстати, превосходно!"
    new "C'est facile ! Je vous laisse faire. Tu es superbe, d'ailleurs !"

    # game/events/ann.rpy:387
    old "А я разве чем-то помешаю?"
    new "Est-ce que je peux faire quelque chose?"

    # game/events/ann.rpy:387
    old "А я не помешаю. Начинай..."
    new "Et je ne te dérange pas. Vas-y..."

    # game/events/ann.rpy:387
    old "Да легко! Не буду мешать..."
    new "Oui facile! Je ne vous dérangerai pas..."

    # game/events/ann.rpy:387
    old "Подожди! Мне кажется или у тебя под халатом что-то шевелится..."
    new "Attends ! C'est moi ou il y a quelque chose qui bouge sous ta robe de chambre..."

    # game/events/ann.rpy:387
    old "А можно остаться и посмотреть?"
    new "Je peux rester et regarder ?"

    # game/events/ann.rpy:387
    old "Да легко! А повернуться можешь, пока я ухожу?"
    new "C'est facile ! Tu peux te retourner pendant que je pars ?"

    # game/events/ann.rpy:479
    old "У меня для тебя кое-что есть."
    new "J’ai quelque chose pour toi."

    # game/events/ann.rpy:479
    old "У тебя самые зачётные сиськи, которые я видел!"
    new "Tu as les plus beaux seins que j'ai vus!"

    # game/events/ann.rpy:479
    old "У тебя самый потрясный зад на свете, мам!"
    new "Tu as le plus beau cul du monde, maman!"

    # game/events/ann.rpy:479
    old "Я просто хотел посмотреть..."
    new "Je voulais juste voir..."

    # game/events/ann.rpy:479
    old "Ой, извини. Я забыл... Хорошо выглядишь, мам!"
    new "Oh, désolé. J'ai oublié... Tu as l'air bien, maman!"

    # game/events/ann.rpy:509
    old "Да не прикрывай такую красоту, все свои же!"
    new "Ne couvre pas une telle beauté, tout le vôtre!"

    # game/events/ann.rpy:509
    old "Сразу видно, что попка у тебя тренированная!"
    new "Il est immédiatement évident que votre cul est entraîné!"

    # game/events/ann.rpy:509
    old "Извини, всё позабыл, когда тебя увидел. Ты прекрасна!"
    new "Désolé, j'ai oublié quand je t'ai vu. Tu es belle!"

    # game/events/ann.rpy:606
    old "Ну мам! Этого было так мало, давай ещё..."
    new "Allez, maman ! Ce n'était pas assez, ayons-en plus..."

    # game/events/ann.rpy:606
    old "Конечно, мам! Хорошего тебе дня..."
    new "Bien sûr, maman ! Passez une bonne journée..."

    # game/events/ann.rpy:688
    old "Мам, всё нормально?"
    new "Maman, ça va?"

    # game/events/ann.rpy:688
    old "Видами любуешься, мам?"
    new "Admires-tu la vue, maman?"

    # game/events/ann.rpy:708
    old "Мам?"
    new "Maman?"

    # game/events/ann.rpy:793
    old "{i}приобнять маму за попку{/i}"
    new "{i}embrasser maman sur le cul{/i}"

    # game/events/ann.rpy:843
    old "А что, я мешаю?"
    new "Est-ce que je me mets en travers du chemin?"

    # game/events/ann.rpy:843
    old "С тобой хотел побыть. Или я мешаю?"
    new "Je voulais être avec toi. Ou est-ce que je dérange?"

    # game/events/ann.rpy:853
    old "{i}приобнять маму{/i}"
    new "{i}embrasser maman{/i}"

    # game/events/ann.rpy:862
    old "{i}продолжить обнимать{/i}"
    new "{i}continuer à étreindre{/i}"

    # game/events/ann.rpy:1058
    old "{i}войти{/i}"
    new "{i}se connecter{/i}"

