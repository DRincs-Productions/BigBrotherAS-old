# TODO: Translation updated at 2022-07-09 12:49

# game/script.rpy:58
translate french intro_8dea4dcf:

    # Max_00 "Меня зовут Макс. Я обычный парень из обычной семьи. Ну как обычной... нормальной. Хотя, кого я обманываю. Нормального в моей семье мало. Но обо всём по порядку." nointeract
    Max_00 "Je m’appelle Max. Je suis un gars ordinaire d’une famille normale. Eh bien...banal quoi. Bien que je suis en train de plaisanter. Normal dans ma famille n'ai pas vraiment une réalité. Mais d’abord, les choses principales." nointeract

# game/script.rpy:60
translate french intro_0f762891:

    # Max_01 "Недавно меня выперли из школы. Официально - за низкую успеваемость. Хотя оценки были так себе, но настоящая причина в математичке. Говорят, я приставал... Да она сама глазки строила! Директору, её мужу, это почему-то не понравилось..."
    Max_01 "j’ai récemment été expulsé de l’école. Officiellement - pour de faibles résultats scolaires. Bien que les notes n'étaient pas tellement basse, le problème était en mathématiques. Ils disent que j’ai molesté la prof de mathématiques... Elle a tout inventée! La recteur, son mari, pour une raison qui m'échappe, n’a pas aimé..."

# game/script.rpy:61
translate french intro_4e148ae4:

    # Max_14 "В общем, сейчас я с неоконченным средним образованием. Без работы и конкретных планов на будущее... Но всё должно измениться, обязательно!"
    Max_14 "Bref, je suis maintenant avec une éducation secondaire incomplète. Sans travail et sans projets concrets pour l'avenir... Mais les choses doivent changer, forcément !"

# game/script.rpy:63
translate french intro_1c9ab78c:

    # Max_04 "Это Анна, моя мама. Сама воспитывает нас с двумя сёстрами уже несколько лет. Работает в офисе какой-то компании. Хотя, зарплата у неё вполне приличная, но почти всё уходит на оплату съёмного жилья, еду и одежду."
    Max_04 "Voici Anna, ma mère. C'est elle qui nous a élevés, mes deux sœurs et moi, depuis plusieurs années maintenant. Elle travaille dans le bureau d'une entreprise. Bien que son salaire soit tout à fait décent, elle en consacre la majeure partie de son revenu au loyer, à la nourriture et aux vêtements."

# game/script.rpy:65
translate french intro_cb16881c:

    # Max_00 "Ну а это наша квартира. Тот ещё тараканник! Мама пытается заработать на первый взнос, чтобы взять ипотеку, но пока что-то не получается. Все мы мечтаем отсюда съехать, но куда?"
    Max_00 "C'est notre appartement. Quel capharnaüm! Ma mère essaie de mettre de côté pour pouvoir contracter un prêt hypothécaire, mais jusqu'à présent, cela n'a pas fonctionné. Nous rêvons tous de déménager, mais où ?"

# game/script.rpy:67
translate french intro_4547bbbc:

    # Max_08 "Алиса, моя старшая сестра. Недавно закончила школу и, так же, как и я, ищет свой путь. Целыми днями сидит в ноутбуке и занимается каким-то своим блогом. Как это часто бывает с братьями и старшими сёстрами, мы не очень ладим..."
    Max_08 "Alice, ma grande sœur. Elle vient de terminer ses études et, comme moi, elle cherche sa voie. Toute la journée, elle est assise devant son ordinateur portable, travaillant sur son propre blog. Comme c'est souvent le cas entre frères et sœurs aînés, nous ne nous entendons pas très bien..."

# game/script.rpy:69
translate french intro_75876970:

    # Max_00 "Лиза, младшая сестрёнка, ещё учится в школе. Не знаю как учится, но если не выгнали как меня, значит всё в порядке. С Лизой мы общаемся на одной волне, хотя изредка ссоримся. Но если что-то случается, защиты ищет именно у меня."
    Max_00 "Lisa, la plus jeune sœur, est encore à l'école. Je ne sais pas pour les études, mais si elle ne se fais pas virer comme moi, c'est bien. Lisa et moi sommes sur la même longueur d'onde, même si nous nous disputons de temps en temps. Mais si quelque chose arrive, elle cherche à se protéger."

# game/script.rpy:71
translate french intro_bebe466e:

    # Max_14 "Отца я плохо помню. Он ушёл, когда мне было лет пять. Мама не хочет рассказывать, что тогда случилось... Ходили слухи, что дела у него пошли в гору. С тех пор мы о нём ничего не слышали. До прошлого месяца, когда пришла та девушка..."
    Max_14 "Je ne me souviens pas très bien de mon père. Il est parti quand j'avais environ cinq ans. Maman ne veut pas me dire ce qui s'est passé alors... La rumeur dit qu'il se porte bien. Nous n'avons plus entendu parler de lui depuis. Jusqu'au mois dernier, quand cette fille est arrivée..."

# game/script.rpy:73
translate french intro_fdc8ae44:

    # Maya_01 "Здравствуйте, меня зовут Майя, я адвокат вашего бывшего мужа. У вас найдётся для меня минутка?"
    Maya_01 "Bonjour, je m'appelle Maya et je suis l'avocate de votre ex-mari. Vous avez une minute pour moi ?"

# game/script.rpy:74
translate french intro_165806c1:

    # Ann_00 "Здравствуйте, да, конечно. Меня зовут Анна, но вы и так это знаете похоже. Так в чём дело? С ним что-то случилось?"
    Ann_00 "Bonjour, oui, bien sûr. Je m’appelle Anna, mais vous le savez déjà. Alors, quel est le problème? Est-ce qu’il lui est arrivé quelque chose ?"

# game/script.rpy:75
translate french intro_32cb457a:

    # Maya_03 "Не переживайте, он жив-здоров, но сам приехать не смог. Сказал, что вы будете не очень рады, хотя новости у меня для вас отличные!"
    Maya_03 "Ne vous inquiétez pas, il est bien vivant, mais il n'a pas pu venir. Il a dit que vous ne seriez pas très heureuse, mais j'ai de bonnes nouvelles pour vous !"

# game/script.rpy:76
translate french intro_9858c279:

    # Ann_01 "Любопытно. И что же это за новости спустя столько лет?"
    Ann_01 "Curieux. Et quelle est cette nouvelle après toutes ces années ?"

# game/script.rpy:77
translate french intro_a97592f1:

    # Maya_02 "Я принесла документы на недвижимость, которую он решил передать вам. Если вы их подпишите, то станете полноправным владельцем."
    Maya_02 "J'ai apporté les actes de propriété, qu'il a décidé de vous céder. Si vous les signez, vous en deviendrez le propriétaire légitime de la propriété."

# game/script.rpy:78
translate french intro_8b0c7301:

    # Ann_17 "Что-то я не поняла. Это совсем на него не похоже. Какая недвижимость? О чём речь?"
    Ann_17 "Je ne comprends pas quelque chose. Ça ne lui ressemble pas du tout. Quel genre de propriété ? De quoi s'agit-il ?"

# game/script.rpy:79
translate french intro_ce5a5dae:

    # Maya_01 "Это двухэтажный дом на юге страны, который построила его компания. А насчёт того, что это на него не похоже... В последние пару лет он очень изменился. Не уверена, что эти изменения пошли на пользу бизнесу."
    Maya_01 "Il s'agit d'une maison à deux étages située dans le sud du pays et construite par son entreprise. Et quant au fait que ça ne lui ressemble pas... Il a beaucoup changé au cours des deux dernières années. Je ne suis pas sûr que ces changements aient profité à ses entreprises."

# game/script.rpy:80
translate french intro_43a9e56e:

    # Ann_01 "Что-то вы меня озадачили. В чём подвох? Я что-то должна? Столько лет никаких вестей, ничего, и тут... дом. И что там с его бизнесом?"
    Ann_01 "Quelque chose me dit de me méfier. Quel est le piège ? Est-ce que je vous dois quelque chose ? Pas de mot pendant des années, rien, et puis... une maison. Et qu'en est-il de ses affaires ?"

# game/script.rpy:81
translate french intro_60f50736:

    # Maya_02 "В данный момент он продал большую часть недвижимости, чтобы погасить налоговую задолженность, но этот дом решил передать вам. Я не знаю причин, да они меня и не касаются."
    Maya_02 "Actuellement, il a vendu la plupart de ses biens pour payer ses dettes fiscales, mais il a décidé de vous céder cette maison. Je ne connais pas les raisons, et elles ne me concernent pas."

# game/script.rpy:82
translate french intro_ea2ebd5a:

    # Ann_00 "Вы знаете, Майя, я не могу принять такой... подарок. Во-первых, я не в курсе его махинаций или что он там придумал, а во-вторых, мы с ним уже столько лет не общались, и мне не нужно от него ничего!"
    Ann_00 "Vous savez, Maya, je ne peux pas accepter ce... cadeau. Premièrement, je ne suis pas au courant de ses machinations ou de ce qu'il prépare, et deuxièmement, nous ne nous sommes pas parlé depuis tant d'années et je n'ai besoin de rien de sa part !"

# game/script.rpy:83
translate french intro_734c8765:

    # Maya_01 "Да, он предупреждал меня о такой вашей реакции... С юридической точки зрения дом абсолютно чист, и вы ничем не рискуете. Кроме того, ваши дети растут и им нужно место, своё личное пространство..."
    Maya_01 "Oui, il m'a prévenu de cette réaction de votre part... Légalement, la maison est complètement en ordre et vous ne risquez rien. De plus, vos enfants grandissent et ils ont besoin de leur propre espace, de leur propre intimité..."

# game/script.rpy:84
translate french intro_8db9c9f1:

    # Ann_14 "С этим не поспоришь. У меня есть время подумать обо всём?"
    Ann_14 "Vous ne pouvez pas argumenter avec ça. Ai-je le temps de réfléchir?"

# game/script.rpy:86
translate french intro_8e21e5ab:

    # Maya_03 "Конечно, Анна. Вот моя визитка. Позвоните, когда будете готовы принять решение. Также я оставлю копии всех документов. До свидания." nointeract
    Maya_03 "Bien sûr, Anna. Voici ma carte. Appelez lorsque vous êtes prêt à prendre une décision. Je vous laisse également des copies de tous les documents. Au revoir." nointeract

# game/script.rpy:90
translate french intro_eaf717b9:

    # Max_04 "К счастью, мама приняла верное решение. Когда мы увидели дом, не поверили своим глазам! Вживую он оказался гораздо больше и красивее, чем на фотографиях. Но самым приятным сюрпризом оказался бассейн!"
    Max_04 "Heureusement, ma mère a pris la bonne décision. Quand nous avons vu la maison, nous n'en avons pas cru nos yeux ! Elle était beaucoup plus grande et plus jolie en vrai que sur les photos. Mais la plus agréable surprise a été la piscine !"

# game/script.rpy:92
translate french intro_afbd2795:

    # Max_00 "Алиса сразу заняла одну из спален. Кстати, их оказалось всего две. Несмотря на огромную гостиную с кухней и просторную ванную, в доме всего три отдельные комнаты: две спальни и кабинет."
    Max_00 "Alice a immédiatement pris possession d'une des chambres. D'ailleurs, il n'y en avait que deux. Malgré l'immense salon avec kitchenette et la salle de bains spacieuse, la maison ne compte que trois pièces distinctes : deux chambres et un bureau."

# game/script.rpy:94
translate french intro_5fc10bfc:

    # Max_08 "В кабинете мебели почти не оказалось, и мне пришла в голову отличная идея разместить здесь наши с Лизой кровати из старой квартиры."
    Max_08 "Il n'y avait presque pas de meubles dans le bureau, et j'ai eu l'idée géniale de mettre les lits de Lisa et moi de l'ancien appartement là."

# game/script.rpy:95
translate french intro_c174fdf7:

    # Ann_01 "Что-то мне не нравится эта твоя идея, Макс. Вы уже взрослые, чтобы спать в одной комнате. Давай этот офис выделим тебе, а вторую спальню - Лизе. Ну а я буду спать в гостиной. Там отличные диванчики..."
    Ann_01 "Je n'aime pas ton idée, Max. Vous êtes assez grands pour ne pas dormir dans la même chambre. Attribuons ce bureau à toi et l'autre chambre à Lisa. Eh bien, je vais dormir dans le salon. Il y a de super canapés là-bas..."

# game/script.rpy:96
translate french intro_8132e149:

    # Max_01 "Мам, мы и так все жили как селёдки в бочке, нам не привыкать. А тут такие огромные комнаты. Нам с Лизой места хватит. Правда, сестрёнка?"
    Max_01 "Maman, on a tous vécu comme des sardines en boîte, on a l'habitude. Et ces pièces sont si grandes. Lisa et moi avons assez de place. Pas vrai, sœurette ?"

# game/script.rpy:97
translate french intro_fbedb073:

    # Lisa_02 "Конечно! Мам, мы нормально с Максом уживаемся. А если будет косячить, выгоню его в эту самую гостиную! А ты свою комнату точно заслужила. И не спорь. Мы все так думаем."
    Lisa_02 "Bien sûr ! Maman, Max et moi, on s'entend bien. Et si il m'énerve, je le jetterai dehors dans le salon ! Et tu as certainement mérité ta propre chambre. Et ne discutes pas. Nous le pensons tous."

# game/script.rpy:99
translate french intro_07cd4260:

    # Ann_07 "Ну хорошо. Транспортная компания сообщила, что наши вещи задерживаются. Что-то я уже жалею, что с ними связалась... Так или иначе, через неделю переезжаем!" nointeract
    Ann_07 "Très bien. La compagnie de déménagement m'a informés que nos affaires avaient été retardées. Je regrette déjà de m'être engagée avec eux... Bref, on déménage dans une semaine !" nointeract

# game/script.rpy:103
translate french intro_dbd412b7:

    # Max_00 "Спустя неделю часть наших вещей из старой квартиры доставили. Но только часть. Как я понял, кое-что потерялось в пути. Надеюсь, компенсируют это как-то..."
    Max_00 "Une semaine plus tard, certaines de nos affaires de l'ancien appartement ont été livrées. Mais seulement une partie. Si j'ai bien compris, quelque chose s'est perdu en chemin. J'espère qu'ils se rattrapent d'une manière ou d'une autre..."

# game/script.rpy:105
translate french intro_356cd733:

    # Max_04 "Мама устроилась на новую работу. Лиза пошла в местную школу. Алиса так и сидит в своём ноуте, а я... На этом мой рассказ заканчивается и начинается что-то новое и интересное!" nointeract
    Max_04 "Maman a trouvé un nouveau travail. Lisa est allée à l'école du quartier. Alice est toujours assise sur son ordinateur portable et moi... C'est ici que mon ancienne vie se termine et que quelque chose de nouveau et d'intéressant commence.!" nointeract

# game/script.rpy:118
translate french about_poss_0cd5a085:

    # Max_00 "\"Возможности\" - это особые пути, по которым можно пройти, чтобы достичь какого-то результата. Обычно они скрыты и не очень очевидны."
    Max_00 "'Opportunité' - ce sont des chemins spécifiques qui peuvent être suivis pour atteindre un certain type de résultat. Elle sont généralement cachées et peu visibles."

# game/script.rpy:119
translate french about_poss_0f75481c:

    # Max_03 "Некоторые действия в игре открывают такие \"возможности\". Увидеть их можно на соответствующем экране, который можно открыть через верхнее меню."
    Max_03 "Certaines actions du jeu ouvrent de telles 'opportunités'. Vous pouvez les voir sur l'écran correspondant, qui peut être ouvert via le menu supérieur."

# game/script.rpy:120
translate french about_poss_09cf07b7:

    # Max_00 "Некоторые \"возможности\" временные. Некоторые могут привести к нежелательному результату. Не стоит хвататься за первую попавшуюся и слепо следовать подсказкам!"
    Max_00 "Quelques 'opportunités' sont temporaire. Certaines peuvent conduire à des résultats indésirables. Ne prenez pas la première qui vous tombe dessus, suivez attentivement les instructions !"

# game/script.rpy:122
translate french about_poss_95f72849:

    # Max_04 "Но и бояться их не стоит. Именно благодаря \"возможностям\" можно добиться того, о чём некоторые даже не мечтают!" nointeract
    Max_04 "Mais vous ne devez pas non plus en avoir peur. C'est grâce à des 'opportunités' que vous pouvez réaliser des choses dont certaines personnes ne rêvent même pas !" nointeract

translate french strings:

    # game/script.rpy:57
    old "{i}далее{/i}"
    new "{i}suivant{/i}"

    # game/script.rpy:57
    old "{i}пропустить{/i}"
    new "{i}sauter - skipper{/i}"

    # game/script.rpy:85
    old "{i}пару недель спустя...{/i}"
    new "{i}quelques semaines plus tard... {/i}"

    # game/script.rpy:98
    old "{i}спустя неделю...{/i}"
    new "{i}une semaine plus tard... {/i}"

    # game/script.rpy:104
    old "{i}Начать игру{/i}"
    new "{i}Démarrer le jeu{/i}"

    # game/script.rpy:104
    old "{i}Повторить историю{/i}"
    new "{i}Répéter le prologue{/i}"

    # game/script.rpy:121
    old "{i}всё ясно{/i}"
    new "{i}tout est clair{/i}"

    # game/script.rpy:121
    old "{i}повторить{/i}"
    new "{i}répéter{/i}"

