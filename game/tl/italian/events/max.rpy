# TODO: Translation updated at 2022-07-09 12:49

# game/events/max.rpy:4
translate italian StartDialog_e88dfdc8:

    # Max_10 "{m}Я чувствую себя слишком уставшим для этого. Было бы неплохо сначала вздремнуть и набраться сил...{/m}"
    Max_10 "{m}Mi sento troppo stanco per questo. Sarebbe bello fare un pisolino prima e recuperare le forze...{/m}"

# game/events/max.rpy:32
translate italian Sleep_52abcf74:

    # Max_00 "{m}Пожалуй, пора ложиться спать...{/m}" nointeract
    Max_00 "{m}Forse è ora di andare a letto...{/m}" nointeract

# game/events/max.rpy:35
translate italian Sleep_9878a7fd:

    # Max_00 "{m}В каком часу мне будет лучше проснуться?{/m}" nointeract
    Max_00 "{m}A che ora sarebbe meglio svegliarmi? {/m}" nointeract

# game/events/max.rpy:47
translate italian Sleep_06b9714c:

    # Max_19 "{m}Как же в этом доме хорошо...{/m}"
    Max_19 "{m}Quanto è bella questa casa...{/m}"

# game/events/max.rpy:65
translate italian Wearied_d8557c8f:

    # Max_10 "{m}Моя голова уже совсем не соображает, нужно ложиться спать...{/m}" nointeract
    Max_10 "{m}La mia testa non pensa affatto, ho bisogno di andare a letto... {/m}" nointeract

# game/events/max.rpy:67
translate italian Wearied_06b9714c:

    # Max_19 "{m}Как же в этом доме хорошо...{/m}"
    Max_19 "{m}Quanto è bella questa casa...{/m}"

# game/events/max.rpy:85
translate italian LittleEnergy_d97dcb62:

    # Max_10 "{m}Я слишком вымотался, нужно хоть немного вздремнуть...{/m}" nointeract
    Max_10 "{m}sono troppo esausto, ho bisogno di fare un pisolino... {/m}" nointeract

# game/events/max.rpy:96
translate italian LittleEnergy_06b9714c:

    # Max_19 "{m}Как же в этом доме хорошо...{/m}"
    Max_19 "{m}Quanto è bella questa casa...{/m}"

# game/events/max.rpy:116
translate italian Nap_9f963ea2:

    # Max_00 "{m}Я сейчас не очень хочу спать, но немного вздремнуть лишним не будет...{/m}" nointeract
    Max_00 "{m}Non ho molto sonno al momento, ma un sonnellino non mi farebbe male...{/m}" nointeract

# game/events/max.rpy:118
translate italian Nap_5e123eb6:

    # Max_00 "{m}Ох и вымотался же я сегодня, надо немного вздремнуть...{/m}" nointeract
    Max_00 "{m}Oh, sono esausto oggi, ho bisogno di fare un pisolino...{/m}" nointeract

# game/events/max.rpy:135
translate italian Nap_06b9714c:

    # Max_19 "{m}Как же в этом доме хорошо...{/m}"
    Max_19 "{m}Quanto è bella questa casa...{/m}"

# game/events/max.rpy:144
translate italian Alarm_9878a7fd:

    # Max_00 "{m}В каком часу мне будет лучше проснуться?{/m}" nointeract
    Max_00 "{m}A che ora sarebbe meglio svegliarmi? {/m}" nointeract

# game/events/max.rpy:157
translate italian Alarm_06b9714c:

    # Max_19 "{m}Как же в этом доме хорошо...{/m}"
    Max_19 "{m}Quanto è bella questa casa...{/m}"

# game/events/max.rpy:172
translate italian Shower_691d4581:

    # Max_19 "{m}Всё-таки чистым быть намного лучше. Хотя не всегда хочется...{/m}" nointeract
    Max_19 "{m}Però è molto meglio essere puliti. Anche se non sempre si vuole...{/m}" nointeract

# game/events/max.rpy:178
translate italian Shower_8b383cf3:

    # Max_03 "{m}Ага! Я только сейчас обратил внимание на то, что здесь есть ещё заднее окно! И, как мне кажется, через него на ванну откроется просто шикарный вид... Конечно же дело не в самой ванне, а в том, кто будет её принимать.{/m}"
    Max_03 "{m}Ho notato solo ora che c'è anche una finestra posteriore! E penso che la vista della vasca da bagno sarebbe spettacolare... Certo, non è la vasca da bagno, ma chi la prende.{/m}"

# game/events/max.rpy:179
translate italian Shower_d716938a:

    # Max_09 "{m}Только вот расположено оно высоковато... Нужно достать что-то, с чего будет удобно подглядывать и что не вызовет, в случае чего, подозрений...{/m}"
    Max_09 "{m}L'unica cosa è che è un po' in alto... Devi prendere qualcosa che sia facile da sbirciare e che non desti sospetti se succede qualcosa...{/m}"

# game/events/max.rpy:180
translate italian Shower_38c5acb7:

    # Max_01 "{m}Возможно подойдёт лестница, а ещё лучше стремянка. О да, пожалуй, это будет то, что нужно!{/m}"
    Max_01 "{m}Forse una scala o, meglio ancora, una scaletta andrebbe bene. Oh sì, forse sarebbe la cosa giusta!{/m}"

# game/events/max.rpy:195
translate italian Bath_691d4581:

    # Max_19 "{m}Всё-таки чистым быть намного лучше. Хотя не всегда хочется...{/m}" nointeract
    Max_19 "{m}Però è molto meglio essere puliti. Anche se non sempre si vuole...{/m}" nointeract

# game/events/max.rpy:201
translate italian Bath_8b383cf3:

    # Max_03 "{m}Ага! Я только сейчас обратил внимание на то, что здесь есть ещё заднее окно! И, как мне кажется, через него на ванну откроется просто шикарный вид... Конечно же дело не в самой ванне, а в том, кто будет её принимать.{/m}"
    Max_03 "{m}Ho notato solo ora che c'è anche una finestra posteriore! E penso che la vista della vasca da bagno sarebbe spettacolare... Certo, non è la vasca da bagno, ma chi la prende.{/m}"

# game/events/max.rpy:202
translate italian Bath_d716938a:

    # Max_09 "{m}Только вот расположено оно высоковато... Нужно достать что-то, с чего будет удобно подглядывать и что не вызовет, в случае чего, подозрений...{/m}"
    Max_09 "{m}L'unica cosa è che è un po' in alto... Devi prendere qualcosa che sia facile da sbirciare e che non desti sospetti se succede qualcosa...{/m}"

# game/events/max.rpy:203
translate italian Bath_38c5acb7:

    # Max_01 "{m}Возможно подойдёт лестница, а ещё лучше стремянка. О да, пожалуй, это будет то, что нужно!{/m}"
    Max_01 "{m}Forse una scala o, meglio ancora, una scaletta andrebbe bene. Oh sì, forse sarebbe la cosa giusta!{/m}"

# game/events/max.rpy:216
translate italian Box_97aab2e1:

    # Max_08 "{m}Так, мама попросила разобрать коробки. Сейчас глянем, что тут у нас...{/m}"
    Max_08 "{m}Bene, mia madre mi ha chiesto di smontare le scatole. Vediamo cosa abbiamo qui...{/m}"

# game/events/max.rpy:218
translate italian Box_53491d2a:

    # Max_09 "{m}Жаль, но все коробки пустые... Но что это такое? Какая-то камера?{/m}"
    Max_09 "{m}È un peccato, ma tutte le scatole sono vuote... Ma cos'è? Una specie di macchina fotografica?{/m}"

# game/events/max.rpy:220
translate italian Box_333c951b:

    # Max_01 "{m}Тут внутри какая-то инструкция, описание... Да это скрытая камера! Любопытно, зачем она понадобилась отцу?{/m}"
    Max_01 "{m}C'è una specie di manuale all'interno, una descrizione... È una telecamera nascosta! Mi chiedo perché mio padre lo voglia.{/m}"

# game/events/max.rpy:224
translate italian Box_911e2e27:

    # Max_10 "{m}Может быть, она установлена где-то в доме и за нами кто-то наблюдает?! Нужно будет осмотреть дом...{/m}" nointeract
    Max_10 "{m}Forse è installato da qualche parte nella casa e qualcuno ci sta guardando! Dovremo dare un'occhiata alla casa...{/m}" nointeract

# game/events/max.rpy:250
translate italian Notebook_299609c3:

    # Max_00 "{m}Итак, чем интересным я займусь?{/m}"
    Max_00 "{m}Allora, quali sono le cose interessanti che farò?{/m}"

# game/events/max.rpy:311
translate italian nothing_search_841c64c4:

    # Max_00 "{m}Сейчас мне нечего искать...{/m}"
    Max_00 "{m}Non c'è niente da cercare ora...{/m}"

# game/events/max.rpy:319
translate italian buyfood_14a9f290:

    # Max_04 "{m}Так... Посмотрим список продуктов... Ага. Сейчас всё закажем...{/m}"
    Max_04 "{m}Quindi... Vediamo la lista della spesa... Sì... Ordiniamo tutto...{/m}"

# game/events/max.rpy:321
translate italian buyfood_da7a3eda:

    # Max_04 "{m}Готово. Да это же самая лёгкая задача!{/m}"
    Max_04 "{m}Fatto. È il compito più facile di tutti!{/m}"

# game/events/max.rpy:324
translate italian buyfood_31efbd83:

    # Max_01 "{m}Готово. То, что я делаю это без маминой финансовой помощи точно пойдёт мне только в плюс.{/m}"
    Max_01 "{m}Fatto. Il fatto che sto facendo questo 157 con l'aiuto finanziario di mia madre sarà sicuramente solo un vantaggio per me.{/m}"

# game/events/max.rpy:358
translate italian create_site_c3e958a9:

    # Max_00 "{m}Итак, пришло время заняться своим сайтом. Для начала нужно купить домен, хостинг, шаблон дизайна и оплатить услуги стримингового сервиса. На всё в сумме нужно $100{/m}" nointeract
    Max_00 "{m}Quindi, è il momento di darsi da fare con il tuo sito web. Prima devi comprare un dominio, un hosting, un modello di design e pagare un servizio di streaming. Tutto sommato, avete bisogno di $100{/m}" nointeract

# game/events/max.rpy:365
translate italian create_site_4c292724:

    # Max_04 "{m}Отлично! Теперь у меня есть свой сайт и домен! Осталось только соединить поток данных от камеры со стриминговым сервисом...{/m}" nointeract
    Max_04 "{m}Grande! Ora ho il mio sito web e il mio dominio! Non resta che collegare il flusso di dati dalla telecamera al servizio di streaming...{/m}" nointeract

# game/events/max.rpy:369
translate italian create_site_2b30be60:

    # Max_04 "{m}Да! Всё работает! Теперь люди смогут заходить на мой сайт и смотреть шоу. Конечно, если они каким-то образом узнают про мой сайт... Ладно, подумаю ещё что можно сделать...{/m}"
    Max_04 "{m}Sì, funziona! Ora la gente potrà andare sul mio sito web e guardare lo spettacolo. Naturalmente, se in qualche modo scoprono il mio sito web... Ok, vedrò cos'altro posso fare...{/m}"

# game/events/max.rpy:404
translate italian about_cam_42f3e467:

    # Max_09 "{m}Так, любопытно... Эти камеры можно настроить так, чтобы они транслировали изображение в интернет!{/m}"
    Max_09 "{m}Quindi, curioso... Queste telecamere possono essere impostate per trasmettere le immagini su Internet!{/m}"

# game/events/max.rpy:405
translate italian about_cam_c6823e2f:

    # Max_07 "{m}Но что ещё интереснее, некоторые люди готовы платить за доступ к таким камерам...{/m}"
    Max_07 "{m}Ma ciò che è ancora più interessante è che alcune persone sono disposte a pagare per avere accesso a tali telecamere...{/m}"

# game/events/max.rpy:406
translate italian about_cam_8ca532b0:

    # Max_09 "{m}Может быть, мне сделать свой сайт и пусть люди мне платят за просмотр видео? Но я не умею ничего толком...{/m}"
    Max_09 "{m}Forse dovrei fare il mio sito web e farmi pagare dalla gente per guardare i video? Ma non so fare nulla in modo corretto...{/m}"

# game/events/max.rpy:419
translate italian about_blog_ea375606:

    # Max_00 "{m}Итак, попробуем что-то найти о блогах. С чего начать?{/m}" nointeract
    Max_00 "{m}Quindi, cerchiamo di trovare qualcosa sul blogging. Da dove cominciare?{/m}" nointeract

# game/events/max.rpy:422
translate italian about_blog_c3072649:

    # Max_10 "{m}Ох... Сколько цифр. Неужели, во всём этом можно разобраться?{/m}" nointeract
    Max_10 "{m}Oh... Così tanti numeri. È davvero possibile dare un senso a tutto questo?{/m}" nointeract

# game/events/max.rpy:431
translate italian about_blog_eaab3fbf:

    # Max_04 "{m}Прикольно... Ага. Котики. Не, устарели. Киски. Ну, смотря какие...{/m}" nointeract
    Max_04 "{m}Questo è divertente... Sì... Gatti. No, obsoleto. Fighette. Beh, dipende...{/m}" nointeract

# game/events/max.rpy:440
translate italian about_blog_f7287104:

    # Max_14 "{m}Ух. Сколько гадости в комментах... Ладно, попробуем найти в этом крупицу смысла...{/m}" nointeract
    Max_14 "{m}Ugh. Così tanta cattiveria nei commenti... Bene, cerchiamo di dare un senso a tutto questo...{/m}" nointeract

# game/events/max.rpy:449
translate italian about_blog_2d9761c1:

    # Max_09 "{m}[_text!t]{/m}" nointeract
    Max_09 "{m}[_text!t]{/m}" nointeract

# game/events/max.rpy:451
translate italian about_blog_116ee643:

    # Max_01 "{m}Это что же получается. Неужели, всё так просто? Главное - сиськи! Не важно о чём блог, важно что на экране. И если там сиськи, всё в порядке с популярностью! Но я и так об этом догадывался...{/m}"
    Max_01 "{m}E questo cosa fa? È davvero così semplice? È tutta una questione di tette! Non importa di cosa parla il blog, è quello che c'è sullo schermo. E se si tratta di tette, va bene essere popolari! Ma io lo sapevo già...{/m}"

# game/events/max.rpy:452
translate italian about_blog_b55fbee5:

    # Max_00 "{m}И зачем я только что-то изучал...{/m}"
    Max_00 "{m}Perché mai ho studiato qualcosa...{/m}"

# game/events/max.rpy:465
translate italian about_secretbook_d428da7a:

    # Max_00 "{m}Так... Сейчас погуглим. Как там она называлась? \"Sugar Daddies\"?... Любовный роман? И что в нём такого может быть?{/m}" nointeract
    Max_00 "{m}Quindi... Ora cerchiamo su Google. Qual era il suo nome lì? \"Sugar Daddy\"?... Una storia d'amore? E cosa potrebbe esserci dentro?{/m}" nointeract

# game/events/max.rpy:467
translate italian about_secretbook_331cf11d:

    # Max_06 "{m}Ого! Да это не простой любовный роман... Это же эротика. Да ещё какая! Теперь понятно, почему Алиса не хотела рассказывать, что читает...{/m}"
    Max_06 "{m}Wow! Questa non è una semplice storia d'amore... Questo è erotismo. Ed è così! Capisco perché Alice non voleva dirti cosa stava leggendo...{/m}"

# game/events/max.rpy:481
translate italian about_spider_c9502343:

    # Max_00 "{m}Так... Пауки. Ох, сколько видов... Какие же тут водятся...{/m}" nointeract
    Max_00 "{m}Quindi... Ragni. Oh, così tante specie... Quali specie ci sono...{/m}" nointeract

# game/events/max.rpy:483
translate italian about_spider_1c3432cf:

    # Max_00 "{m}Ага, отлично. Выбираем наш регион, сортируем по видам пауков...{/m}"
    Max_00 "{m}Sì, fantastico. Scegli la nostra regione, ordina per specie di ragni...{/m}"

# game/events/max.rpy:485
translate italian about_spider_d718f75f:

    # Max_00 "{m}Так, пауки. Смотрим какие водятся в этом климате...{/m}"
    Max_00 "{m}Ok, ragni. Vediamo quali tipi si trovano in questo clima...{/m}"

# game/events/max.rpy:486
translate italian about_spider_06795649:

    # Max_00 "{m}Так, и что у нас получается?{/m}"
    Max_00 "{m}Quindi, cosa otteniamo?{/m}"

# game/events/max.rpy:488
translate italian about_spider_825e59bc:

    # Max_01 "{m}Ну вот, подходит. Самый популярный паук в наших краях. Ага! Теперь узнаем как его поймать...{/m}" nointeract
    Max_01 "{m}Bene, ci siamo. Il ragno più popolare da queste parti. Aha! Ora scopriamo come prenderlo...{/m}" nointeract

# game/events/max.rpy:491
translate italian about_spider_43ff8442:

    # Max_09 "{m}Так, питается комарами и мошками. Как это обычно... нет, это ничего не даёт...{/m}" nointeract
    Max_09 "{m}Quindi, nutrendosi di zanzare e moscerini. Come al solito... no, non fa niente...{/m}" nointeract

# game/events/max.rpy:496
translate italian about_spider_313f4e4c:

    # Max_04 "{m}Вот, отлично! Ночью отлично маскируются, значит, не подходит, а вот как только солнце начинает прогревать землю, выползают из травы проверить добычу. А это у нас часов 10-11?{/m}"
    Max_04 "{m}Ecco, così va bene! Si mimetizzano perfettamente di notte, quindi non si avvicinano, ma appena il sole comincia a scaldare il terreno, strisciano fuori dall'erba per controllare la loro preda. E sono le 10 o le 11?{/m}"

# game/events/max.rpy:497
translate italian about_spider_1b3b8906:

    # Max_01 "{m}Будем искать!{/m}"
    Max_01 "{m}Continueremo a cercare!{/m}"

# game/events/max.rpy:513
translate italian SearchCam_29b2ce2f:

    # Max_04 "{m}Ого! Вот же она! Кто-то её так хорошо запрятал в стену, что найти камеру можно только точно зная, что ищешь...{/m}"
    Max_04 "{m}Eccolo! Qualcuno l'ha nascosto così bene nel muro che puoi trovare la telecamera solo se sai esattamente cosa stai cercando...{/m}"

# game/events/max.rpy:514
translate italian SearchCam_6eab0c74:

    # Max_09 "{m}Так... Но она ни к чему не подключена сейчас. Видимо, отец так следил за ходом строительства и ремонта, а сейчас уже некому следить и не за чем...{/m}"
    Max_09 "{m}Quindi... Ma ora non è collegato a niente. A quanto pare, mio padre teneva d'occhio la costruzione e la ristrutturazione, ma ora non c'è nessuno da tenere d'occhio e niente da fare...{/m}"

# game/events/max.rpy:515
translate italian SearchCam_7c5a1bd7:

    # Max_04 "{m}Но если её подключить, то можно подглядывать и за кое-чем другим. Вот только нужно во всём как следует разобраться!{/m}"
    Max_04 "{m}Ma se lo colleghi, puoi sbirciare anche qualcos'altro. Si tratta solo di capire tutto!{/m}"

# game/events/max.rpy:523
translate italian SearchCam_73e63b9a:

    # Max_14 "{m}Кажется, здесь нет никаких камер... Нужно поискать в самом доме!{/m}"
    Max_14 "{m}Sembra che non ci siano telecamere qui... Dovremmo guardare nella casa stessa!{/m}"

# game/events/max.rpy:525
translate italian SearchCam_74aa4ac8:

    # Max_14 "{m}Кажется, здесь нет никаких камер... Может быть, стоит поискать в другой комнате?{/m}"
    Max_14 "{m}Sembra che non ci siano telecamere qui dentro... Forse dovremmo guardare nell'altra stanza?{/m}"

# game/events/max.rpy:537
translate italian ClearPool_f95b5748:

    # Max_11 "{m}Эх... Не лёгкая это работа, но нужно отработать те деньги, что мама уже заплатила...{/m}"
    Max_11 "{m}Eh... Non è un lavoro facile, ma bisogna lavorare con i soldi che la mamma ha già pagato...{/m}"

# game/events/max.rpy:540
translate italian ClearPool_26cb581b:

    # Max_01 "{m}Эх... Работа нудная, но важно, чтобы мои девочки плескались в чистой водичке. И теперь, я слежу за этим сам.{/m}"
    Max_01 "{m}Eh... È un lavoro noioso, ma è importante che le mie bambine sguazzino nell'acqua pulita. E ora, me ne sto assicurando io stesso.{/m}"

# game/events/max.rpy:560
translate italian DishesWashed_62001d7c:

    # Max_00 "{m}Эх... столько посуды. И почему в этом огромном доме нет маленькой посудомоечной машины?{/m}" nointeract
    Max_00 "{m}Eh... così tanti piatti. E perché non c'è una piccola lavastoviglie in questa casa enorme?{/m}" nointeract

# game/events/max.rpy:586
translate italian delivery1_f0fc4226:

    # Max_07 "{m}Звонок в ворота! Похоже, к нам кто-то приехал...{/m}"
    Max_07 "{m}La campana al cancello! Sembra che qualcuno sia arrivato...{/m}"

# game/events/max.rpy:589
translate italian delivery1_80685efe:

    # Sam_00 "Здравствуйте! По этому адресу на сегодня назначена доставка. Распишитесь?"
    Sam_00 "Ciao! La consegna è prevista per questo indirizzo oggi. Segno?"

# game/events/max.rpy:590
translate italian delivery1_509c6af4:

    # Max_00 "Конечно! А что тут?"
    Max_00 "Certo! Cosa sta succedendo qui?"

# game/events/max.rpy:592
translate italian delivery1_0ba1be34:

    # Max_00 "Да, то что нужно. Спасибо!"
    Max_00 "sì, è quello di cui hai bisogno. Grazie!"

# game/events/max.rpy:619
translate italian delivery2_f0fc4226:

    # Max_07 "{m}Звонок в ворота! Похоже, к нам кто-то приехал...{/m}"
    Max_07 "{m}La campana al cancello! Sembra che qualcuno sia arrivato...{/m}"

# game/events/max.rpy:623
translate italian delivery2_60d5c126:

    # Christine_00 "Здравствуйте! По этому адресу на сегодня назначена доставка. Распишитесь?"
    Christine_00 "Ciao! La consegna è prevista per questo indirizzo oggi. Segno?"

# game/events/max.rpy:624
translate italian delivery2_509c6af4:

    # Max_00 "Конечно! А что тут?"
    Max_00 "Certo! Cosa sta succedendo qui?"

# game/events/max.rpy:626
translate italian delivery2_0ba1be34:

    # Max_00 "Да, то что нужно. Спасибо!"
    Max_00 "sì, è quello di cui hai bisogno. Grazie!"

# game/events/max.rpy:636
translate italian delivery2_885c0cfc:

    # Max_02 "{m}Боди у меня! Теперь, нужно подарить его Алисе и больше всего мне может повезти, когда она занимается своим блогом. Она и так в это время в нижнем белье, а с учётом того, что она получит боди раньше времени, то вполне может переодеться и при мне...{/m}"
    Max_02 "{m}ho Bodie! Ora, ho bisogno di darlo ad Alice e soprattutto posso essere fortunato quando è impegnata nel suo blog. È già in mutande in questo momento, e dato il fatto che otterrà un body prima del tempo, potrebbe benissimo cambiare i vestiti con me ... {/m}"

# game/events/max.rpy:638
translate italian delivery2_1f5e97ce:

    # Max_10 "{m}Боди у меня! Вот только поздно... Эрик уже купил Алисе то, что она хотела. Остаётся лишь выставить на ebay, так хотя бы половину стоимости верну.{/m}"
    Max_10 "{m}ho Bodie! È troppo tardi... Eric aveva già comprato ad Alice quello che voleva. Resta solo da mettere su ebay, quindi almeno la metà del costo verrà restituita. {/m}"

# game/events/max.rpy:649
translate italian BookRead_9562ffab:

    # Max_00 "Пришло время почитать что-то..." nointeract
    Max_00 "È tempo di leggere qualcosa..." nointeract

# game/events/max.rpy:658
translate italian BookRead_manual_41be558e:

    # Max_00 "Хм... куча непонятных слов. Кажется, нужно будет заново перечитать первые главы...\n\n{color=[orange]}{i}(Книга изучена на 20%%){/i}{/color}"
    Max_00 "Hmm... un mucchio di parole incomprensibili. Sembra che i primi capitoli dovranno essere riletti...\n\n{color=[orange]}{i}(Il libro è stato letto al 20%%){/i}{/color}"

# game/events/max.rpy:660
translate italian BookRead_manual_0826f3fe:

    # Max_00 "Так, ну с этим я уже разобрался, хорошо... А это что такое? Не ясно. Нужно будет всё осмыслить...\n\n{color=[orange]}{i}(Книга изучена на 40%%){/i}{/color}"
    Max_00 "Quindi, beh, l'ho capito, ok... E che cos'è? Non è chiaro. Dovremo dare un senso a tutto...\n\n{color=[orange]}{i}(Il libro è stato letto al 40%%){/i}{/color}"

# game/events/max.rpy:662
translate italian BookRead_manual_3605e1d8:

    # Max_00 "Ого, вот это здорово! Уже можно делать сайт? А, нет... Ещё не всё понятно... Ну, разберусь в другой раз.\n\n{color=[orange]}{i}(Книга изучена на 60%%){/i}{/color}"
    Max_00 "Wow, è fantastico! Possiamo già fare un sito web? Ah, no... Non lo so ancora... Beh, lo capirò un'altra volta.\n\n{color=[orange]}{i}(Il libro è stato letto al 60%%){/i}{/color}"

# game/events/max.rpy:664
translate italian BookRead_manual_1c08c83d:

    # Max_00 "Так, ну теперь картина вырисовывается. Осталось разобраться только с мелочами... Или это не мелочи?\n\n{color=[orange]}{i}(Книга изучена на 80%%){/i}{/color}"
    Max_00 "Quindi, beh, ora il quadro sta emergendo. Resta da fare solo per affrontare le piccole cose... O non è banale?\n\n{color=[orange]}{i}(Il libro è stato letto al 80%%){/i}{/color}"

# game/events/max.rpy:666
translate italian BookRead_manual_bd8ad388:

    # Max_00 "Всё, вот теперь точно всё понятно! Я уже могу сделать свой сайт и транслировать на него изображение! Но как получать за это деньги?"
    Max_00 "Tutto, ora tutto è chiaro di sicuro! Posso già creare il mio sito web e trasmetterci un'immagine! Ma come essere pagati per questo?"

# game/events/max.rpy:678
translate italian BookRead_sex_ed_e2e2e4d9:

    # Max_01 "Ага. У каждого есть свои особенности, а то я не знал! Вот, строение половых органов девочки-подростка, то что надо... Будем читать и разглядывать.\n\n{color=[orange]}{i}(Книга изучена на 25%%){/i}{/color}"
    Max_01 "sì. Ognuno ha le sue caratteristiche, o io non lo sapevo! Qui, la struttura dei genitali di una ragazza adolescente, ciò di cui hai bisogno... Leggeremo e guarderemo.\n\n{color=[orange]}{i}(Il libro è stato letto al 25%%){/i}{/color}"

# game/events/max.rpy:680
translate italian BookRead_sex_ed_5dc0d1da:

    # Max_03 "Так, это не особо интересно... А вот сексуальное поведение подростков - это как раз про меня! Ещё про мои утренние стояки написали бы, было бы вообще супер...\n\n{color=[orange]}{i}(Книга изучена на 50%%){/i}{/color}"
    Max_03 "Ok, questo non è molto interessante... Ma il comportamento sessuale degli adolescenti riguarda solo me! Se tu potessi scrivere anche delle mie erezioni mattutine, sarebbe fantastico...\n\n{color=[orange]}{i}(Il libro è studiato al 50%%){/i}{/color}"

# game/events/max.rpy:682
translate italian BookRead_sex_ed_cdf53c7a:

    # Max_07 "Ого, здесь даже есть краткий исторический очерк о сексуальном воспитании детей и подростков... Как только голову не дурили за всё это время!\n\n{color=[orange]}{i}(Книга изучена на 75%%){/i}{/color}"
    Max_07 "Wogo, c'è anche un breve saggio storico sull'educazione sessuale per bambini e adolescenti... Non appena la testa non è stata ingannata per tutto questo tempo!\n\n{color=[orange]}{i}(Il libro è stato letto al 75%%){/i}{/color}"

# game/events/max.rpy:687
translate italian BookRead_sex_ed_58528ae0:

    # Max_04 "Вот и последние главы... Всё-таки прикосновения очень важны! Да я и на практике уже это понял... Эх, надо было раньше эту книжку купить! Но лучше поздно, чем никогда. Материал усвоен и теперь можно дарить её Лизе."
    Max_04 "Ecco gli ultimi capitoli... Tuttavia, il tatto è molto importante! Sì, l'ho già capito in pratica... Oh, avrei dovuto comprare questo libro prima! Ma meglio tardi che mai. Il materiale è stato assimilato e ora puoi darlo a Lisa."

# game/events/max.rpy:689
translate italian BookRead_sex_ed_37326ac9:

    # Max_04 "Вот и последние главы... Кажется, я понял о чём говорила Лиза. Эх, надо было раньше эту книжку прочитать, сейчас бы не тратил на это время! Но лучше поздно, чем никогда. Материал усвоен и теперь можно снова приставать к Лизе."
    Max_04 "Ecco gli ultimi capitoli... Credo di sapere di cosa stesse parlando Lisa. Eh, avrei dovuto leggere questo libro prima, non ci avrei perso tempo ora! Ma meglio tardi che mai. Il materiale era stato imparato e ora potevo infastidire Lisa di nuovo."

# game/events/max.rpy:703
translate italian SearchSecretBook_4d6b282e:

    # Max_10 "{m}Так... И с чего начать поиск? И нужно поспешить: если Алиса меня поймает, то сначала убьёт, а только потом поздоровается...{/m}" nointeract
    Max_10 "{m}Quindi... Allora, da dove comincio a cercare? E dobbiamo sbrigarci: se Alice mi becca, prima mi ucciderà e poi mi saluterà...{/m}" nointeract

# game/events/max.rpy:718
translate italian SearchSecretBook_pillow_9214580d:

    # Max_14 "{m}Нет, здесь её нет... Ну где же эта чёртова книга? Шаги? Нет, показалось...{/m}" nointeract
    Max_14 "{m}No, non è qui... Beh, dov'è il maledetto libro? Passi? No, ho pensato...{/m}" nointeract

# game/events/max.rpy:731
translate italian SearchSecretBook_bed_fdbef300:

    # Max_14 "{m}Нет, тут её точно нет. Ну где же она? Кажется, я слышу шум...{/m}" nointeract
    Max_14 "{m}No, sicuramente non è qui. Beh, dov'è? Credo di sentire un rumore...{/m}" nointeract

# game/events/max.rpy:745
translate italian SearchSecretBook_table_1c89acaf:

    # Max_14 "{m}Может быть, её здесь нет? Или попытаться ещё поискать, на свой страх и риск?{/m}" nointeract
    Max_14 "{m}Forse non è qui? O provare a guardare ancora un po', a proprio rischio e pericolo?{/m}" nointeract

# game/events/max.rpy:761
translate italian SearchSecretBook_wardrobe_b73b38d5:

    # Max_04 "{m}Вот же она! И зачем её так прятать? Любопытная обложка. Запомню-ка я название. Интересно, о чём эта книга? Может быть, погуглить? Так, всё, надо уходить...{/m}"
    Max_04 "{m}Eccola! Perché nasconderlo così? È una copertina curiosa. Mi ricorderò il titolo. Mi chiedo di cosa si tratti. Forse dovrei cercarlo su Google. Va bene, devo andare...{/m}"

# game/events/max.rpy:772
translate italian InstallCam_90e45062:

    # Max_00 "{m}Здесь уже есть камера. Пожалуй, стоит установить её в другом месте.{/m}"
    Max_00 "{m}Qui c'è già una telecamera. Forse dovremmo installarlo da qualche altra parte.{/m}"

# game/events/max.rpy:780
translate italian InstallCam_44699dcc:

    # Max_04 "{m}В этой комнате столько всего может происходить... Думаю, зрителям понравится! Главное - спрятать все провода, чтобы Лиза не заметила новую микро-камеру...{/m}" nointeract
    Max_04 "{m}C'è così tanto da fare in questa stanza... Penso che il pubblico lo adorerà! La cosa principale è nascondere tutti i fili in modo che Lisa non si accorga della nuova microcamera... {/m}" nointeract

# game/events/max.rpy:786
translate italian InstallCam_b9324e31:

    # Max_04 "{m}Пусть зрители посмотрят, чем Алиса занимается в своей комнате, когда её не видят... Я бы и сам был бы рад посмотреть, но пока такой функции у меня нет...{/m}" nointeract
    Max_04 "{m}Lascia che il pubblico veda cosa sta facendo Alice nella sua stanza quando non viene vista... Sarei felice di vederlo da solo, ma non ho ancora quella funzione ... {/m}" nointeract

# game/events/max.rpy:792
translate italian InstallCam_9d11ed16:

    # Max_04 "{m}Конечно, здесь редко происходят события. Зато, когда они происходят, то здесь такое... Думаю, зрители будут рады таким моментам...{/m}" nointeract
    Max_04 "{m}Naturalmente, gli eventi accadono raramente qui. Ma quando accadono, c'è questo... Penso che il pubblico sarà felice di questi momenti... {/m}" nointeract

# game/events/max.rpy:799
translate italian InstallCam_bc72cb94:

    # Max_03 "{m}Теперь через обе камеры можно увидеть всё самое интересное! Девочки любят покрасоваться у зеркала, а моя мама и Эрик, как я заметил, стараются не упустить возможность потрахаться перед этим же зеркалом... Моим зрителям это явно понравится!{/m}" nointeract
    Max_03 "{m}Ora attraverso entrambe le telecamere puoi vedere tutte le più interessanti! Le ragazze amano mostrarsi allo specchio, e mia madre ed Eric, come ho notato, cercano di non perdere l'occasione di scopare davanti allo stesso specchio... I miei spettatori lo adoreranno! {/m}" nointeract

# game/events/max.rpy:805
translate italian InstallCam_3a6027ee:

    # Max_04 "{m}Конечно, с точки зрения морали ставить камеру в ванной сомнительно. Однако, тут и так окно во всю стену. Так что, формально я лишь приоткрыл это окно...{/m}" nointeract
    Max_04 "{m}Naturalmente, dal punto di vista della moralità, è dubbio mettere una macchina fotografica in bagno. Tuttavia, qui e così la finestra in tutto il muro. Quindi, formalmente, ho appena aperto quella finestra... {/m}" nointeract

# game/events/max.rpy:811
translate italian InstallCam_918d8b5b:

    # Max_04 "{m}Уж не знаю, будет ли какой-то толк от этой камеры... Тут так редко что-то происходит... Ну пусть будет. Раз уж взялся всё подключать...{/m}" nointeract
    Max_04 "{m}non so se ci sarà qualche utilità da parte di questa fotocamera... È così raro che qui succeda qualcosa... Bene, lascia che sia. Da quando mi sono impegnato a collegare tutto... {/m}" nointeract

# game/events/max.rpy:818
translate italian InstallCam_3a93939c:

    # Max_04 "{m}Вот теперь зрители смогут насладится всеми мокрыми и блестящими красотами, происходящими во дворе...{/m}" nointeract
    Max_04 "{m}Ora il pubblico potrà godere di tutte le bellezze bagnate e lucenti che si svolgono nel cortile ... {/m}" nointeract

# game/events/max.rpy:824
translate italian InstallCam_3426c2fd:

    # Max_04 "{m}Двор... Тут почти всё время кто-то есть и что-то делает, пока светит солнце. Думаю, тут зрители будут зависать постоянно в надежде увидеть кого-то с голыми сиськами...{/m}" nointeract
    Max_04 "{m}Yard... C'è quasi tutto il tempo qualcuno lì e fa qualcosa mentre il sole splende. Penso che qui il pubblico uscirà costantemente nella speranza di vedere qualcuno con le tette nude ... {/m}" nointeract

# game/events/max.rpy:826
translate italian InstallCam_7711d18e:

    # Max_09 "{m}Пожалуй, из-за большой площади мне стоило бы установить здесь несколько камер, чтобы зрители смогли лучше разглядеть каждую попку, которая тут бывает...{/m}"
    Max_09 "{m}Forse, a causa della vasta area, dovrei installare diverse telecamere qui in modo che il pubblico possa vedere meglio ogni che accade qui... {/m}"

# game/events/max.rpy:860
translate italian SearchSpider_d37d0a80:

    # Max_00 "{m}Так, нужно хорошенько рассмотреть траву...{/m}" nointeract
    Max_00 "{m}Ok, dobbiamo dare una buona occhiata all'erba...{/m}" nointeract

# game/events/max.rpy:865
translate italian SearchSpider_1b27d19f:

    # Max_04 "Ага! Попался! Отлично..."
    Max_04 "sì. Ottenuto! È fantastico..."

# game/events/max.rpy:869
translate italian SearchSpider_f2d13038:

    # Max_00 "{m}Нет, ничего похожего на большого страшного паука тут нет... Может быть, я всех переловил и стоит подождать денёк-другой?{/m}"
    Max_00 "{m}No, niente come un grande ragno spaventoso qui... Forse li ho presi tutti e dovrei aspettare un giorno o due.{/m}"

# game/events/max.rpy:882
translate italian HideSpider_a2694b69:

    # Max_00 "{m}Я могу не успеть как следует припрятать паука, прежде чем Алиса вернется из ванной.{/m}"
    Max_00 "{m}Potrei non avere il tempo di nascondere bene il ragno prima che Alice torni dal bagno.{/m}"

# game/events/max.rpy:887
translate italian HideSpider_ebac98bc:

    # Max_00 "{m}Интересно, что будет, если Алиса заметит паука ночью? Она прибежит за помощью? Вот только этот монстр может сбежать... Так что, чем позже я его спрячу, тем больше шансов на успех...{/m}" nointeract
    Max_00 "{m}Mi chiedo cosa succederebbe se Alice avvistasse il ragno di notte? Verrebbe a chiedere aiuto? Solo che questo mostro potrebbe scappare... Quindi, più tardi lo nascondo, maggiori sono le mie possibilità di successo...{/m}" nointeract

# game/events/max.rpy:890
translate italian HideSpider_9502351b:

    # Max_00 "{m}Что ж, будем надеяться, что паук не сбежит до того, как Алиса ляжет спать...{/m}"
    Max_00 "{m}Beh, speriamo che il ragno non scappi prima che Alice vada a letto...{/m}"

# game/events/max.rpy:933
translate italian ViewLesson_f9eae862:

    # Max_00 "{m}Хорошая штука эти онлайн-курсы - можно научиться всему, не входя из дома! Вот только и стоит это немало...{/m}"
    Max_00 "{m}Buona cosa questi corsi online - puoi imparare tutto senza uscire di casa! Ne vale la pena... {/m}"

# game/events/max.rpy:940
translate italian SearchCigarettes_7d1de5a5:

    # Max_09 "{m}Так... Где же Алиса спрятала сигареты сегодня?{/m}"
    Max_09 "{m}Quindi... Dove ha nascosto le sigarette Alice oggi?{/m}"

# game/events/max.rpy:959
translate italian SearchCigarettes_no_025a648e:

    # Max_10 "{m}Кажется, здесь их нет... Пора уходить, а то если кто-то заметит меня...{/m}" nointeract
    Max_10 "{m}Non sembrano essere qui... È ora di andare, o se qualcuno mi nota...{/m}" nointeract

# game/events/max.rpy:967
translate italian SearchCigarettes_yes_f44440e9:

    # Max_04 "{m}Ага, нашёл! Так... Теперь их нужно положить таким образом, чтобы мама их заметила, если заглянет в комнату...{/m}" nointeract
    Max_04 "{m}Sì, l'ho trovato! Giusto... Ora dobbiamo metterli in modo che la mamma li noti se guarda nella stanza...{/m}" nointeract

# game/events/max.rpy:985
translate italian need_money_7d60b08e:

    # Max_10 "{m}Сегодня уже четверг. Последний день когда я могу заказать подарки девчонкам, чтобы опередить Эрика.{/m}"
    Max_10 "{m}Oggi è già giovedì. L'ultimo giorno in cui posso ordinare i regali per le ragazze per anticipare Eric.{/m}"

# game/events/max.rpy:992
translate italian need_money_d5c0c6aa:

    # Max_11 "{m}Нужно скорее купить платье Алисе и купальник для Лизы, а денег у меня хватает лишь на что-то одно...{/m}"
    Max_11 "{m}Devo comprare un vestito per Alice e un costume da bagno per Lisa, e ho abbastanza soldi solo per una cosa...{/m}"

# game/events/max.rpy:994
translate italian need_money_b39c15a3:

    # Max_11 "{m}Нужно скорее купить платье Алисе и купальник для Лизы, а денег мне не хватит даже на что-то одно...{/m}"
    Max_11 "{m}Devo comprare al più presto un vestito per Alice e un costume da bagno per Lisa, e non ho abbastanza soldi neanche per una cosa...{/m}"

# game/events/max.rpy:1000
translate italian need_money_adfa0cdd:

    # Max_11 "{m}Я уже купил платье Алисе, но на купальник для Лизы мне не хватит денег...{/m}"
    Max_11 "{m}Ho già comprato un vestito per Alice, ma non ho abbastanza soldi per un costume da bagno per Lisa...{/m}"

# game/events/max.rpy:1006
translate italian need_money_ae118bc4:

    # Max_11 "{m}Я уже купил купальник для Лизы, но на платье Алисе мне не хватит денег...{/m}"
    Max_11 "{m}Ho già comprato un costume da bagno per Lisa, ma non ho abbastanza soldi per il vestito di Alice...{/m}"

# game/events/max.rpy:1011
translate italian need_money_aa38a4e7:

    # Max_08 "{m}Сайт у меня есть и уже приносит какую-то прибыль, но нужно время, чтобы раскрутить его.{/m}"
    Max_08 "{m}Ho un sito web e sta già generando qualche reddito, ma ci vuole tempo per farlo funzionare.{/m}"

# game/events/max.rpy:1013
translate italian need_money_6eaa0c79:

    # Max_08 "{m}К сожалению, у меня нет источника доходов, все мои деньги я получаю только от мамы. Ну и от Алисы ещё могу...{/m}"
    Max_08 "{m}Purtroppo non ho nessuna fonte di reddito, tutti i miei soldi vengono solo da mia madre. Posso anche prenderne un po' da Alice...{/m}"

# game/events/max.rpy:1014
translate italian need_money_14f2a995:

    # Max_09 "{m}Нужно поискать какую-нибудь информацию в интернете, может есть возможность получить кредит.{/m}"
    Max_09 "{m}Dovresti cercare qualche informazione su internet, forse c'è la possibilità di ottenere un prestito.{/m}"

# game/events/max.rpy:1034
translate italian cheat_money_46383d7e:

    # "На данном этапе игры у Макса не может быть такой суммы. Взлом игры может привести к непредсказуемым последствиям, в частности, к потере некоторых возможностей и функционала игры, а так же к возникновению критических ошибок, которые не позволят вам продолжить игру."
    "In questa fase del gioco, Max non può avere una tale quantità. L'hacking del gioco può portare a conseguenze imprevedibili, in particolare, alla perdita di alcune caratteristiche e funzionalità del gioco, nonché al verificarsi di errori critici che non ti permetteranno di continuare il gioco."

# game/events/max.rpy:1038
translate italian cheat_money_strateg_16b9bde6:

    # "Вы либо аккуратный взломщик, либо хороший стратег. В любом случае вы не нуждаетесь в дополнительных методах получения денег. Учтите, взлом игры может привести к непредсказуемым последствиям, в частности, к потере некоторых возможностей и функционала игры, а так же к возникновению критических ошибок, которые не позволят вам продолжить игру."
    "O sei un ladro pulito o un buon stratega. In ogni caso, non hai bisogno di metodi aggiuntivi per ottenere denaro. Si prega di notare che l'hacking del gioco può portare a conseguenze imprevedibili, in particolare, alla perdita di alcune caratteristiche e funzionalità del gioco, nonché al verificarsi di errori critici che non ti permetteranno di continuare il gioco."

# game/events/max.rpy:1046
translate italian about_credit_3ac552dc:

    # Max_00 "{m}Итак, поищем, где можно простому парню разжиться деньгами...{/m}"
    Max_00 "{m}Quindi, cerchiamo dove un ragazzo semplice può trovare dei soldi...{/m}"

# game/events/max.rpy:1047
translate italian about_credit_aa91eb63:

    # Max_09 "{m}Это не подходит... Здесь нужно иметь официальное трудоустройство на работе...{/m}"
    Max_09 "{m}Questo non è adatto... Devi avere un impiego ufficiale qui...{/m}"

# game/events/max.rpy:1048
translate italian about_credit_2eacd86e:

    # Max_07 "{m}Ага, а вот это может и подойти - краткосрочные займы начинающим интернет-предпринимателям. О, да, это про меня...{/m}"
    Max_07 "{m}Sì, potrebbe funzionare - prestiti a breve termine agli imprenditori di internet in fase di avvio. Oh, sì, sono io...{/m}"

# game/events/max.rpy:1049
translate italian about_credit_90bb447c:

    # Max_01 "Бла, бла, бла... ... если у Вас есть работающий проект в интернете, мы предоставляем займы на раскрутку Вашего бизнеса..."
    Max_01 "Bla, bla, bla... ... Se hai un progetto di lavoro su Internet, forniamo prestiti per promuovere il tuo business..."

# game/events/max.rpy:1051
translate italian about_credit_73692d65:

    # Max_04 "{m}Подытожим условия: \n{b}В течение месяца нужно вернуть всю сумму займа + 10%% \nВ случае не погашения в срок, сумма долга утраивается каждые 30 дней, а с моего сайта будут ежедневно изымать половину прибыли. И занять ещё раз уже не получится...{/b} \n\nЛучше, конечно же, до такого не доводить.{/m}"
    Max_04 "{m}Riassumiamo le condizioni: \n{b}Entro un mese, è necessario rimborsare l'intero importo del prestito + 10%% \nSe non ripago in tempo, il debito si triplica ogni 30 giorni, e la metà dei profitti sarà presa dal mio sito ogni giorno. E non ci sarà più modo di prendere in prestito...{/b} \n\nMeglio, ovviamente, non portarlo a questo.{/m}"

# game/events/max.rpy:1052
translate italian about_credit_5011fb20:

    # Max_05 "{m}Регистрируюсь... Указываю свои реквизиты... Свой сайт в качестве источника дохода... Готово!{/m}"
    Max_05 "{m}Registrazione... Fornendo i miei dati... Il mio sito web come fonte di reddito... Fatto!{/m}"

# game/events/max.rpy:1053
translate italian about_credit_ec914392:

    # Max_02 "{m}Теперь я могу взять кредит, если срочно понадобятся деньги. Главное вовремя его погасить, чтобы проблем не было...{/m}"
    Max_02 "{m}Ora posso prendere un prestito se ho bisogno di soldi urgentemente. L'importante è pagare in tempo, così non ci saranno problemi...{/m}"

# game/events/max.rpy:1056
translate italian about_credit_dc6f567b:

    # Max_16 "{m}Вот чёрт, а у меня нет никакого проекта! Получается, что денег мне никто не даст.{/m}"
    Max_16 "{m}Oh, merda, non ho nessun progetto! A quanto pare nessuno mi darà dei soldi.{/m}"

# game/events/max.rpy:1067
translate italian getting_load_f43833f1:

    # Max_00 "{m}Сколько мне сейчас нужно занять?{/m}" nointeract
    Max_00 "{m}Quanto devo prendere in prestito ora?{/m}" nointeract

# game/events/max.rpy:1087
translate italian return_part_loan_67582e30:

    # Max_00 "{m}Сколько я верну сейчас?{/m}" nointeract
    Max_00 "{m}Quanto mi verrà restituito ora?{/m}" nointeract

# game/events/max.rpy:1112
translate italian bedtime_thoughts_a9917f30:

    # Max_07 "{m}Как же напакостить Эрику, чтобы он хотя бы на время перестал лезть к моим сёстрам... и Кире... да и к маме тоже... Хм... У меня была бы отличная возможность что-нибудь подмешать в его еду за ужином, если помочь Алисе накрыть на стол! Стоит посмотреть в интернет-магазине, можно ли что-то такое купить...{/m}"
    Max_07 "{m}Come attaccare Eric, in modo che almeno per un po 'abbia smesso di arrampicarsi sulle mie sorelle ... e Kira... e anche a mia mamma... Hmm... Avrei una grande opportunità di mescolare qualcosa nel suo pasto a cena se aiutassi Alice ad impostare la tavola! Vale la pena cercare nel negozio online, se è possibile acquistare qualcosa di simile ... {/m}"

# game/events/max.rpy:1147
translate italian SearchPhone_no_9a51253a:

    # Max_10 "{m}Здесь телефона Лизы нет.{/m}"
    Max_10 "{m}Il telefono di Lisa non è qui.{/m}"

# game/events/max.rpy:1155
translate italian SearchPhone_yes_48a26421:

    # Max_07 "{m}А вот и телефон Лизы! Надеюсь, диктофон на нём есть, иначе я влип...{/m}"
    Max_07 "{m}Ecco il telefono di Lisa! Spero che ci sia un registratore, altrimenti sono fregato...{/m}"

# game/events/max.rpy:1157
translate italian SearchPhone_yes_075e9758:

    # Max_09 "{m}Да, диктофон есть. Теперь, пока Лиза не вернулась, нужно идти к Эрику и попробовать вывести его на чистую воду...{/m}" nointeract
    Max_09 "{m}Sì, c'è un registratore. Ora, prima che Lisa torni, dobbiamo andare da Eric e cercare di smascherarlo...{/m}" nointeract

translate italian strings:

    # game/events/max.rpy:31
    old "{i}установить будильник{/i}"
    new "{i}impostare l'allarme{/i}"

    # game/events/max.rpy:34
    old "{i}в 6 утра{/i}"
    new "{i}alle 6 del.m{/i}"

    # game/events/max.rpy:34
    old "{i}в 7 утра{/i}"
    new "{i}alle 7 del.m{/i}"

    # game/events/max.rpy:84
    old "{i}вздремнуть{/i}"
    new "{i}fare un pisolino{/i}"

    # game/events/max.rpy:120
    old "{i}подремать пару часов{/i}"
    new "{i}fare un pisolino per un paio d'ore{/i}"

    # game/events/max.rpy:120
    old "{i}подремать 3 часа{/i}"
    new "{i}fare un pisolino per 3 ore{/i}"

    # game/events/max.rpy:120
    old "{i}подремать 4 часа{/i}"
    new "{i}fare un pisolino per 4 ore{/i}"

    # game/events/max.rpy:120
    old "{i}подремать 5 часов{/i}"
    new "{i}fare un pisolino per 5 ore{/i}"

    # game/events/max.rpy:120
    old "{i}не-а, может позже...{/i}"
    new "{i}no, forse più tardi... {/i}"

    # game/events/max.rpy:278
    old "{i}почитать о камерах{/i}"
    new "{i}leggi le telecamere{/i}"

    # game/events/max.rpy:280
    old "{i}читать о блогах{/i}"
    new "{i}leggi di blog{/i}"

    # game/events/max.rpy:282
    old "{i}узнать о книге Алисы{/i}"
    new "{i}conoscere il libro di Alice{/i}"

    # game/events/max.rpy:284
    old "{i}читать о пауках{/i}"
    new "{i}leggi di spiders{/i}"

    # game/events/max.rpy:286
    old "{i}искать информацию по кредитам{/i}"
    new "{i}cercare informazioni sui prestiti{/i}"

    # game/events/max.rpy:357
    old "Оплатить всё ($100)"
    new "Paga per tutto (€ 100)"

    # game/events/max.rpy:364
    old "Настроить работу сайта"
    new "Personalizza il sito"

    # game/events/max.rpy:418
    old "Собрать статистику"
    new "Raccogli statistiche"

    # game/events/max.rpy:418
    old "Просмотреть популярные блоги"
    new "Visualizza i blog più popolari"

    # game/events/max.rpy:418
    old "Почитать комменты на блогах"
    new "Leggi i commenti sui blog"

    # game/events/max.rpy:421
    old "Проанализировать результаты"
    new "Analizza i risultati"

    # game/events/max.rpy:421
    old "Построить таблицу"
    new "Tabella di compilazione"

    # game/events/max.rpy:421
    old "Нарисовать график"
    new "Disegnare un grafico"

    # game/events/max.rpy:424
    old "Хм... Так... Ага. Это сюда запишем, это сюда..."
    new "Hmm... Così... Aha. Scriviamolo qui, è qui..."

    # game/events/max.rpy:426
    old "Так. Из этой таблицы мы делаем вывод. Ага. Вот это значит, что... Нет, не так. Вот. Вроде получилось..."
    new "Così. Da questa tabella traiamo una conclusione. Aha. Ciò significa che... No, non lo è. Qui. Sembra che si sia scoperto..."

    # game/events/max.rpy:428
    old "Рисователь графиков из меня ещё тот. Но попробуем. Так, это шкала популярности, это... Ага. Кажется, всё сходится..."
    new "Il cassetto del grafico da me è sempre lo stesso. Ma proviamoci. Quindi, è una scala di popolarità, è... Aha. Tutto sembra andare di pari passo..."

    # game/events/max.rpy:430
    old "Составить список"
    new "Crea una lista"

    # game/events/max.rpy:430
    old "Отсортировать..."
    new "Sorta..."

    # game/events/max.rpy:430
    old "Сравнить количество лайков..."
    new "Confronta il numero di Mi piace..."

    # game/events/max.rpy:433
    old "Так, вычёркиваем из списка этих, вот этих и тех. Что тут у нас остаётся?"
    new "Quindi, sappiamo dall'elenco di questi, questi e quelli. Che cosa ci resta qui?"

    # game/events/max.rpy:435
    old "Так, сравниваем аудиторию. Время публикации... Исключаем сомнительный контент..."
    new "Quindi, confrontiamo il pubblico. Tempo di pubblicazione... Escludi i contenuti discutibili..."

    # game/events/max.rpy:437
    old "Сортируем по количеству лайков. Убираем тех, кто с дизлайками больше этого процента..."
    new "Ordina in ordine il numero di Mi piace. Rimuoviamo coloro che hanno antipatie più di questa percentuale..."

    # game/events/max.rpy:439
    old "Воспользоваться поиском"
    new "Usa la ricerca"

    # game/events/max.rpy:439
    old "Читать всё подряд"
    new "Leggi tutto"

    # game/events/max.rpy:439
    old "Выбрать лучшие комменты"
    new "Seleziona i commenti migliori"

    # game/events/max.rpy:442
    old "Так, в поиске мы видим любопытные результаты. Так, выберем то что нам подходит..."
    new "Quindi, nella ricerca vediamo risultati interessanti. Quindi, scegliamo quello che fa per noi..."

    # game/events/max.rpy:444
    old "Ох. Всё оказалось ещё хуже. Кажется, тут нет ничего полезного... Хотя. Думаю, можно сделать даже какой-то вывод..."
    new "Oh. Era anche peggio. Non sembra esserci nulla di utile qui... Anche se. Penso che tu possa anche trarre qualche conclusione..."

    # game/events/max.rpy:446
    old "Так, топовые комменты все сводятся к одному. Не может быть?"
    new "Quindi, i commenti in alto si riducono tutti a uno. Assolutamente no?"

    # game/events/max.rpy:448
    old "Сделать вывод..."
    new "Dedurre..."

    # game/events/max.rpy:464
    old "{i}читать о книге{/i}"
    new "{i}letto del libro{/i}"

    # game/events/max.rpy:480
    old "Искать виды насекомых по регионам"
    new "Cerca specie di insetti per regione"

    # game/events/max.rpy:480
    old "Читать где водятся какие пауки"
    new "Leggi dove ci sono ragni"

    # game/events/max.rpy:487
    old "Выяснить, чем питается..."
    new "Scopri cosa mangia..."

    # game/events/max.rpy:487
    old "Почитать о повадках..."
    new "Leggi le abitudini..."

    # game/events/max.rpy:648
    old "{i}читать \"WEB STANDARDS\"{/i}"
    new "{i}leggere \"WEB STANDARDS\"{/i}"

    # game/events/max.rpy:648
    old "{i}читать \"СЕКС-ОБРАЗОВАНИЕ\"{/i}"
    new "{i}leggere \"EDUCAZIONE SESSUALE\"{/i}"

    # game/events/max.rpy:702
    old "{i}искать под подушкой{/i}"
    new "{i}guarda sotto il cuscino{/i}"

    # game/events/max.rpy:702
    old "{i}искать в шкафу{/i}"
    new "{i}guarda nell'armadio{/i}"

    # game/events/max.rpy:702
    old "{i}прекратить поиски{/i}"
    new "{i}smetti di cercare{/i}"

    # game/events/max.rpy:859
    old "{i}искать...{/i}"
    new "{i}alla ricerca di... {/i}"

    # game/events/max.rpy:886
    old "{i}Подложить сейчас{/i}"
    new "{i}Metti ora{/i}"

    # game/events/max.rpy:932
    old "Вы просматриваете видеоурок и повышаете свои навыки."
    new "Guarda un video tutorial e migliora le tue abilità."

    # game/events/max.rpy:966
    old "{i}подставить Алису{/i}"
    new "{i}sostituto Alice{/i}"

    # game/events/max.rpy:966
    old "{i}не подставлять Алису{/i}"
    new "{i}non sostituire Alice{/i}"

    # game/events/max.rpy:1066
    old "$500"
    new "€500"

    # game/events/max.rpy:1066
    old "$1000"
    new "€1000"

    # game/events/max.rpy:1066
    old "$2000"
    new "€2000"

    # game/events/max.rpy:1066
    old "$5000"
    new "€5000"

    # game/events/max.rpy:1086
    old "$50"
    new "€50"

    # game/events/max.rpy:1086
    old "$100"
    new "€100"

    # game/events/max.rpy:1086
    old "$200"
    new "€200"

    # game/events/max.rpy:1156
    old "{i}идти к Эрику с включённым диктофоном{/i}"
    new "{i}per andare da Eric con il registratore acceso{/i}"

